			...
			<div class="related span2 marginLeftZero">
				<div class="">
					<ul class="nav nav-stacked nav-pills">
						...
						{if (vtranslate($RELATED_LINK->getLabel(),{$MODULE_NAME}) eq 'Contact Details') || (vtranslate($RELATED_LINK->getLabel(),{$MODULE_NAME}) eq 'Project Details')}<div id="murali" style="margin:12px;" onClick="javaScript:timedRefresh();">{/if}
						<li class="{if $RELATED_LINK->getLabel()==$SELECTED_TAB_LABEL}active{/if}" data-url="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}" data-label-key="{$RELATED_LINK->getLabel()}" data-link-key="{$RELATED_LINK->get('linkKey')}" >
							<a href="javascript:void(0);" class="textOverflowEllipsis" style="width:auto" title="{vtranslate($RELATED_LINK->getLabel(),{$MODULE_NAME})}"><strong
							>{vtranslate($RELATED_LINK->getLabel(),{$MODULE_NAME})}</strong></a>
						</li>
						{if (vtranslate($RELATED_LINK->getLabel(),{$MODULE_NAME}) eq 'Contact Details') || (vtranslate($RELATED_LINK->getLabel(),{$MODULE_NAME}) eq 'Project Details')}</div>{/if}
						{/foreach}
