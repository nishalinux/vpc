...
Vtiger_Header_Js.extend('MailManager_QuickCreate_Js', {
	...
	registerQuickCreatePostLoadEvents: function(form, params) {
		...
		//form.find('#goToFullForm').remove();//modified by murali to display GoToFullform button mail manager quick create popup
		
		//added by murali to display GoToFullform button mail manager quick create popup--Start
		form.on('click','#goToFullForm',function(e) {
		var form = jQuery(e.currentTarget).closest('form');
        var editViewUrl = jQuery(e.currentTarget).data('editViewUrl');
			thisInstance.quickCreateGoToFullForm(form, editViewUrl);
			jQuery(".cancelLink").click();// to close popup after clicking on GoToFullform button
		});
		//added by murali to display GoToFullform button mail manager quick create popup--End
		...
		*/
		...
		folder_open: function(name, page){
			...
			if(jQuery('#search_type').val()) {
				vttype = jQuery('#vttype').val()
				query += "&type=" + encodeURIComponent(vttype);
			}
			...
			var progressIndicatorElement = jQuery.progressIndicator({
				...
				"&_page=" + encodeURIComponent(page) + query).then(function(response) { 
					...
					var type = jQuery('#search_type').val(vttype);
									  jQuery('#search_type > option').removeAttr("selected");
  
			jQuery("#search_type option[value='" + vttype + "']").attr("selected","selected");
		...
		refreshCurrentFolder: function(){
			var fVal = jQuery('#ck_open_close').val();//murali
			//console.log("flagValue Is=="+fVal);//murali					
			if(fVal != 'open'){//murali
		...
		mail_find_relationship: function(){
            ...
            AppConnector.request(MailManager._baseurl() + "_operation=relation&_operationarg=find&_mfrom=" + encodeURIComponent(meta['from']) +'&_msendto='+ encodeURIComponent(meta['to']) +'&_folder=' +encodeURIComponent(meta['folder']) +'&_msgno=' +encodeURIComponent(meta['msgno']) +'&_msguid=' +encodeURIComponent(meta['msguid'].replace('<', '&lt;').replace('>', '&gt;'))).then(function(responseJSON) { 
		...
		mail_reply: function(all){
			if (typeof(all) == 'undefined')
				all = true;
var from = jQuery('#_mailopen_from').html();
			var lguser = jQuery('.login_user').val();           //added by madhav
			var cd = all ? jQuery('#_mailopen_cc').html() : '';//added by madhav
			
		
			var md = all ? jQuery('#_mailopen_to').html() : '';//added by madhav
				if(cd != null){
					if (cd.startsWith(lguser) == true && cd.endsWith(",") == true){
						cd = cd.replace(lguser+",", '');
						}else if(cd == lguser && cd.endsWith(",") == false){
							cd = cd.replace(lguser, '');
							} else {
							cd = cd.replace(", "+lguser, '');
							}
					if (md.startsWith(lguser) == true){
						if(md.indexOf(',') == -1){
						md = md.replace(lguser, '');	
						} else {
						md = md.replace(lguser+",", '');
						}
			...
			Vtiger_Index_Js.showComposeEmailPopup(params, function(win){
				if (typeof win != 'undefined') {
					setTimeout(function() {fillComposeEmailForm(win);}, 5000);
				}
			});
		},
		...
		mail_forward: function(messageId){
			...
			var fwdBody = MailManager.sprintf('<p></p><p style="margin:0;padding:0;">%s, %s, %s:</p><br/><p>%s<br/>%s</p>%s', 'On ' + date, from, 'wrote', MailManager.i18n('JSLBL_FORWARD_MESSAGE_TEXT'), fwdMsgMetaInfo, body); // MODIFIED BY MADHAV

		

			var attachmentCount = jQuery("#mmAttchmentCount").val();
			if(attachmentCount) {
				VtigerJS_DialogBox.block();
					...
					"&body=" + encodeURIComponent(fwdBody)).then(function(responseJSON) {
                                            responseJSON = JSON.parse(responseJSON);
		...
		save_draft: function(form){
			...
			alert('savedraft 1353');
						alert('savedraft 13534');
		...
		search_mails: function(foldername){
			...
			MailManager.Request(baseurl, params, function(response){
				...
				var element = jQuery('#search_txt');
				  jQuery('#search_type > option').removeAttr("selected");
								jQuery('#vttype').val(type);

			jQuery("#search_type option[value='" + type + "']").attr("selected","selected");
		...
		addRequiredElements : function() {
			...
			jQuery('#search_type > option').removeAttr("selected");
			jQuery('#vttype').val(option);

			jQuery("#search_type option[value='" + option + "']").attr("selected","selected");
			var dateformat = jQuery('#jscal_dateformat').val();
			var element = jQuery('#search_txt');
			if(option == 'ON') {
				element.closest('div').addClass('date')
