		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			if ($insertion_mode == 'edit') {
				if ($table_name != 'vtiger_ticketcomments' && $uitype != 4) {
					if(($uitype != '515' && $uitype != '514') && $module == 'ProjectTask')  {
			...
			} elseif(($uitype != '515' && $uitype != '514') && $module == 'ProjectTask')  {
	...
	function retrieve_entity_info($record, $module) {
		...
		if ($cachedModuleFields) {
			...
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				if(($module == 'Invoice' || $module == 'Quotes' || $module == 'SalesOrder' || $module == 'PurchaseOrder' || $module == 'PickingSlip' || $module == 'ProjectStock')
...
//Added by Richie
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		if ($numOfFields > 0) {
			...
			if(($currentModule == 'Contacts' || $currentModule == 'Leads') && $related_module == 'PBXManager'){
			$query = "SELECT vtiger_crmentity.*, $other->table_name.*,vtiger_pbxmanagercf.cf_1077";	//added by murali to display time zone values in related list of contacts and leads
			}else{
			$query = "SELECT vtiger_crmentity.*, $other->table_name.*";
			}
			$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>'vtiger_users.first_name',
			...
			$query .= " INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = $other->table_name.$other->table_index";
			if(($currentModule == 'Contacts' || $currentModule == 'Leads') && $related_module == 'PBXManager'){
			$query .= " INNER JOIN vtiger_pbxmanagercf ON vtiger_pbxmanagercf.pbxmanagerid = vtiger_pbxmanager.pbxmanagerid";	//added by murali to display time zone values in related list of contacts and leads
			}
			...
			//echo ">---->".$currentModule."/".$related_module;
