        ...
        <table class="table table-bordered listViewEntriesTable">
            ...
            {foreach item=RELATED_RECORD from=$RELATED_RECORDS}
				...
				{/if}
							...
							{assign var="EMAILATTACHMENTS" value=$RELATED_RECORD->getEmailRelatedAttachmentDetails({$RELATED_RECORD->getId()})}
									{foreach $EMAILATTACHMENTS as $k=>$ATTACHMENTDETAILS}
											<a href="{$ATTACHMENTDETAILS['path']}/{$ATTACHMENTDETAILS['fileid']}_{$ATTACHMENTDETAILS['attachment']}" target="_blank">{$ATTACHMENTDETAILS['attachment']}</a>
										<br/>
									{/foreach}
                            {else}
                                {$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}
