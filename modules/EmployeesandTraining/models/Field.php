 ...
 */
class EmployeesandTraining_Field_Model extends Vtiger_Field_Model {
	...
	public function isAjaxEditable() {
		$ajaxRestrictedFields = array('4','72','515','514','537','538','545','546','516','540','19');
		if(!$this->isEditable() || in_array($this->get('uitype'), $ajaxRestrictedFields)) {
			return false;
		}
		//echo $this->getFieldName();
		if($this->getFieldName() == 'cf_2177'){
			return false;
		}
		return true;
	}
	...
	public function getAllRolesCRM(){
		$db = PearDatabase::getInstance();
		$params = array();
		$sql = 'SELECT * FROM vtiger_role where depth !=0';		
		...
		for ($i=0; $i<$noOfRoles; ++$i) {
			...
			$roleslist[ $role['rolename']] =$role['roleid'];
		}
		return $roleslist;
    }
}
