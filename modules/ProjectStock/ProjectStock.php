<?php
class ProjectStock extends CRMEntity {
    var $log;
    var $db;
    var $table_name = "vtiger_projectstock";
    var $table_index = 'projectstockid';
    var $tab_name = Array('vtiger_crmentity', 'vtiger_projectstock', 'vtiger_projectstockcf','vtiger_inventoryproductrel');
    var $tab_name_index = Array('vtiger_crmentity' => 'crmid', 'vtiger_projectstock' => 'projectstockid', 'vtiger_projectstockcf' => 'projectstockid','vtiger_inventoryproductrel'=>'id');
    var $customFieldTable = Array('vtiger_projectstockcf', 'projectstockid');
    var $entity_table = "vtiger_crmentity";
    var $object_name = "ProjectStock";
    var $new_schema = true;
    var $update_product_array = Array();
    var $column_fields = Array();
    var $sortby_fields = Array('projectstock_no', 'smownerid', 'createdtime');
    // This is used to retrieve related vtiger_fields from form posts.
    var $additional_column_fields = Array('smownerid', 'projectstock_no');
    
    var $list_fields = Array(
        'ProjectStock No' => Array('projectstock', 'projectstock_no'),
        'Project' => Array('projectstock' => 'projectid'),
        'Created Time' => Array('projectstock' => 'createdtime'),
        'Assigned To' => Array('crmentity' => 'smownerid')
    );
    
    var $list_fields_name = Array(
        'ProjectStock No' => 'projectstock_no',
        'Project' => 'projectid',
        'Created Time' => 'createdtime',
        'Assigned To' => 'assigned_user_id'
    );
    
    var $list_link_field = 'projectstock_no';
    ...
    var $mandatory_fields = Array('projectstock_no', 'createdtime', 'modifiedtime', 'assigned_user_id');
    var $def_basicsearch_col = 'projectstock_no';
    var $isLineItemUpdate = true;

    /** 
     * Constructor Function for projectstock class
     * This function creates an instance of LoggerManager class using getLogger method
     * creates an instance for PearDatabase class and get values for column_fields array of projectstock class.
     */
    function ProjectStock() {
        $this->log = LoggerManager::getLogger('ProjectStock');
        $this->db = PearDatabase::getInstance();
        $this->column_fields = getColumnFields('ProjectStock');
    }

    function save_module($module) {
        $db = PearDatabase::getInstance();
        if ($_REQUEST['action'] != 'ProjectStockAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW' && $_REQUEST['action'] != 'MassEditSave' && $_REQUEST['action'] != 'ProcessDuplicates' && $_REQUEST['action'] != 'SaveAjax' && $this->isLineItemUpdate != false) {
            $recordModel = Vtiger_Record_Model::getInstanceById($this->id, 'ProjectStock');
            
            //get the old qty values for the products
            $oldRelatedProducts = $recordModel->getProducts();
                    
            saveInventoryProductDetails($this, 'ProjectStock');
    ...
    function get_history($id) {
        ...
        return getHistory('ProjectStock', $query, $id);
    }
