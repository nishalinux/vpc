...
function GetForgotPasswordUI($mail_send_message='')
{
    ...
    $list .= '<body class="bg-gray"> <img src="images/TheraCann_Logo.png" width="169" height="69"  class="logo"> ';
...
if($_REQUEST['mail_send_message'] != '')
{
	...
	if($mail_send_message[0] == 'true')
	{
		...
		$login_error_msg = getTranslatedString("Mail has been sent to your mail id with the customer portal login details");
		$login_error_msg = base64_encode('<font color=green size=2px;> '.$login_error_msg.' </font>');
		header("Location: login.php?login_error=$login_error_msg");
		exit;
	}
	elseif($mail_send_message[0] == 'false')
	{
		$login_error_msg = "Invalid Email Address";
		
		$login_error_msg = base64_encode('<font color=red size=2px;> '.$login_error_msg.' </font>');
		header("Location: login.php?login_error=$login_error_msg");
		exit;
		//echo $list;
	}
