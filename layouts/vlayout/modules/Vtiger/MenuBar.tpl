...
{strip}
	...
	<div class="navbar" id="topMenus" style="overflow: hidden;height:40px;">
		<div class="navbar-inner" id="nav-inner">
					...
					<span id="headerLinksBig" class="pull-right headerLinksContainer">
						...
						{foreach key=index item=obj from=$HEADER_LINKS}
							...
							<span class="dropdown span{if !empty($src)} settingIcons {/if}">
									...
									{if !empty($src)}
									{if $title == 'LBL_FEEDBACK'} <a id="menubar_item_right_{$title}"    href="#"><img src="{$src}" alt="{vtranslate($title,$MODULE)}" title="{vtranslate($title,$MODULE)}" /></a>
			...
			<br><br><br><br>
			<span style="  width: 590px; height: 500px; background:lightgrey; align:centre; margin: 0 auto; box-shadow: 1px 1px 1px 1px black; display: none;" id="popup">
     <iframe style="width: 100%; height: 90% " id="popupFrame" src="">Help</iframe>
     </span>
		</div>
	</div>
...
function displayPopup() {
  ...
  frame.src = "https://demo.theracanncorp.com/Homestuff.php?module={$MODULE}";
