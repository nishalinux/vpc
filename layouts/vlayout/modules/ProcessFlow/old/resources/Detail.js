...
Vtiger_Detail_Js("ProcessFlow_Detail_Js",{},{
	...
	registerEventForRelatedTabClick : function(){
		...
		jQuery('.related', detailContainer).on('click', 'li', function(e, urlAttributes){
			...
			//Panels Auto Reload Purpose
			var urlParams = {};
			url.replace(
				new RegExp("([^?=&]+)(=([^&]*))?", "g"),function($0, $1, $2, $3) {
					urlParams[$1] = $3;
			}); 
			if(urlParams.requestMode !='' && urlParams.module != ''){
	...
	registerSummaryViewContainerEvents : function(summaryViewContainer){
		...
		jQuery('.changeDetailViewMode').on('click',function(e){
			...
			if(urlParams.requestMode !='' && urlParams.module != ''){
				element.progressIndicator({'mode': 'hide'});
				window.location = url+'&tab_label='+urlParams.module+" Details";
				return false;
			}
			//Ended Here
		});
	...
	registerEvents : function(){
		...
		this.registerEventForRelatedTabClick();//Panels purpose
	}
});
