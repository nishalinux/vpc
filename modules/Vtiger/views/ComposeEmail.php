...
class Vtiger_ComposeEmail_View extends Vtiger_Footer_View {
	...
	public function composeMailData($request){
		...
		$viewer->assign('MAX_UPLOAD_SIZE', vglobal('upload_maxsize'));
		 $moduleArray = $moduleModel->getEmailRelatedModules();//SL added to display dropdown in sort order:11th august 16
			sort($moduleArray);//SL added to display dropdown in sort order:11th august 16
		$viewer->assign('RELATED_MODULES',$moduleArray );//SL added to display dropdown in sort order:11th august 16
		if ($documentIds) {
			$attachements = array();
