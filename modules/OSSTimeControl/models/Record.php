...
Class OSSTimeControl_Record_Model extends Vtiger_Record_Model
{
	...
	public static function recalculateTimeControl($id, $name)
	{
		...
		$sumTime = number_format($db->query_result($result, 0, 'sum'), 2);
		...
		if ($result->rowCount()) {
			...
			$db->pquery('UPDATE '.$table.' SET sum_time=? WHERE '.$focus->table_index.'=?;', [$sumTime, $id]);
		}
	}
	...
	public static function setSumTime($data)
	{
		...
		$db->pquery('UPDATE vtiger_osstimecontrol SET sum_time=? WHERE crmiosstimecontrolidd=?;', [$time, $data->getId()]);
	}
