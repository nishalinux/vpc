 ...
 ********************************************************************************/
 // This is the proforma vtDZiner SampleAnalysis.php script which will get converted as the written out as SampleAnalysis.php
...
// vtiger_sampleanalysis is a custom vtiger entity generated by vtDZiner Module DZiner
class SampleAnalysis extends CRMEntity {
	var $log;
	var $db;

	var $module_name="SampleAnalysis";
	var $table_name = "vtiger_sampleanalysis";
	var $table_index= 'sampleanalysisid';

	var $tab_name = Array('vtiger_crmentity','vtiger_sampleanalysis','vtiger_sampleanalysiscf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_sampleanalysis'=>'sampleanalysisid','vtiger_sampleanalysiscf'=>'sampleanalysisid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_sampleanalysiscf', 'sampleanalysisid');
	...
	var $list_link_field= 'sampleanalysisname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'sampleanalysisname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'sampleanalysisname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_sampleanalysisgrouprelation','sampleanalysisid');
	function SampleAnalysis() {
		$this->log = LoggerManager::getLogger('sampleanalysis');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('SampleAnalysis');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['SAMPLEANALYSIS_SORT_ORDER'] != '')?($_SESSION['SAMPLEANALYSIS_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'SampleAnalysisname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['SAMPLEANALYSIS_ORDER_BY'] != '')?($_SESSION['SAMPLEANALYSIS_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		require('user_privileges/user_privileges_'.$current_user->id.'.php');
        	...
        	$tab_id = getTabid("SampleAnalysis");
		...
		if($account_required)
		{
			$query = "SELECT vtiger_sampleanalysis.sampleanalysisid,  vtiger_sampleanalysis.sampleanalysisname, vtiger_sampleanalysis.dateclosed FROM vtiger_sampleanalysis, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_sampleanalysis.sampleanalysisid, vtiger_sampleanalysis.sampleanalysisname, vtiger_crmentity.smcreatorid FROM vtiger_sampleanalysis inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_sampleanalysis.sampleanalysisid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('SampleAnalysis',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export SampleAnalysis Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("SampleAnalysis", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_sampleanalysis
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_sampleanalysis.sampleanalysisid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_sampleanalysiscf on vtiger_sampleanalysiscf.sampleanalysisid=vtiger_sampleanalysis.sampleanalysisid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_sampleanalysis.sampleanalysisid, vtiger_sampleanalysis.sampleanalysisname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_sampleanalysis
					inner join vtiger_contsampleanalysisrel on vtiger_contsampleanalysisrel.sampleanalysisid = vtiger_sampleanalysis.sampleanalysisid
					inner join vtiger_contactdetails on vtiger_contsampleanalysisrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_sampleanalysis.sampleanalysisid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_sampleanalysis on vtiger_sampleanalysis.sampleanalysisid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'SampleAnalysis'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_sampleanalysis ON vtiger_sampleanalysis.sampleanalysisid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_sampleanalysis.sampleanalysisid = $id";
	...
	/**
	* Function to get SampleAnalysis related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('SampleAnalysis',$query,$id);
	}


	  /**
	  * Function to get SampleAnalysis related Quotes
	  * @param  integer   $id  - sampleanalysisid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'sampleanalysis_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_sampleanalysis.sampleanalysisname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_sampleanalysis on vtiger_sampleanalysis.sampleanalysisid=vtiger_quotes.sampleanalysisid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contsampleanalysisrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contsampleanalysisrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contsampleanalysisrel"=>"sampleanalysisid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"sampleanalysisid","vtiger_salesorder"=>"sampleanalysisid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_sampleanalysis","sampleanalysisid");
		$query .= " left join vtiger_crmentity as vtiger_crmentitySampleAnalysis on vtiger_crmentitySampleAnalysis.crmid=vtiger_sampleanalysis.sampleanalysisid and vtiger_crmentitySampleAnalysis.deleted=0
		left join vtiger_sampleanalysiscf on vtiger_sampleanalysiscf.sampleanalysisid = vtiger_sampleanalysis.sampleanalysisid
		left join vtiger_groups vtiger_groupsSampleAnalysis on vtiger_groupsSampleAnalysis.groupid = vtiger_crmentitySampleAnalysis.smownerid
		left join vtiger_users as vtiger_usersSampleAnalysis on vtiger_usersSampleAnalysis.id = vtiger_crmentitySampleAnalysis.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_sampleanalysis"=>"sampleanalysisid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_sampleanalysis"=>"sampleanalysisid"),
			"Quotes" => array("vtiger_quotes"=>array("sampleanalysisid","quoteid"),"vtiger_sampleanalysis"=>"sampleanalysisid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("sampleanalysisid","salesorderid"),"vtiger_sampleanalysis"=>"sampleanalysisid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_sampleanalysis"=>"sampleanalysisid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-SampleAnalysis Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contsampleanalysisrel WHERE sampleanalysisid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
