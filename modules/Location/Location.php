 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Location.php script which will get converted as the written out as Location.php
...
// vtiger_location is a custom vtiger entity generated by vtDZiner Module DZiner
class Location extends CRMEntity {
	var $log;
	var $db;
	...
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_location'=>'locationid','vtiger_locationcf'=>'locationid');
	/**
	...
	var $list_fields = Array(
			'Location'=>Array('location'=>'locationname'),
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'locationname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'locationname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_locationgrouprelation','locationid');
	function Location() {
		$this->log = LoggerManager::getLogger('location');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Location');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['LOCATION_SORT_ORDER'] != '')?($_SESSION['LOCATION_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Locationname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['LOCATION_ORDER_BY'] != '')?($_SESSION['LOCATION_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_location.locationid,  vtiger_location.locationname, vtiger_location.dateclosed FROM vtiger_location, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_location.locationid, vtiger_location.locationname, vtiger_crmentity.smcreatorid FROM vtiger_location inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_location.locationid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Location',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Location", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				...
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_location.locationid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_locationcf on vtiger_locationcf.locationid=vtiger_location.locationid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_location.locationid, vtiger_location.locationname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_location
					inner join vtiger_contlocationrel on vtiger_contlocationrel.locationid = vtiger_location.locationid
					inner join vtiger_contactdetails on vtiger_contlocationrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_location.locationid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_location on vtiger_location.locationid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Location'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_location ON vtiger_location.locationid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_location.locationid = $id";
	...
	/**
	* Function to get Location related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Location',$query,$id);
	}


	  /**
	  * Function to get Location related Quotes
	  * @param  integer   $id  - locationid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'location_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_location.locationname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_location on vtiger_location.locationid=vtiger_quotes.locationid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contlocationrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contlocationrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contlocationrel"=>"locationid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"locationid","vtiger_salesorder"=>"locationid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_location","locationid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityLocation on vtiger_crmentityLocation.crmid=vtiger_location.locationid and vtiger_crmentityLocation.deleted=0
		left join vtiger_locationcf on vtiger_locationcf.locationid = vtiger_location.locationid
		left join vtiger_groups vtiger_groupsLocation on vtiger_groupsLocation.groupid = vtiger_crmentityLocation.smownerid
		left join vtiger_users as vtiger_usersLocation on vtiger_usersLocation.id = vtiger_crmentityLocation.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_location"=>"locationid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_location"=>"locationid"),
			"Quotes" => array("vtiger_quotes"=>array("locationid","quoteid"),"vtiger_location"=>"locationid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("locationid","salesorderid"),"vtiger_location"=>"locationid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_location"=>"locationid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Location Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contlocationrel WHERE locationid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
