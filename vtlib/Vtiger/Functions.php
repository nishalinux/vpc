...
class Vtiger_Functions {
	...
	static function updateCRMRecordLabel($module, $id, $label) {
		...
		if ($labelInfo) {
			...
			$log->debug('functions updateCRMRecordLabelllllllllllllllllllllllllllllllllllllllllllllllll');
	...
	static function initStorageFileDirectory() {
		...
		if (!is_dir($filepath . $year . "/" . $month . "/" . $week)) {
			...
			$old_umask = umask(0);
			mkdir($filepath . "$year/$month/$week", 0777);
			umask($old_umask);
		}
...
return $saveimage;
    ...
    */
	 ...
	 */
	 //Portal 6.5  Started Here :: Jan 25 2018
