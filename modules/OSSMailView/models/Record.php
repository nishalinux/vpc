...
class OSSMailView_Record_Model extends Vtiger_Record_Model
{
	...
	{
		global $log;
		//$log->debug('checkMailExist');
		$db = PearDatabase::getInstance();
		//echo 'SELECT ossmailviewid FROM vtiger_ossmailview WHERE id ='. $uid.' AND mbox = '.$folder.' AND rc_user = '.$rcId;
		$query = 'SELECT ossmailviewid FROM vtiger_ossmailview WHERE id = ? AND mbox = ? AND rc_user = ?';
		$result = $db->pquery($query, [$uid, $folder, $rcId]);
		//$log->debug('checkMailExist='.$query);
		//return $db->getRowCount($result) > 0 ? $db->getSingleValue($result) : false;
		return $db->getRowCount($result) > 0 ? $db->query_result($result, 0, 'ossmailviewid') : false;
	}
	...
	public function getReletedRecords($record)
	{
		...
		$result = $db->pquery($query,array($record, 0));
        $countt = $db->num_rows($result);
		if($countt>0){
	while ($row = $db->fetch_array($result)) {
			...
			$relations[$module][] = [
				'id' => $row['crmid'],
				'module' => $module,
				'label' => Vtiger_Util_Helper::getLabel($row['crmid'])
			];
		}
		}
		return $relations;
	...
	public static function addRelated($params)
	{
		...
		if ($newModule == 'Products') {
			$params =array($crmid, $newCrmId,$params['mod'],$currentUser->getId(),date('Y-m-d H:i:s'));
     		$sql2 = "insert into vtiger_seproductsrel(crmid, productid, setype,rel_created_user,rel_created_time) values(?, ?, ?,?,?)";
			$result = $db->pquery($sql2, $params);

		} elseif ($newModule == 'Services') {
			$params =array($crmid, $params['mod'],$newCrmId,$newModule);
     		$sql2 = "insert into vtiger_crmentityrel(crmid, module, relcrmid,relmodule) values(?, ?, ?,?)";
			$result = $db->pquery($sql2, $params);
	...
	public static function removeRelated($params)
	{
		...
		$db->pquery('DELETE FROM vtiger_ossmailview_relation WHERE ossmailviewid = ? AND crmid = ?', [$params['mailId'],$params['crmid']]);
