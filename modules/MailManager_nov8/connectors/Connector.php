	...
	public function __construct($url, $username, $password, $baseUrl=false) {
		global $log;
			$start = microtime(true);
								$log->debug('__construct__construct__construct rrrrrrrrrrrrrrrr\n\n<br><br>'.PHP_EOL);
...
//Added by abilash for attachment list in MailManager:Start

	public function attachmentName($port,$messageid,$fetchbody=true){
	vimport ('~modules/Settings/MailConverter/handlers/MailRecord.php');
		return new Vtiger_MailRecord($port,$messageid,$fetchbody=true);
}
//Added by abilash for attachment list in MailManager:End
		...
		if ($folderCheck->Nmsgs) {
			...
			$sequence = sprintf("%s:%s", $reverse_start, $reverse_end);
				$tstart = microtime(true);
								$log->debug('Counterrrrrrrrrrrrrrrrrrrrrrrrrrrrrroutt rrrrrrrrrrrrrrrr\n\n<br><br>'.PHP_EOL);

				$log->debug(microtime(true)-$tstart);
				$log->debug('Counterrrrrrrrrrrrrrrrrrrrrrrrrrrrrroutttt rrrrrrrrrrrrrrrr\n\n<br><br>'.PHP_EOL);
			...
			foreach($records as $result) {
				$log->debug('Counterrrrrrrrrrrrrrrrrrrrrrrrrrrrrrif rrrrrrrrrrrrrrrr\n\n<br><br>'.PHP_EOL);
				$log->debug($cntr++);
			$log->debug('start time before folderMails to connector\n\n\n\n<br><br>'.PHP_EOL);	$log->debug(microtime(true)-$tstart);
			$flagseen = $result->seen;//Added by SL for attachments in listview of MailManager:start
			if($flagseen == 1){//Added by SL for attachments in listview of MailManager
$mail_header=imap_fetchheader($this->mBox,$result->msgno);
 $startsAt = strpos($mail_header, "Content-Type: multipart/") + strlen("Content-Type: multipart/");
 $endsAt = strpos($mail_header, ";", $startsAt);
 $resulted = substr($mail_header, $startsAt, $endsAt - $startsAt);
 				$log->debug('Counterrrrrrrrrrrrrrrrrrrrrrrrrrrrrrif rrrrrrrrrrrrrrrr\n\n<br><br>'.PHP_EOL);

if($resulted =='mixed'){
$result->{"mattch"} = 'Exists';//Added by Abilash for attachment on 30 Nov 2016
$result->{"mboxx"} = imap_uid($this->mBox, $result->msgno);
		...
		//}
		$this->markMailRead($result->msgno);//Added by SL for attachments in listview of MailManager*/
...
$result->{"mboxx"} = imap_uid($this->mBox, $result->msgno);
			...
			//$result->{"mattch"} = $attmnt->_attachments;//Added by Abilash for attachment on 30 Nov 2016
	//$result->{"mboxx"} = imap_uid($this->mBox, $result->msgno);
			
			}
			$this->markMailUnread($result->msgno);//Added by Abilash for attachment on 30 Nov 2016
				
			}//Added by SL for attachments in listview of MailManager:end
...
public function openMailAttach($msgno) {
		$this->clearDBCache();
		return new MailManager_Message_Model($this->mBox, $msgno, true,true);
	}
...
$result->{"mboxx"} = imap_uid($this->mBox, $result->msgno);
		...
		$this->markMailRead($result->msgno);//Added by Abilash for attachment on 30 Nov 2016
