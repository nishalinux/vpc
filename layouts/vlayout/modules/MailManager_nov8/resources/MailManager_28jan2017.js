...
console.log(loction);
		...
		mail_reply: function(all){
			...
			if(all == true){
					//alert(md+"abhiiiiiiii");
					if(md == '' && cd == ''){
					var cc ="";	
					//alert ("murali"+cc+"murali");
					} else if(md == '' && cd != ''){
						var cc = cd;
					} else if(md != '' && cd == ''){
						var cc = md;
			...
			//var cc =cm.str_replace((madhu, ',', '');
			
		   //var cc = jQuery('#current_user_id').val();
			 //alert(madhu);
...
//alert(cc);
  ...
  /*var ajaxResult = [];
     $.ajax({
        type: 'post',
        url: 'modules/MailManager/actions/Server.php',
        data: {userid: abi},
	...
	*/		
			...
			//var cg = cp.shift();
			//var cc = cg;  
            //var divArray = $('cg').toArray();
            //var cc = $(divArray.shift());
            //'record' : jQuery('#current_user_id').val(),
           

             //var cc = cg.remove('#_mailopen_from');
		...
		mail_forward: function(messageId){
			...
			var from = jQuery('#_mailopen_from').html();Vtiger_Index_Js.sh
			var to = jQuery('#_mailopen_to').html();
			var cc = jQuery('#_mailopen_cc') ? jQuery('#_mailopen_cc').html() : '';
			var subject = jQuery('#_mailopen_subject').html();
			var body = jQuery('#_mailopen_body').html();
			...
			var fwdBody = MailManager.sprintf('<p></p><p style="margin:0;padding:0;">%s, %s, %s:</p><br/><p>%s<br/>%s</p>%s', 'On ' + date, from, 'wrote', MailManager.i18n('JSLBL_FORWARD_MESSAGE_TEXT'), fwdMsgMetaInfo, body);
