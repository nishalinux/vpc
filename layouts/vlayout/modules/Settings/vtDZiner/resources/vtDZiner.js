 ...
 * All Rights Reserved.
 *************************************************************************************/
jQuery.Class ('Settings_vtDZiner_Js', {
	...
	updatedParentRelatedList : {'updated' : [], 'deleted' : []},//Manasa

	removeModulesArray : false,
	
	removeParentModulesArray : false,//Manasa
	 ...
	 * Function to set the removed modules array used in related list
	 */
	setParentRemovedModulesList : function() {
		var thisInstance = this;
		var relatedList = jQuery('#relatedTabOrder');
		var container = relatedList.find('.relatedParentTabModulesList');
		
		thisInstance.removeParentModulesArray = JSON.parse(container.find('.ParentRemovedModulesListArray').val());
	 ...
	 * Function to regiser the event to make the related modules sortable
	 */
	makeParentRelatedModuleSortable : function() {
		var thisInstance = this;
		var relatedModulesContainer = jQuery('#relatedTabOrder');
		var modulesList = relatedModulesContainer.find('li.relatedParentModule');
		relatedModulesContainer.sortable({
			'containment' : relatedModulesContainer,
			'items' : modulesList,
			'revert' : true,
			...
			'cursor' : 'move',
			'update' : function(e, ui) {
				thisInstance.showParentSaveButton();
			}
		});
	},
	 ...
	 * Function which will disable the save button in related tabs list
	 */
	disableParentSaveButton : function() {
		var relatedList = jQuery('#relatedTabOrder');
		var saveButton = relatedList.find('.saveParentRelatedList');
		saveButton.attr('disabled', 'disabled');
	},
	/**
	 * Function which will enable the save button in realted tabs list
	 */
	showParentSaveButton : function() {
		var relatedList = jQuery('#relatedTabOrder');
		var saveButton = relatedList.find('.saveParentRelatedList');
		if(saveButton.attr('disabled') ==  'disabled') {
			saveButton.removeAttr('disabled');
...
/**
	...
	registerRelatedListEvents : function() {
		var thisInstance = this;
		var relatedList = jQuery('#relatedTabOrder');
		var container = relatedList.find('.relatedTabchildModules');
	...
	/**
	 * Function to register all the Parent relatedList Events
	 */
	registerParentRelatedListEvents : function() {
		var thisInstance = this;
		var relatedList = jQuery('#relatedTabOrder');
		var container = relatedList.find('.relatedParentTabModulesList');
		var pl = container.find('.ParentModulesListArray').val();
		var allModulesListParentArray =  '';
		if(pl != ''){
			var allModulesListParentArray =  JSON.parse(container.find('.ParentModulesListArray').val());
		}
		//JSON.parse(container.find('.ParentModulesListArray').val());
		var ulEle = container.find('ul.relatedParentModulesList');
		var selectEle = container.find('[name="addToParentList"]');
		app.showSelect2ElementView(selectEle, {maximumSelectionSize : 1, closeOnSelect : true, dropdownCss : {'z-index' : 0}});
		selectEle.on('change', function() {
			var selectedVal = selectEle.val();

			var moduleLabel = allModulesListParentArray[selectedVal];
		
			//remove the element if its already exists
			ulEle.find('.module_'+selectedVal[0]).remove();

			//append li element for the selected module
			var liEle = container.find('.moduleParentCopy').clone(true, true);
			liEle.data('relationId', selectedVal[0]).find('.moduleParentLabel').text(moduleLabel);
			ulEle.append(liEle.removeClass('hide moduleParentCopy').addClass('relatedParentModule module_'+selectedVal[0]));
			
			//thisInstance.makeParentRelatedModuleSortable();

			//remove that selected module from the select element
			selectEle.select2('data',[]);
			selectEle.find('option[value="'+selectedVal[0]+'"]').remove();

			thisInstance.removeParentModulesArray.splice(thisInstance.removeParentModulesArray.indexOf(selectedVal[0]),1);
			thisInstance.showParentSaveButton();
		})
		...
		container.find('.close').one('click', function(e) {
			var currentTarget = jQuery(e.currentTarget);
			thisInstance.showParentSaveButton();
			var liEle = currentTarget.closest('li.relatedParentModule');
			var relationId = liEle.data('relationId');
			var moduleLabel = liEle.find('.moduleLabel').text();
			liEle.fadeOut('slow').addClass('deleted');
			selectEle.append('<option value="'+relationId+'">'+moduleLabel+'</option>');
		...
		//register click event for save related  list button
		relatedList.on('click', '.saveParentRelatedList', function(e) {
			var currentTarget = jQuery(e.currentTarget);
			if(currentTarget.attr('disabled') != 'disabled') {
				thisInstance.disableParentSaveButton();
				thisInstance.updatedParentRelatedList['deleted'] = [];
				for(var key in thisInstance.removeParentModulesArray) {
					thisInstance.updatedParentRelatedList['deleted'].push(thisInstance.removeParentModulesArray[key]);
				}
				thisInstance.saveParentRelatedListInfo();
			}
		})
	},
/**
	 ...
	 */
	saveParentRelatedListInfo : function() {
		var thisInstance = this;
		...
		params['related_info'] = thisInstance.getUpdatedParentModulesInfo();
		params['sourceModule'] = jQuery('#selectedModuleName').val();
		params['key'] = 'Parent';//Manasa
	 ...
	 * Function to get the updates happened with the related modules list
	 */
	getUpdatedParentModulesInfo : function() {
		var thisInstance = this;
		var relatedList = jQuery('#relatedTabOrder');
		var removedModulesList = relatedList.find('li.relatedParentModule').filter('.deleted');
		var updatedModulesList = relatedList.find('li.relatedParentModule').not('.deleted');
		thisInstance.updatedParentRelatedList['updated'] = [];

		//update deleted related modules list
		removedModulesList.each(function(index,domElement) {
			var relationId = jQuery(domElement).data('relationId');
			thisInstance.updatedParentRelatedList['deleted'].push(relationId);
		});
		//update the existing related modules list
		updatedModulesList.each(function(index,domElement){
			var relationId = jQuery(domElement).data('relationId');
			thisInstance.updatedParentRelatedList['updated'].push(relationId);
		});
		return thisInstance.updatedParentRelatedList;
	},
	...
	registerFieldSequenceSaveClick : function() {
		...
		var vtlayout = jQuery(".layoutTabs"); 
		
		vtlayout.on('click','.jtab',function(){
			var uitabname = jQuery(this).text();
			if(jQuery.trim(uitabname) == 'Layout DZiner'){
				jQuery('.addCustomModuleModal').addClass('hide');//Feb 2 2018 
				jQuery('.addCategoryModal').addClass('hide');//Feb 3 2018
			}
		});
	},
	...
	registerFieldTypeChangeEvent : function(form) {
		...
		form.find('[name="fieldType"]').on('change', function(e) {
			...
			// Added STP 11 December 2014 UI Type 10
            if(selectedOption.data('relatedtooption')) {
                var relatedtoOption = form.find('.relatedtoOption');
	...
	registerAddCustomBlockEvent : function() {
		...
		contents.find('.addCustomBlock').click(function(e) {
			...
			var callBackFunction = function(data) {
				...
				app.showSelect2ElementView(data.find('select'), select2params);
	...
	saveBlockDetails : function(form) {
		...
		switch(params['blockType']){
			...
			case 'Comments' :
				params['mode'] = 'saveComments';
				break;

			case 'Subpanels' :
				params['mode'] = 'saveSubPanels';
				break;

			case 'Related' :
				params['mode'] = 'saveRelated';
				break;
		}
	 ...
	 * Function to register the click event for related modules list tab
	 */
	relatedModulesParentTabClickEvent : function() {
		var thisInstance = this;
		var contents = jQuery('#layoutEditorContainer').find('.contents');
		var relatedContainer = contents.find('#relatedTabOrder');
		var relatedTab = contents.find('.relatedListTab');
		relatedTab.click(function() {
			if(relatedContainer.find('.relatedParentTabModulesList').length > 0) {
	...
	showRelatedTabModulesList : function(relatedContainer) {
		...
		AppConnector.request(params).then(
			function(data) {
				...
				if(jQuery(data).find('.relatedParentTabModulesList').length > 0){
					thisInstance.registerParentRelatedListEvents();//manasa parent 
					//thisInstance.makeParentRelatedModuleSortable();
					thisInstance.setParentRemovedModulesList();
				}
			},
			function(error) {
			}
	...
	getModuleLayoutEditor : function(selectedModule) {
		//var thisInstance = this;
		var thisInstance = new Settings_vtDZiner_Js();
		var aDeferred = jQuery.Deferred();
		var progressIndicatorElement = jQuery.progressIndicator({
			'position' : 'html',
			'blockInfo' : {
	...
	registerModulesChangeEvent : function() {
		...
		container.on('change', '[name="layoutEditorModules"]', function(e) {
			...
			window.open("index.php?module=vtDZiner&parent=Settings&view=Index&&sourceModule="+selectedModule,"_self");
	...
	registerEvents : function() {
		...
		//thisInstance.registerParentRelatedListEvents();//Manasa
		thisInstance.relatedModulesParentTabClickEvent();//Manasa
	}
});

jQuery(document).ready(function() {
	var instance = new Settings_vtDZiner_Js();
...
Vtiger_WholeNumberGreaterThanZero_Validator_Js("Vtiger_DecimalMaxLength_Validator_Js",{
	...
	validate: function(){
		...
		if(response != true){
			...
			var fieldValue = this.getFieldValue();
			//Manasa Modified on 24th APR 2018 :: Decimal Length Restriction
			if (fieldValue > 10 && !(fieldValue < 0)) {
				var errorInfo = app.vtranslate('JS_LENGTH_SHOULD_BE_LESS_THAN_EQUAL_TO')+' '+10;
				this.setError(errorInfo);
				return false;
			}
...
Vtiger_WholeNumberGreaterThanZero_Validator_Js("Vtiger_MaxLength_Validator_Js",{
	...
	validate: function(){
		...
		if(response != true){
			...
			var fieldValue = this.getFieldValue();
			//manasa restricted to 25 length for integers :: Apr 24 2018
			var selectedfield = jQuery("#createFieldForm").find('select[name="fieldType"] option:selected').val();
			if(selectedfield == 'Integer'){
				if (fieldValue > 25) {
				var errorInfo = app.vtranslate('JS_LENGTH_SHOULD_BE_LESS_THAN_EQUAL_TO')+' 25';
				this.setError(errorInfo);
				return false;
				}
			}
			//Ended here
			else if (fieldValue > 255) {
...
jQuery.Class('vtDZiner_functions',{},{

	registerHideModal : function() {
		jQuery('.jtab').on('click',function(){
			   jQuery("div.modal", ".ModuleSettingsmodals").each(function(){
	...
	registerCustomTabUIEvent : function (){
		var thisInstance = this;
		var vtContents = jQuery('#layoutEditorContainer').find('.vtContents');
		var contents = jQuery('#layoutEditorContainer').find('.contents');
		vtContents.find('.addCustomTabtabUI').click(function(event) {
			...
			event.preventDefault();		
				...
				var current_module=app.getModuleName();
				var target = jQuery(event.currentTarget);
				var params = {};
				params['module'] = current_module;
				params['parent'] = app.getParentModuleName();
				params['view'] = 'CustomTabUI';
				params['mode'] = 'UILayout';
				AppConnector.request(params).then(
				function(data) {
				var callBackFunction = function(data) {
				}
				var params1 = {};
				params1.data = data ;
				params1.css = {'width':'90%','text-align':'left','left':'1.5%'};
				progressIndicatorElement.progressIndicator({'mode' : 'hide'});
				
					app.showModalWindow(data,function(data){
						if(typeof callBackFunction == 'function'){
							var GridContainer=jQuery('#frm_invoice').find('.contentsBackground');
							
							
							if(GridContainer.height() > 380)
							app.showScrollBar(GridContainer, {'height':'500px'});
							callBackFunction(data);
						}
					}, {'width':'70%','height':'200px','text-align':'left','left':'1.5%'});
					$( "#tabs" ).tabs();
		},
				function(error) {
				progressIndicatorElement.progressIndicator({'mode' : 'hide'});
	...
	registervtDZinerActivation : function() {		// Function to register vtDZiner activation 
			var thisInstance = this;
			//var contents = jQuery('#vtDZinerSettings').find('.contents');
			jQuery('#vtDZActivateButton').click(function(e) {
				 var addModuleContainer = jQuery('.vtDZinerActivationModal').clone(true, true);
				 var callBackFunction = function(data) {
					 data.find('.vtDZinerActivationModal').removeClass('hide');
					  var form = data.find('.vtDZinerActivationForm');
					  var params = app.validationEngineOptions;
					   params.onValidationComplete = function(form, valid){
							if(valid) {
										var saveButton = form.find(':submit');
										saveButton.attr('disabled', 'disabled');
										thisInstance.vtDZinerActivation(form).then(
										function(data) {
										var params = {};
										//var result = data['result'];
										...
										//	window.location = "index.php?parent=Settings&module=vtDZiner&sourceModule=Assignment";
									}
									return false;
						   }
					   form.validationEngine(params);  
				   ...
				   app.showModalWindow(addModuleContainer,function(data) {
							if(typeof callBackFunction == 'function') {
									callBackFunction(data);
							}
					}, {'width':'1000px'});
	...
	vtDZinerActivation : function(form) {
		var thisInstance = this;
		var modalHeader = form.closest('#globalmodal').find('.modal-header h3');
		var aDeferred = jQuery.Deferred();
		modalHeader.progressIndicator({smallLoadingImage : true, imageContainerCss : {display : 'inline', 'margin-left' : '18%',position : 'absolute'}});
	...
	registervtDZinerDeActivation : function() {		// Function to register vtDZiner deactivation 
		var thisInstance = this;
		var contents = jQuery('#layoutEditorContainer').find('.contents');
		jQuery('.vtDZinerDeActivation').click(function(e) {
		//bootbox.alert("Feature disabled in demo site");
		var addModuleContainer = contents.find('.vtDZinerDeActivationModal').clone(true, true);
		var callBackFunction = function(data) {
			data.find('.vtDZinerDeActivationModal').removeClass('hide');
			var form = data.find('.vtDZinerDeActivationForm');
			var params = app.validationEngineOptions;
			params.onValidationComplete = function(form, valid){
			if(valid) {
				var saveButton = form.find(':submit');
				saveButton.attr('disabled', 'disabled');
				thisInstance.vtDZinerDeActivation(form).then(
					function(data) {
						data = eval('('+data+')');
						var daresult = data["success"];
						if(data["success"]) {
							window.location.reload();
						}
						else {
							bootbox.alert("Something went wrong, handle this");
							console.log(data);
						}
					}
				);
	...
	vtDZinerDeActivation : function(form) {
		var thisInstance = this;
		var modalHeader = form.closest('#globalmodal').find('.modal-header h3');
		var aDeferred = jQuery.Deferred();
		modalHeader.progressIndicator({smallLoadingImage : true, imageContainerCss : {display : 'inline', 'margin-left' : '18%',position : 'absolute'}});
	...
	registerGoogleMapAPIKey: function(){
		var thisInstance = this;
		var aDeferred = jQuery.Deferred();
		var params = {};
		jQuery(".googleapikey").on('click',function(){
			params['module'] = 'vtDZiner';
			params['parent'] = 'Settings';
			params['action'] = 'Field';
			params['mode'] = 'googleMapKey';
			params['mapkey'] = jQuery("#mapkey").val();
			AppConnector.request(params).then(
				function(data) {
					var message ={};
					message['text'] = data['result']['message'];
					Settings_Vtiger_Index_Js.showMessage(message);
					aDeferred.resolve(data);
					window.location.reload();
					jQuery("#vtDZinerSettings").find('.googleapikeydisplay').removeClass('hide');
					jQuery("#vtDZinerSettings").find('.edit').addClass('hide');
				},
				function(error) {
		...
		jQuery(".editgooglemapkey").on('click',function(e){
			jQuery(this).closest('td').find('.googleapikeydisplay').addClass('hide');
			jQuery(this).closest('td').find('.edit').removeClass('hide');
		});
		return aDeferred.promise();
		
	},

	registerAddParentRelationEvent : function() {
		var contents = jQuery('.relatedTabModulesList').find('.contents');
		contents.find('.addParentRelation').click(function(e) {
			var selectedModuleName=jQuery('#selectedModuleName').val();
			var element1 = jQuery(e.currentTarget);
			var tabid='';
			var tabid = element1.data('id');
			var relmodule = element1.data('module');
			displayAddChildForm('parent',selectedModuleName,tabid,relmodule);
		});
	},

	registerAddChildRelationEvent : function() {
		var contents = jQuery('.relatedTabModulesList').find('.contents');
		contents.find('.addChildRelation').click(function(e) {
			var selectedModuleName=jQuery('#selectedModuleName').val();
			var element1 = jQuery(e.currentTarget);
			var tabid='';
			var tabid = element1.data('id');
			var relmodule = element1.data('module');
			displayAddChildForm('child',selectedModuleName,tabid,relmodule);			
		});
	},
 /*
	registerAddCustomModuleEvent : function() {
		var target= "addCustomModule";
		var vtContents = jQuery('#layoutEditorContainer').find('.moduleDZinerButtons');
		var contents = jQuery('#layoutEditorContainer').find('.contents');
		vtContents.find('.addCustomModule').click(function(e) {
			oc_addCustomModule(contents, target)
		});
	},
*/
	registerModuleSettingsOptions : function() {
		var moduleSettings = ['removeModule', 'exportModule', 'disableModule', 'enablePortal', 'enableTracker', 'registerCustomWorkflow', 'changeParentCategory', 'uploadModuleImageIcon', 'enablevtDZiner', 'addCategory', 'upgradevtDZiner', 'addCustomModule'];
		jQuery.each(moduleSettings,function(i,target){
			var contents = jQuery('#layoutEditorContainer').find('.contents');
			jQuery('.'+target).click(function(e) {
				moduleSettingsOnClick(contents, target);
			});			
		});
	},
	/**
	...
	registerEvents : function() {
		var thisInstance = this;
		thisInstance.registerModuleSettingsOptions();

		thisInstance.registerAddParentRelationEvent();
		thisInstance.registerAddChildRelationEvent();

		thisInstance.registervtDZinerActivation();
		thisInstance.registervtDZinerDeActivation();
		thisInstance.registerHideModal();
		
		thisInstance.registerGoogleMapAPIKey();
		//thisInstance.registerCustomTabUIEvent();
	}
});

jQuery.Class('vtDZiner_fileupload',{},{
	process_fileupload : function(){
		jQuery('.uploadModuleImageIconForm').find('.file_uploader').change(function(e){
		var src=$(this).val();
		if(src!="") {
			...
			if (!!file.type.match(/image.*/)) {
				...
				jQuery('.uploadModuleImageIconForm').find('#preview').html("");
				  reader.onloadend = function(e){
					var image = $('<img>').attr('src',e.target.result);
					$(image).appendTo(jQuery('.uploadModuleImageIconForm').find('#preview'));
					};
				}else{
					Vtiger_Helper_Js.showPnotify("Upload file size is too big.\nRecommended dimension is 24X24");
					e.preventDefault();
					return false;
				}
			}
	...
	jQuery('.uploadModuleImageIconForm').find('.file_uploader_sum').change(function(e){
		var src=$(this).val();
		if(src!="")
		{
			formdata= new FormData();
				...
				if (!!file.type.match(/image.*/))
                  {
					...
					var reader = new FileReader();
					reader.readAsDataURL(file);
					jQuery('.uploadModuleImageIconForm').find('#preview_sum').html("");
					  reader.onloadend = function(e){
						var image = $('<img>').attr('src',e.target.result);
						$(image).appendTo(jQuery('.uploadModuleImageIconForm').find('#preview_sum'));
						};
					} else{
						Vtiger_Helper_Js.showPnotify("Upload file size is too big.\nRecommended dimension is 24X24");
						e.preventDefault();
	...
	save_fileupload : function(){
		var thisInstance = this;
		jQuery('.uploadModuleImageIconForm').submit(function (e) {
			 var form = jQuery(e.currentTarget);
			  if (typeof form.data('submit') != "undefined") {
                return false;
            } else {
				thisInstance.saveIconToServerNow(form).then(
                            function(data) {
                                app.hideModalWindow();
                            },
                            function(error, err) {
	...
	saveIconToServerNow: function(form) {
        var thisInstance = this;
        var aDeferred = jQuery.Deferred();
		...
		if (formData) {
			...
			aDeferred.resolve(data);
			Vtiger_Helper_Js.showPnotify("Icon updated successfully");
			
			jQuery('#moduleimg').attr('src',jQuery('#moduleimg').attr('src')+'?'+Math.random());
			},
	...
	registerEvents : function() {
		var thisInstance = this;
		var formData;
		thisInstance.process_fileupload();
		thisInstance.save_fileupload();
	},
});

jQuery(document).ready(function() {
	// could move the rest of the Functions.tpl 
	//jQuery(document).ready(function(){
	var instance = new vtDZiner_functions();
	instance.registerEvents();
	var instance = new vtDZiner_fileupload();
	instance.registerEvents();
	jQuery("[name='linktype']").append('<option value=""></option>');//manasa
	jQuery.each(widgetTypesList, function(index, value){
		jQuery("[name='linktype']").append('<option value='+value[0]+' title="'+value[1]+'">'+value[0]+'</option>');
	});
	jQuery("#linktype").addClass("chzn-select");
	jQuery.each(widgetObject, function( index, value ) {
		//manasa
		//console.log(value);
		if('linkid' in value){
		jQuery(".widgetlist").append("<tr><td>"+value.linklabel+"</td><td>"+value.linktype+"</td><td>"+value.linkurl+"</td><td><span class='pull-right'><i title='Edit widget properties' class='icon-pencil alignMiddle' onclick='editWidget("+value.linkid+");'></i><i title='Delete widget' class='icon-trash alignMiddle' onclick='deleteWidget("+value.linkid+");'></i></span></td></tr>");
		}
	});
	jQuery(".pickblocklist tbody").empty();
	jQuery.each(vtBlockDetail, vtplwalker);
	setUpPickBlocks();
	setupLangDZiner();
	...
	jQuery('.massEditTabs a[href="'+hash+'"]').trigger('click');
});
//Manasa added on 27-01-2018
Vtiger_Base_Validator_Js("Vtiger_AlphaNumericName_Validator_Js",{
	invokeValidation: function(field, rules, i, options){
		var alphaNumericInstance = new Vtiger_AlphaNumericName_Validator_Js();
	...
	validate: function(){
		...
		if (!fieldValue.match(alphaNumericRegex)) {
			var errorInfo = app.vtranslate("JS_CONTAINS_ILLEGAL_CHARACTERS")+('_,+,-,#,@..etc');
			this.setError(errorInfo);
			return false;
		}
		var modulelist = jQuery("select[name='AvailableModules']");
		var newfieldValue = fieldValue.replace(/ /g, "");
		var modulestatus =jQuery(modulelist).find("option[value='"+newfieldValue+"']").text();
		if(modulestatus != ''){
			var errorInfo = "Duplicate Module "+fieldValue+"("+modulestatus+")"+" "+app.vtranslate("is present in CRM");
			this.setError(errorInfo);
			return false;
		}
        return true;
