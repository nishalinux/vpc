...
class MailManager_Relation_View extends MailManager_Abstract_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if ('find' == $this->getOperationArg($request)) {
			...
			$log->debug("7386524455");
			$log->debug(print_r($linkedto, true));
			...
			if (empty($linkedto)){
				...
				//$for_crmid = $adb->getUniqueId('vtiger_crmentity');
				$log->debug("8333983567");
				$log->debug(print_r($allowedModules, true));
				//$log->debug($for_crmid."o:----<");
				$log->debug("^V^V^V^V^V^V^");
				foreach (self::$MODULES as $MODULE) {
					if(!in_array($MODULE, $allowedModules)) continue;

					$from = $request->get('_mfrom');
					$log->debug("03551a1230");
					$log->debug(print_r($request, true));
					$log->debug($from);
					$log->debug("^H^H^H^H^H^");
					...
					//Murali--start to add into lead
					$log->debug("555555555");
					$q1 = $adb->pquery("select * from vtiger_leaddetails, vtiger_crmentity where (email like '{$from}' or secondaryemail like '{$from}') and vtiger_crmentity.crmid=vtiger_leaddetails.leadid and vtiger_crmentity.deleted=0", array());
					$nrows = $adb->num_rows($q1);
					$cquery = $adb->pquery("select * from vtiger_contactdetails, vtiger_crmentity where (email like '{$from}' or secondaryemail like '{$from}') and vtiger_crmentity.crmid=vtiger_contactdetails.contactid and vtiger_crmentity.deleted=0", array());
					$cnrows = $adb->num_rows($cquery);
					$log->debug("PPPP");
					$log->debug($cnrows);
					$aquery = $adb->pquery("select * from vtiger_account, vtiger_crmentity where (email1 like '{$from}' or email2 like '{$from}') and vtiger_crmentity.crmid=vtiger_account.accountid and vtiger_crmentity.deleted=0", array());
					$anrows = $adb->num_rows($aquery);
					$log->debug("VVVV");
					$log->debug($anrows);
					//if a mail comes from new email id, a new lead to be created
					if((!$nrows && !$cnrows && !$anrows) && (empty($results['Contacts']) && empty($results['Leads']) && empty($results['Accounts']))){
					require_once 'modules/Leads/Leads.php';
					$Lead = new Leads();
					$lname = explode("@",$from);
					$Lead->column_fields['lastname'] = $lname[0];
					$Lead->column_fields['email'] = $from;
					$Lead->column_fields['description'] = "This is created by custom code";
					//$Lead->column_fields['description'] = $request->get('_mbody');// mail body is not coming from this request.
					$Lead->save('Leads');
					...
					$lqury = "SELECT MAX(leadid) AS lid FROM vtiger_leaddetails where email='".$from."'";
					$leadid = $adb->pquery($lqury, array());
					$lid = $adb->query_result($leadid, 0, "lid");
					...
					$params2 = array($email_id,$from,html_entity_decode($mto),'[""]','[""]',' ',$lid.'@-1|','MailManager');
					$IE = $adb->pquery($insertEmail, $params2);
					
					$insertMailrel = "insert into vtiger_mailmanager_mailrel (mailuid,crmid,emailid) values (?,?,?)";
					$params3 = array(html_entity_decode($request->get('_msguid')),$lid,$email_id);
					...
					$params5 = array($lid,$email_id);
					$SEACT = $adb->pquery($insertSEAct, $params5);
					break;
					//if email is related to both of  contact and lead		
					} elseif(($nrows && $cnrows && !$anrows) && (!empty($results['Contacts']) && !empty($results['Leads']) && empty($results['Accounts']))){
					...
					$mto = $adb->query_result($dl, 0, "mt");
					$log->debug("&&&&&");
					$log->debug(html_entity_decode($sub)."/".html_entity_decode($bdy)."/".html_entity_decode($mto));
					...
					$ACT = $adb->pquery($insertAct, $params4);
					$log->debug("44444");
					$log->debug(print_r($params4, true));
					
					$insertSEAct = "insert into vtiger_seactivityrel (crmid, activityid) values (?,?)";
					$params5 = array($conid,$email_id);
					$SEACT = $adb->pquery($insertSEAct, $params5);
					$log->debug("22222");
					$log->debug(print_r($params5, true));
					break;
					//if mail is related to both accounts and leads	
					} elseif(($nrows && $anrows && !$cnrows) && (!empty($results['Accounts']) && !empty($results['Leads']) && empty($results['Contacts']))){
					$log->debug("101010101");	
					$lcrmid = $adb->pquery("SELECT MAX(crmid) AS cid FROM vtiger_crmentity", array());
					$lastcrmid = $adb->query_result($lcrmid, 0, "cid");
					$log->debug("909090");
					...
					$params1 = array($lastcrmid,$curr_usr_id,$curr_usr_id,$curr_usr_id,'Emails',html_entity_decode($bdy),$date_var,$date_var,null,null,0,1,0,html_entity_decode($sub));
					$IC = $adb->pquery($insertCRM, $params1);
					$aocid = $results['Accounts'][0]['id'];
					$log->debug($aocid."CCCC");
					...
					$insertEmail = "insert into vtiger_emaildetails (emailid, from_email, to_email, cc_email, bcc_email, assigned_user_email, idlists, email_flag) values (?,?,?,?,?,?,?,?)";
					$params2 = array($email_id,$from,html_entity_decode($mto),'[""]','[""]',' ',$aocid.'@-1|','MailManager');
					$IE = $adb->pquery($insertEmail, $params2);
					$log->debug("4334");
					$log->debug(print_r($params2, true));
					...
					$insertSEAct = "insert into vtiger_seactivityrel (crmid, activityid) values (?,?)";
					$params5 = array($aocid,$email_id);
					$SEACT = $adb->pquery($insertSEAct, $params5);
					$log->debug("877878");
					$log->debug(print_r($params5, true));
					break;
					//if mail is related to both accounts and contacts	
					} elseif(($anrows && $cnrows && !$nrows) && (!empty($results['Accounts']) && !empty($results['Contacts']) && empty($results['Leads']))){
					$log->debug("5%5%5%5%");	
					$lcrmid = $adb->pquery("SELECT MAX(crmid) AS cid FROM vtiger_crmentity", array());
					$lastcrmid = $adb->query_result($lcrmid, 0, "cid");
					$log->debug("%0%0%0");
					...
					} elseif(($nrows && !$anrows && !$cnrows) && (!empty($results['Leads']) && empty($results['Contacts']) && empty($results['Accounts']))){
					$log->debug("5%5%5%5%");	
					$lcrmid = $adb->pquery("SELECT MAX(crmid) AS cid FROM vtiger_crmentity", array());
					$lastcrmid = $adb->query_result($lcrmid, 0, "cid");
					$log->debug("%0%0%0");
					...
					$insertEmail = "insert into vtiger_emaildetails (emailid, from_email, to_email, cc_email, bcc_email, assigned_user_email, idlists, email_flag) values (?,?,?,?,?,?,?,?)";
					$params2 = array($email_id,$from,html_entity_decode($mto),'[""]','[""]',' ',$ldid.'@-1|','MailManager');
					$IE = $adb->pquery($insertEmail, $params2);
					$log->debug("$$33");
					$log->debug(print_r($params2, true));
					
					$insertMailrel = "insert into vtiger_mailmanager_mailrel (mailuid,crmid,emailid) values (?,?,?)";
					$params3 = array(html_entity_decode($request->get('_msguid')),$ldid,$email_id);
					$MREL = $adb->pquery($insertMailrel, $params3);
					$log->debug("##!!!!");
					$log->debug(print_r($params3, true));				
					...
					$insertSEAct = "insert into vtiger_seactivityrel (crmid, activityid) values (?,?)";
					$params5 = array($ldid,$email_id);
					$SEACT = $adb->pquery($insertSEAct, $params5);
					$log->debug("@@99@@");
					$log->debug(print_r($params5, true));
					break;
					//if mail is related only to contact	
					} elseif(($cnrows && !$nrows && !$anrows) && (!empty($results['Contacts']) && empty($results['Leads']) && empty($results['Accounts']))){
					$log->debug("5%5%5%5%");	
					$lcrmid = $adb->pquery("SELECT MAX(crmid) AS cid FROM vtiger_crmentity", array());
					$lastcrmid = $adb->query_result($lcrmid, 0, "cid");
					$log->debug("%0%0%0");
					...
					$insertEmail = "insert into vtiger_emaildetails (emailid, from_email, to_email, cc_email, bcc_email, assigned_user_email, idlists, email_flag) values (?,?,?,?,?,?,?,?)";
					$params2 = array($email_id,$from,html_entity_decode($mto),'[""]','[""]',' ',$ctid.'@-1|','MailManager');
					$IE = $adb->pquery($insertEmail, $params2);
					$log->debug("$$33");
					$log->debug(print_r($params2, true));
					
					$insertMailrel = "insert into vtiger_mailmanager_mailrel (mailuid,crmid,emailid) values (?,?,?)";
					$params3 = array(html_entity_decode($request->get('_msguid')),$ctid,$email_id);
					$MREL = $adb->pquery($insertMailrel, $params3);
					$log->debug("##!!!!");
					$log->debug(print_r($params3, true));				
					...
					$insertSEAct = "insert into vtiger_seactivityrel (crmid, activityid) values (?,?)";
					$params5 = array($ctid,$email_id);
					$SEACT = $adb->pquery($insertSEAct, $params5);
					$log->debug("@@99@@");
					$log->debug(print_r($params5, true));
					break;
					//if mail is related only to account	
					} elseif(($anrows && !$cnrows && !$nrows) && (!empty($results['Accounts']) && empty($results['Leads']) && empty($results['Contacts']))){
					$log->debug("5%5%5%5%");	
					$lcrmid = $adb->pquery("SELECT MAX(crmid) AS cid FROM vtiger_crmentity", array());
					$lastcrmid = $adb->query_result($lcrmid, 0, "cid");
					$log->debug("%0%0%0");
					...
					$insertEmail = "insert into vtiger_emaildetails (emailid, from_email, to_email, cc_email, bcc_email, assigned_user_email, idlists, email_flag) values (?,?,?,?,?,?,?,?)";
					$params2 = array($email_id,$from,html_entity_decode($mto),'[""]','[""]',' ',$actid.'@-1|','MailManager');
					$IE = $adb->pquery($insertEmail, $params2);
					$log->debug("$$33");
					$log->debug(print_r($params2, true));
					
					$insertMailrel = "insert into vtiger_mailmanager_mailrel (mailuid,crmid,emailid) values (?,?,?)";
					$params3 = array(html_entity_decode($request->get('_msguid')),$actid,$email_id);
					$MREL = $adb->pquery($insertMailrel, $params3);
					$log->debug("##!!!!");
					$log->debug(print_r($params3, true));				
					...
					$log->debug("-_-_-_-_-_-_-_");
					$log->debug(print_r($results['Leads'], true));
					$log->debug(print_r($results, true));
					$kg = implode(', ',array_keys($results));
					$log->debug(print_r($kg,true));
					$log->debug("_|_|_|_|_");
