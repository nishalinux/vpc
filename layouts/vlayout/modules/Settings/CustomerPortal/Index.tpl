...
<div class="container-fluid">
	...
	<div class="contents row-fluid">
		<form id="customerPortalForm" class="form-horizontal" method="GET">
			<div class="row-fluid">
				<input type="hidden" name="portalModulesInfo" value="" />
				<!-- added by jyothi -->
				<div class="control-group">
					<label class="muted control-label">{vtranslate('Portal Type', $QUALIFIED_MODULE)}</label>
					<div class="controls">
						<span class="row-fluid">
							<select id="portaltype" name="portaltype" class="select2 span3">
								<option>Select an Option</option>
								{foreach item=PORTAL_VALUE from=$PORTAL_VALUES}
									<option>{$PORTAL_VALUE}</option>
								{/foreach}
							<!-- 	<option>Type1</option>
								<option>Type2</option> -->
							</select>
							<span class="span1">&nbsp;</span>
							<span class="span7"><i class="icon-info-sign alignMiddle"></i>&nbsp;{vtranslate('Type of User login into the Customer Portal', $QUALIFIED_MODULE)}</span>
							<span class="span1">&nbsp;</span>
						</span>
					</div>
				</div>
					...
					<div class="controls">
						<span class="row-fluid">
							<select name="privileges" class="select2 span3">
								...
								<!-- {foreach item=PROFILE_MODEL from=$PROFILE_MODELS}
									<option>{$PROFILE_MODEL}</option>
								{/foreach} -->
				...
				<table id="portalModulesTable" class="table table-bordered table-condensed themeTableColor portal_table">
					<thead>
					...
					<tbody>
						<!-- {$MODULES_MODELS|@print_r} -->
						{if $MODULES_MODELS eq ""}
							<tr>
								<td><strong>Select portaltype to enable modules</strong></td>
							</tr>
						{else}
