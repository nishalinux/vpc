            ...
            if($eventName == 'vtiger.entity.aftersave.final') {  
                ...
                if($num > 0){
					...
					if ( $conf['register_changes'] == 1 )
                        ...
                        $db->pquery("UPDATE vtiger_modtracker_detail SET postvalue='**********' WHERE ", $where, [implode(',',$toUpdate)]);
					else
                        //$adb->update('vtiger_modtracker_detail', ['postvalue' => '**********', 'prevalue' => '**********'], $where, [implode(',',$toUpdate)]);
                        $db->pquery("UPDATE vtiger_modtracker_detail SET postvalue='**********', prevalue='**********' WHERE ", $where, [implode(',',$toUpdate)]);
				}
            }
        }
    }
