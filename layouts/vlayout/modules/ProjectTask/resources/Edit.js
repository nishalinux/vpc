...
Vtiger_Edit_Js("ProjectTask_Edit_Js",{},{ 
	...
	registerWidthChangeEvent : function() { 
		...
		/* New Changes */
		var user_task_allocatedhours = jQuery('.user_task_allocatedhours');
		var user_task_workedhours = jQuery('.user_task_workedhours');
		var user_start_date = jQuery('.user_start_date');
		var start_date = jQuery('.ProjectTask_editView_fieldName_startdate');
		...
		/*New chnages */
		Worked_Hours.attr('readonly', true); 
		user_task_allocatedhours.on('change',function(){		
			var subtotal = 0;
			jQuery('.user_task_allocatedhours').each(function() {
				var quantity = jQuery(this).val();
				console.log(quantity);	
				if(quantity != '')	{
					subtotal = parseInt(subtotal) + parseInt(quantity);
				}
			}); 
			Task_Budget_Hours.val(subtotal);
		});
		
		user_task_workedhours.on('change',function(){	 	
			var subtotal = 0;
			jQuery('.user_task_workedhours').each(function() {
				var quantity = jQuery(this).val();
				console.log(quantity);	
				if(quantity != '')	{
					subtotal = parseInt(subtotal) + parseInt(quantity);
		...
		/*Select Node
		jQuery('.jstree-grid-cell').on('click',function(){ 
			alert('clicked');
	...
	callingTree : function(){
		...
		jQuery('#task_tree_in_editer').jstree({ 
			'plugins' : [  'types',  'grid','sort','themes','contextmenu','search' ],
			 
			'core' : {
				  'data' : {
					"url" : "index.php?module=ProjectTask&action=AjaxTree&mode=getJsTreeForProjectTask&projectid="+projectid+"&selectId="+parentId,
					"dataType" : "json" 
				  }
				},		 	
			...
			'types' : {	
				...
				function(evt, data){
					...
					if(parentId != '' && record == task_id){
						alert('Canot select Self Task.');
						jQuery('#task_tree_in_editer').jstree("deselect_all");
						// jQuery('#'+parentId+'_anchor').attr('aria-selected', true);
						//jQuery('#ProjectTask_editView_fieldName_pcm_parent_task_id').val('');
						jQuery("#task_tree_in_editer").jstree("select_node", parentId); 
					}else{ 
						jQuery('#ProjectTask_editView_fieldName_pcm_parent_task_id').val(task_id);
