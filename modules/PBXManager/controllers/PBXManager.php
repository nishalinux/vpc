...
class PBXManager_PBXManager_Controller {
	...
	file_put_contents("/tmp/debug4.txt", "callerIdNumber = $callerNumber callerid2=$callerNumber2 ui=$userInfo\n", FILE_APPEND);
	...
	file_put_contents("/tmp/debug4.txt", "2   callerIdNumber = $callerNumber callerid2=$callerNumber2 ui=$userInfo\n", FILE_APPEND);
...
*/
        $temp = $request->get('dialString');
        $temp = explode("/", $temp);
	$to = $temp[1];
	file_put_contents("/tmp/debug5.txt", "callerIdNumber = $callerNumber to=$to\n", FILE_APPEND);
            $request->set('to', $to);
