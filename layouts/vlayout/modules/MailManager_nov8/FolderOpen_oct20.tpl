							...
							{if $FOLDER->isSentFolder()}
								{foreach item=MAIL from=$FOLDER->mails()}
									...
									<tr class="listViewEntries {if $MAIL->isRead()}mm_normal{else}fontBold{/if} mm_clickable"
										...
										<td width="27%" class="narrowWidthType" onclick="MailManager.mail_open('{$FOLDER->name()}', {$MAIL->msgNo()},'{'fdopen'}');">{vtranslate('LBL_TO', $MODULE)}: {$MAIL->to()}</td>
										<td class="narrowWidthType" onclick="MailManager.mail_open('{$FOLDER->name()}', {$MAIL->msgNo()},'{'fdopen'}');">{$MAIL->subject()}</td>
										{* Added by SL for attachments in listview of mail manager:start *}
										<td width="17%" class="narrowWidthType" align="right" > {foreach from=$MAIL->AttNo() key="key" item="value"}
										<a href="index.php?module={$MODULE}&view=Index&_operation=mail&_operationarg=attachment_dld&_muid={$MAIL->AttMuid()}&_atname={$key|@escape:'htmlall':'UTF-8'}">{$key}</a>                            &nbsp; <br>
										{/foreach}</td>{* Added by SL for attachments in listview of mail manager:end *}
										<td width="17%" class="narrowWidthType" align="right" onclick="MailManager.mail_open('{$FOLDER->name()}', {$MAIL->msgNo()},'{'fdopen'}');">{$MAIL->date(true)}</td>
									</tr>
								{/foreach}
							{else}
								{foreach item=MAIL from=$FOLDER->mails()}
									...
									<tr class="listViewEntries {if $MAIL->isRead()}mm_normal{else}fontBold{/if} mm_clickable"
										id="_mailrow_{$MAIL->msgNo()}" onmouseover='MailManager.highLightListMail(this);' onmouseout='MailManager.unHighLightListMail(this);'>
										<td width="3%" class="narrowWidthType"><input type='checkbox' value = "{$MAIL->msgNo()}" name = 'mc_box' onclick='MailManager.toggleSelectMail(this.checked, this);'></td>
										<td width="27%" class="narrowWidthType" onclick="MailManager.mail_open('{$FOLDER->name()}', {$MAIL->msgNo()},'{'fdopen'}');">{$MAIL->from(30)}</td>
										<td class="narrowWidthType" onclick="MailManager.mail_open('{$FOLDER->name()}', {$MAIL->msgNo()},'{'fdopen'}');">{$MAIL->subject()}</td>
										{* Added by SL for attachments in listview of mail manager:start *}
										<td width="17%" class="narrowWidthType" align="right"  > {foreach from=$MAIL->AttNo() key="key" item="value"}
										<a href="index.php?module={$MODULE}&view=Index&_operation=mail&_operationarg=attachment_dld&_muid={$MAIL->AttMuid()}&_atname={$key|@escape:'htmlall':'UTF-8'}">{$key}</a>  <br>
