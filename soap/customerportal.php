 ...
 *	@param array $input_array - array which contains the following parameters
 =>	int $id - customer id
	...
	*/
function get_combo_values($input_array,$portaltype)
{
	global $adb,$log;
	...
	{
		$check = checkModuleActive('Products',$portaltype);
		if($check == false){
 ...
 *	@param array $input_array - array which contains the following parameters
 =>	int $id - customer id
	...
	*/
function get_KBase_details($input_array,$portaltype='')
{
	global $adb,$log;
...
function update_acmpr_comment($input_array)
{
	global $adb,$mod_strings,$current_language; 
        $mod_strings = return_module_language($current_language, 'ACMPR');
	$adb->println("Inside customer portal function update_acmpr_comment");
	$adb->println($input_array);

	$id = $input_array['id'];
	$sessionid = $input_array['sessionid'];
	$acmprid = (int) $input_array['acmprid'];
	$ownerid = (int) $input_array['ownerid'];
	$comments = $input_array['comments'];
	...
	if(trim($comments) != '') {
		...
		$modComments->column_fields['assigned_user_id'] =  $current_user->id;
		$modComments->column_fields['customer'] = $ownerid;
		$modComments->column_fields['related_to'] = $acmprid;
		$modComments->column_fields['from_portal'] = true;
		$modComments->save('ModComments');
	}
}
...
function authenticate_user($username,$password,$version,$login = 'true')
{
	...
	$sql = "select id, user_name, user_password,last_login_time, support_start_date, support_end_date, cryptmode,portaltype
	...
	for ($i = 0; $i < $num_rows; ++$i) {
		...
		$log->debug("!!!!!!!!!!!!!!!!!");
$log->debug($adb->query_result($result, $i, 'user_password'));
$encpwd = Vtiger_Functions::compareEncryptedPassword($password, $adb->query_result($result, $i, 'user_password'), $adb->query_result($result, $i, 'cryptmode'));
$log->debug($encpwd);
...
$log->debug($customerid);
	...
	$list[0]['portaltype'] = $adb->query_result($result,$i,'portaltype');
...
{
	global $adb,$mod_strings,$log,$PORTAL_URL;
	...
	$conid = $adb->query_result($res,0,'id');
	...
	if (!empty($adb->query_result($res, 0, 'cryptmode'))) {
		...
		$adb->pquery("UPDATE vtiger_portalinfo SET user_password=? where id=?",array($enc_password,$conid));
		// TODO - we need to send link to reset the password
		// For now CRM user can do the same.
	}

	$fromquery = "select vtiger_users.user_name,vtiger_contactdetails.contactid,  vtiger_users.email1 from vtiger_users inner join vtiger_crmentity on vtiger_users.id = vtiger_crmentity.smownerid inner join vtiger_contactdetails on vtiger_contactdetails.contactid=vtiger_crmentity.crmid where vtiger_contactdetails.email =?";
	...
	/*Retrieving Email subject and body*/
	$templatename = 'Customer Login Details';
	$tempsql = $adb->pquery("select subject,body from vtiger_emailtemplates where templatename=?",array($templatename));
	$contents = decode_html($adb->query_result($tempsql ,0,'body'));
	$contents = str_replace('$contact_name$','Portal User',$contents);
	$contents = str_replace('$login_name$',$user_name,$contents);
	$contents = str_replace('$password$',$password,$contents);
	$contents = str_replace('$URL$',$PORTAL_URL,$contents);
	$mail = new PHPMailer();
...
function get_ticket_attachments($input_array)
{
	...
	$adb->println($input_array);
	$portaltype = $input_array['portaltype'];
	$check = checkModuleActive('Documents',$portaltype);
	if($check == false){
 ...
 *	@param array $input_array - array which contains the following values
 =>	int $id - customer ie., contact id
	...
	*	return array $output - This will return all the file details related to the ticket
	*/
 function get_acmpr_attachments($input_array)
 {
 	global $adb,$log;
 	$log->debug("Entering customer portal function get_acmpr_attachments");
 	$adb->println("INPUT ARRAY for the function get_acmpr_attachments");
 	$adb->println($input_array);
 	$portaltype = $input_array['portaltype'];
 	$check = checkModuleActive('Documents',$portaltype);
 	if($check == false){
 	...
 	$id = $input_array['id'];
 	$sessionid = $input_array['sessionid'];
 	$acmprid = $input_array['acmprid'];

 	$isPermitted = check_permission($id,'ACMPR',$acmprid);
 	if($isPermitted == false) {
 		return array("#NOT AUTHORIZED#");
 	}
 	...
 	$res = $adb->pquery($query, array($docid));
 	$noofrows = $adb->num_rows($res);
 	for($i=0;$i<$noofrows;$i++)
 	{
 		$filename = $adb->query_result($res,$i,'filename');
...
// 		//$filecontents = base64_encode(file_get_contents($filepath.$fileid."_".$filename));//fread(fopen($filepath.$filename, "r"), $filesize));
 	...
 	$log->debug("Exiting customer portal function get_acmpr_attachments");
 	return $output;
 }
...
function add_acmpr_attachment($input_array)
{
	global $adb,$log;
	global $root_directory, $upload_badext;
	$log->debug("Entering customer portal function add_acmpr_attachment");
	$adb->println("INPUT ARRAY for the function add_acmpr_attachment");
	$adb->println($input_array);
	$id = $input_array['id'];
	$sessionid = $input_array['sessionid'];
	$acmprid = $input_array['acmprid'];
	...
	$relatedquery = "insert into vtiger_seattachmentsrel values(?,?)";
	$relatedresult = $adb->pquery($relatedquery, array($acmprid, $attachmentid));
	...
	$tic_doc = 'insert into vtiger_senotesrel values(?,?)';
	$res = $adb->pquery($tic_doc,array($acmprid,$focus->id));
	$log->debug("Exiting customer portal function add_ticket_attachment");
}
...
function get_list_values($id,$module,$sessionid,$portaltype='',$only_mine='true')
{
	...
	$check = checkModuleActive($module,$portaltype);
	if($check == false){
		return array("#MODULE INACTIVE#");
	}
	...
	else if ($module == 'Documents')
	{
		...
		$params = array($entity_ids_list);
	}else if ($module == 'ACMPR'){
		$query = "select vtiger_acmpr.*,vtiger_acmprcf.*,vtiger_crmentity.smownerid from vtiger_acmpr  inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_acmpr.acmprid inner join vtiger_acmprcf on vtiger_acmprcf.acmprid=vtiger_acmpr.acmprid where vtiger_crmentity.deleted = 0 and cf_1706 IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
		...
		$query = "SELECT vtiger_project.*,vtiger_projectcf.*, vtiger_crmentity.smownerid
					FROM vtiger_project
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_project.projectid INNER JOIN vtiger_projectcf ON vtiger_project.projectid = vtiger_projectcf.projectid
					WHERE vtiger_crmentity.deleted = 0 AND (vtiger_projectcf.cf_968 IN (".generateQuestionMarks($entity_ids_list).") OR vtiger_projectcf.cf_969 IN (".generateQuestionMarks($entity_ids_list)."))";
		$params = array($entity_ids_list,$entity_ids_list);
		$fields_list['Related To'] = 'entityid';//'linktoaccountscontacts';
	}

	$res = $adb->pquery($query,$params);
	$noofdata = $adb->num_rows($res);
	$log->debug("ROWS FOLUND $noofdata");
	...
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($module == 'ACMPR'){
				if($fieldname == 'acmprname'){
					$fieldid = $adb->query_result($res,$j,'acmprid');
					$fieldvalue ='<a href="index.php?module=ACMPR&action=index&id='.$fieldid.'">'.$fieldvalue.'</a>';
				}
			}
			if($module == 'Project'){
 ...
 *	return $filecontents array with single file contents like [fileid] => filecontent
 */
function get_filecontent_detail($id,$folderid,$module,$customerid,$sessionid,$portaltype)
{
	global $adb,$log;
	global $site_URL;
	$log->debug("Entering customer portal function get_filecontent_detail ");
	$isPermitted = check_permission($customerid,$module,$id,$portaltype);
	if($isPermitted == false) {
		return array("#NOT AUTHORIZED#");
	}
...
function get_pdf($id,$block,$customerid,$sessionid,$portaltype)
{
	global $adb;
	...
	$isPermitted = check_permission($customerid,$block,$id,$portaltype);
	if($isPermitted == false) {
		return array("#NOT AUTHORIZED#");
	}
 ...
 *
 */
function get_product_list_values($id,$modulename,$sessionid,$portaltype,$only_mine='true')
{
	require_once('modules/Products/Products.php');
	require_once('include/utils/UserInfoUtil.php');
	global $current_user,$adb,$log;
	$log->debug("Entering customer portal function get_product_list_values ..");
	$check = checkModuleActive($modulename,$portaltype);
	if($check == false){
		return array("#MODULE INACTIVE#");
	}
	$user = new Users();
	...
	$checkQuotes = checkModuleActive('Quotes',$portaltype);
	if($checkQuotes == true){
	...
	$checkInvoices = checkModuleActive('Invoice',$portaltype);
 ...
 */
function get_details($id,$module,$customerid,$sessionid,$portaltype)
{
	...
	if($module == 'Quotes'){
		...
		$query = "select vtiger_acmpr.*,vtiger_acmprcf.*,vtiger_crmentity.smownerid from vtiger_acmpr inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_acmpr.acmprid inner join vtiger_acmprcf on vtiger_acmprcf.acmprid=vtiger_acmpr.acmprid
		 where vtiger_acmpr.acmprid=? AND vtiger_crmentity.deleted = 0";
	}
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		if($columnname == 'parent_id' || $columnname == 'contactid' || $columnname == 'accountid' || $columnname == 'potentialid'
			|| $fieldname == 'account_id' || $fieldname == 'contact_id' || $columnname == 'cf_968' || $columnname == 'cf_969')
		{
			$crmid = $fieldvalue;
			$modulename = getSalesEntityType($crmid);
			if ($crmid != '' && $modulename != '') {
 ...
 */
function check_permission($customerid, $module, $entityid,$portaltype='') {
	global $adb,$log;
	...
	switch($module) {
		...
		case 'Documents'	: 	$query = "SELECT vtiger_senotesrel.notesid FROM vtiger_senotesrel
								...
								if(checkModuleActive('Project',$portaltype)) {
									$query = "SELECT vtiger_senotesrel.notesid FROM vtiger_senotesrel
										INNER JOIN vtiger_project ON vtiger_project.projectid = vtiger_senotesrel.crmid INNER JOIN vtiger_projectcf ON vtiger_projectcf.projectid = vtiger_project.projectid
										WHERE (vtiger_projectcf.cf_968 IN (".generateQuestionMarks($allowed_contacts_and_accounts).") OR vtiger_projectcf.cf_969 IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
										AND vtiger_senotesrel.notesid = ?";
									$res = $adb->pquery($query, array($allowed_contacts_and_accounts,$allowed_contacts_and_accounts, $entityid));
									if ($adb->num_rows($res) > 0) {
										return true;
							...
							$query = "SELECT vtiger_troubletickets.ticketid FROM vtiger_troubletickets
									...
									WHERE vtiger_crmentity.deleted = 0 AND
											(vtiger_crmentityrel.crmid IN
												(SELECT projectid FROM vtiger_projectcf WHERE cf_968
											...
											OR vtiger_crmentityrel.relcrmid IN
												(SELECT projectid FROM vtiger_projectcf WHERE cf_969
		...
		case 'Project'	:	$query = "SELECT vtiger_project.projectid FROM vtiger_project inner join vtiger_projectcf on vtiger_projectcf.projectid = vtiger_project.projectid
							...
							break;
		case 'ACMPR'	:	$query = "select vtiger_acmpr.acmprid from vtiger_acmpr
		 inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_acmpr.acmprid inner join vtiger_acmprcf on vtiger_acmprcf.acmprid=vtiger_acmpr.acmprid
		 where vtiger_acmpr.applicant  IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND vtiger_acmpr.acmprid  = ?";
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
								return true;
			}
 ...
 */
function get_documents($id,$module,$customerid,$sessionid,$portaltype)
{
	global $adb,$log;
 ...
 * 			$customerid :: INT contact's Id'
 */
function get_project_components($id,$module,$customerid,$sessionid,$portaltype) {
	checkFileAccessForInclusion("modules/$module/$module.php");
	require_once("modules/$module/$module.php");
	require_once('include/utils/UserInfoUtil.php');
	...
	if ($module == 'ProjectTask') {
		$query ="SELECT vtiger_projecttask.*, vtiger_crmentity.smownerid
				...
				INNER JOIN vtiger_project ON vtiger_project.projectid = vtiger_projecttask.projectid INNER JOIN vtiger_projecttaskcf ON vtiger_projecttaskcf.projecttaskid = vtiger_projecttask.projecttaskid AND vtiger_project.projectid = ?
				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_projecttask.projecttaskid AND vtiger_crmentity.deleted = 0 and pcm_active=1";
	...
	elseif ($module == 'Changes') {
		$ptquery ="SELECT vtiger_projecttask.*, vtiger_crmentity.smownerid
				FROM vtiger_projecttask
				INNER JOIN vtiger_project ON vtiger_project.projectid = vtiger_projecttask.projectid INNER JOIN vtiger_projecttaskcf ON vtiger_projecttaskcf.projecttaskid = vtiger_projecttask.projecttaskid AND vtiger_project.projectid = ?
				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_projecttask.projecttaskid AND vtiger_crmentity.deleted = 0";
		$ptres = $adb->pquery($ptquery,array(vtlib_purify($id)));
		$ptrows = $adb->num_rows($ptres);
		$ptids= array();
		for($pt=0;$pt<$ptrows;$pt++){
			$ptids[] = $adb->query_result($ptres,$pt,'projecttaskid');
		}
		$query ="SELECT vtiger_change.*,vtiger_changecf.*, vtiger_crmentity.smownerid
				FROM vtiger_change 	INNER JOIN vtiger_changecf ON vtiger_changecf.changeid = vtiger_change.changeid INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_change.changeid AND vtiger_crmentity.deleted = 0 and (vtiger_changecf.c_old_task_ref IN (".generateQuestionMarks($ptids).") OR vtiger_changecf.c_new_task_ref IN (".generateQuestionMarks($ptids)."))";
	}
	if ($module == 'Changes') {
		$res = $adb->pquery($query,array($ptids,$ptids));
	}else{
	...
	for( $j= 0;$j < $noofdata; ++$j) {
		...
		foreach($fields_list as $fieldlabel => $fieldname) {
			...
			$output[0][$module]['head'][0][$i]['fielddata'] = $fieldlabel;
			/*if($fieldname == 'title'){
				$fieldvalue = '<a href="index.php?module=ProjectTask&action=index&fun=detail&projecttaskid='.$projecttaskid.'">'.$fieldvalue.'</a>';
			//}else{*/
				$fieldvalue = $adb->query_result($res,$j,$fieldname);
			//}
			if($fieldname == 'smownerid'){
 ...
 * 			$customerid :: INT contact's Id'
 */
function get_project_tickets($id,$module,$customerid,$sessionid,$portaltype='') {
	require_once('modules/HelpDesk/HelpDesk.php');
	require_once('include/utils/UserInfoUtil.php');
 ...
 *
 */
function get_service_list_values($id,$modulename,$sessionid,$portaltype='',$only_mine='true')
{
...
function get_modules($params='')
{
	...
	if($modules === false) {
		...
		$query = $adb->pquery("SELECT vtiger_customerportal_tabs.* FROM vtiger_customerportal_tabs
			...
			WHERE vtiger_tab.presence = 0 AND vtiger_customerportal_tabs.visible = 1 and type=?", array($portaltype));
...
function checkModuleActive($module,$params){
	global $adb,$log;

	$isactive = false;
	if($params != ''){
		$portaltype = array('portaltype'=>$params);
		$modules = get_modules($portaltype);
	}else{
