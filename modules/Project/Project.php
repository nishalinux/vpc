...
class Project extends CRMEntity {
	...
	//Added on nov15th 2017 Pickingslip Related List display purpose
	/**
	 ...
	 */
	function get_PickingSlip_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		//Overridding Default Query
		$query= "SELECT vtiger_crmentity.*, vtiger_pickingslip.*, CASE WHEN (vtiger_users.user_name NOT LIKE '') THEN CONCAT(vtiger_users.first_name,' ',vtiger_users.last_name) ELSE vtiger_groups.groupname END AS user_name FROM vtiger_pickingslip INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_pickingslip.pickingslipid left JOIN vtiger_crmentityrel ON (vtiger_crmentityrel.relcrmid = vtiger_crmentity.crmid OR vtiger_crmentityrel.crmid = vtiger_crmentity.crmid) LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid WHERE vtiger_crmentity.deleted = 0 AND vtiger_pickingslip.projectid=$id";
