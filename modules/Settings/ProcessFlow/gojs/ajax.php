...
if($_REQUEST){
    ...
    switch ($mode) {
        ...
        case 'save':
                ...
                foreach ($process_data['nodeDataArray'] as $key => $value)
                {
                    foreach ($value as $keyid => $data)
                    {
                        ...
                        if($data != "")
                        {
                            if(preg_match('/[\'\"]/', $data)) {
                                $i++;
                                $errormsg .= "Objects Content('. $data .') <br>"; 
                            }
                        }
                    }    
                }
                ...
                if($i > 0)
                {
                    echo json_encode(array('status'=>0,'message'=>$errormsg.'have Special characters, please check and try again.'));
                    exit;
                }
                ...
                $materials_data = $params['material'];
            ...
            { 
                ...
                #if product is yes 
                if(isset($task['Product']) && $task['Product'] == 'Yes'){
                    if(count($materials_data) >0 ){
                        foreach($materials_data as $products_array_data){
                            $max_qty = $products_array_data['qty'];
                            $materialid = $products_array_data['materialid'];
                            
                            $pdata = mysqli_query($link, "SELECT productname FROM vtiger_products where productid=$materialid"); 
                            $pdata = mysqli_fetch_assoc($pdata);
                            $pname = htmlspecialchars($pdata['productname']);
                            $productname = "Enter used Qty of ($pname)";
                            $field_name = 'product_'.$materialid . '_'.abs($task['key']);
                            $fielddata['html'][] = array(
                                                "type"=>"div",
                                                "html" => array(
                                                    'name'          => $field_name,
                ...
                #if IsFinalResult is yes 
                if(isset($task['IsFinalResult']) && $task['IsFinalResult'] == 'Yes'){
                    $field = 'Enter Final Quantity';
                    $name = strtolower(str_replace(" ","_",$field)); 
                    $fielddata['html'][] = array(
                                            "type"=>"div",
                                            "html" => array(
                ...
                #if category is document, adding upload document input 
