 ...
 * All Rights Reserved.
 ************************************************************************************/
class Changes extends CRMEntity {
	var $db, $log; // Used in class functions of CRMEntity

	var $table_name = 'vtiger_change';
	var $table_index= 'changeid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_changecf', 'changeid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity','vtiger_change','vtiger_changecf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_change'=>'changeid',
		'vtiger_changecf'=>'changeid',
	...
	var $list_fields = Array(
		...
		'Change No'=>Array('change'=>'change_no'),
		'Change Name'=>Array('change'=>'changename'),
        'Commission Rate'=>Array('change'=>'commissionrate'),
		'No of Units'=>Array('change'=>'qty_per_unit'),
		'Price'=>Array('change'=>'unit_price')
	);
	...
	var $list_link_field= 'changename';
	...
	var $popup_fields = Array ('changename','change_usageunit','unit_price');

	// Placeholder for sort fields - All the fields will be initialized for Sorting through initSortFields
	var $sortby_fields = Array();

	// For Alphabetical search
	var $def_basicsearch_col = 'changename';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'changename';

	// Required Information for enabling Import feature
	var $required_fields = Array('changename'=>1);

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('changename', 'assigned_user_id');

	var $default_order_by = 'changename';
	...
	{
		//Inserting into change_taxrel table
		if($_REQUEST['ajxaction'] != 'DETAILVIEW'&& $_REQUEST['action'] != 'MassEditSave' && $_REQUEST['action'] != 'ProcessDuplicates')
		{
			$this->insertTaxInformation('vtiger_producttaxrel', 'Changes');
			$this->insertPriceInformation('vtiger_productcurrencyrel', 'Changes');
		}
		// Update unit price value in vtiger_productcurrencyrel
		$this->updateUnitPrice();
	}

	/**	function to save the change tax information in vtiger_changetaxrel table
	 *	@param string $tablename - vtiger_tablename to save the change tax relationship (changetaxrel)
	...
	*/
	...
	/**	function to save the change price information in vtiger_changecurrencyrel table
	 *	@param string $tablename - vtiger_tablename to save the change currency relationship (changecurrencyrel)
	 *	@param string $module	 - current module name
	 *	$return void
	*/
	function insertPriceInformation($tablename, $module)
	{
		...
		$change_base_conv_rate = getBaseConversionRateForProduct($this->id, $this->mode,$module);
		...
		for($i=0;$i<count($currency_details);$i++)
		{
			...
			if($_REQUEST[$cur_checkname] == 'on' || $_REQUEST[$cur_checkname] == 1)
			{
				...
				$actual_conversion_rate = $change_base_conv_rate * $conversion_rate;
				...
				if ($_REQUEST['base_currency'] == $cur_valuename) {
					$adb->pquery("update vtiger_change set currency_id=?, unit_price=? where changeid=?", array($curid, $actualPrice, $this->id));
				}
			}else{
				$curid = fetchCurrency($current_user->id);
				$adb->pquery("update vtiger_change set currency_id=? where changeid=?", array($curid, $this->id));
			}
		}
	...
	function updateUnitPrice() {
		$prod_res = $this->db->pquery("select unit_price, currency_id from vtiger_change where changeid=?", array($this->id));
	...
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery('Changes', "detail_view");
		...
		$query .= $this->getNonAdminAccessControlQuery('Changes',$current_user);
	...
	/**	function used to get the list of quotes which are related to the change
	...
	/**	function used to get the list of purchase orders which are related to the change
	 *	@param int $id - change id
	 *	@return array - array which will be returned from the function GetRelatedList
	 */
	function get_purchase_orders($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_crmentity.*,
			...
			INNER JOIN vtiger_change
				ON vtiger_change.changeid = vtiger_inventoryproductrel.productid
			LEFT JOIN vtiger_groups
				ON vtiger_groups.groupid = vtiger_crmentity.smownerid
	...
	/**	function used to get the list of sales orders which are related to the change
	 *	@param int $id - change id
	 *	@return array - array which will be returned from the function GetRelatedList
	 */
	function get_salesorder($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_crmentity.*,
			vtiger_salesorder.*,
			vtiger_change.changename AS changename,
			vtiger_account.accountname,
			case when (vtiger_users.user_name not like '') then $userNameSql
				else vtiger_groups.groupname end as user_name
	...
	/**	function used to get the list of invoices which are related to the change
	 *	@param int $id - change id
	 *	@return array - array which will be returned from the function GetRelatedList
	 */
	...
	/**	function used to get the list of pricebooks which are related to the change
	 *	@param int $id - change id
	 *	@return array - array which will be returned from the function GetRelatedList
	 */
	function get_change_pricebooks($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		global $currentModule,$log,$singlepane_view,$mod_strings;
		$log->debug("Entering get_change_pricebooks(".$id.") method ...");
		...
		if($actions) {
			...
			if(in_array('SELECT', $actions) && isPermitted($related_module,1, '') == 'yes' && isPermitted($currentModule,'EditView',$id) == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_ADD_TO'). " ". getTranslatedString($related_module) ."' class='crmbutton small create'" .
					" onclick='this.form.action.value=\"AddChangeToPriceBooks\";this.form.module.value=\"$currentModule\"' type='submit' name='button'" .
		...
		$log->debug("Exiting get_change_pricebooks method ...");
		return $return_value;
	}


	/**	Function to display the Changes which are related to the PriceBook
	 ...
	 *	return array $return_data which will be formed like array('header'=>$header,'entries'=>$entries_list) where as $header contains all the header columns and $entries_list will contain all the Change entries
	 */
	function getPriceBookRelatedChanges($query,$focus,$returnset='')
	{
		global $log;
		$log->debug("Entering getPriceBookRelatedChanges(".$query.",".get_class($focus).",".$returnset.") method ...");

		global $adb;
		global $app_strings;
		global $current_language,$current_user;
		$current_module_strings = return_module_language($current_language, 'Changes');
		...
		$header=array();
		$header[]=$current_module_strings['LBL_LIST_CHANGE_NAME'];
		if(getFieldVisibilityPermission('Changes', $current_user->id, 'unit_price') == '0')
			$header[]=$current_module_strings['LBL_CHANGE_UNIT_PRICE'];
		...
		for($i=0; $i<$numRows; $i++) {
			...
			if($currency_id != null) {
				$prod_prices = getPricesForProducts($currency_id, array($entity_id),'Changes');
			...
			$entries = Array();
			$entries[] = textlength_check($adb->query_result($list_result,$i,"changename"));
			if(getFieldVisibilityPermission('Changes', $current_user->id, 'unit_price') == '0')
			...
			$action = "";
			if(isPermitted("PriceBooks","EditView","") == 'yes' && isPermitted('Changes', 'EditView', $entity_id) == 'yes') {
			...
			if(isPermitted("PriceBooks","Delete","") == 'yes' && isPermitted('Changes', 'Delete', $entity_id) == 'yes')
			{
				if($action != "")
					$action .= '&nbsp;|&nbsp;';
		...
		$log->debug("Exiting getPriceBookRelatedChanges method ...");
		return $return_data;
	}
	...
	function generateReportsQuery($module,$queryPlanner){
	   	global $current_user;
			...
			$matrix->setDependency('vtiger_seproductsrel',array('vtiger_crmentityRelChanges','vtiger_accountRelChanges','vtiger_leaddetailsRelChanges','vtiger_changecf','vtiger_potentialRelChanges'));
			$query = "from vtiger_change
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_change.changeid";
			if ($queryPlanner->requireTable("vtiger_changecf")){
			    $query .= " left join vtiger_changecf on vtiger_change.changeid = vtiger_changecf.changeid";
			}
			if ($queryPlanner->requireTable("vtiger_usersChanges")){
			    $query .= " left join vtiger_users as vtiger_usersChanges on vtiger_usersChanges.id = vtiger_crmentity.smownerid";
			}
			if ($queryPlanner->requireTable("vtiger_groupsChanges")){
			    $query .= " left join vtiger_groups as vtiger_groupsChanges on vtiger_groupsChanges.groupid = vtiger_crmentity.smownerid";
			}
			if ($queryPlanner->requireTable("vtiger_seproductsrel")){
			    $query .= " left join vtiger_seproductsrel on vtiger_seproductsrel.productid= vtiger_change.changeid";
			}
			if ($queryPlanner->requireTable("vtiger_crmentityRelChanges")){
			    $query .= " left join vtiger_crmentity as vtiger_crmentityRelChanges on vtiger_crmentityRelChanges.crmid = vtiger_seproductsrel.crmid and vtiger_crmentityRelChanges.deleted = 0";
			}
			if ($queryPlanner->requireTable("vtiger_accountRelChanges")){
			    $query .= " left join vtiger_account as vtiger_accountRelChanges on vtiger_accountRelChanges.accountid=vtiger_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("vtiger_leaddetailsRelChanges")){
			    $query .= " left join vtiger_leaddetails as vtiger_leaddetailsRelChanges on vtiger_leaddetailsRelChanges.leadid = vtiger_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("vtiger_potentialRelChanges")){
			    $query .= " left join vtiger_potential as vtiger_potentialRelChanges on vtiger_potentialRelChanges.potentialid = vtiger_seproductsrel.crmid";
			}
			if ($queryPlanner->requireTable("vtiger_lastModifiedByChanges")){
			    $query .= " left join vtiger_users as vtiger_lastModifiedByChanges on vtiger_lastModifiedByChanges.id = vtiger_crmentity.modifiedby";
			}
			if ($queryPlanner->requireTable("innerChange")){
			    $query .= " LEFT JOIN (
					SELECT vtiger_change.changeid,
							(CASE WHEN (vtiger_change.currency_id = 1 ) THEN vtiger_change.unit_price
								ELSE (vtiger_change.unit_price / vtiger_currency_info.conversion_rate) END
							) AS actual_unit_price
					FROM vtiger_change
					LEFT JOIN vtiger_currency_info ON vtiger_change.currency_id = vtiger_currency_info.id
					LEFT JOIN vtiger_productcurrencyrel ON vtiger_change.changeid = vtiger_productcurrencyrel.productid
					AND vtiger_productcurrencyrel.currencyid = ". $current_user->currency_id . "
				) AS innerChange ON innerChange.changeid = vtiger_change.changeid";
			}
			return $query;
	}
	...
	function generateReportsSecQuery($module,$secmodule, $queryPlanner) {
		global $current_user;
		$matrix = $queryPlanner->newDependencyMatrix();
		$matrix->setDependency('vtiger_change',array('actual_unit_price','vtiger_currency_info','vtiger_productcurrencyrel','vtiger_changecf','vtiger_crmentityChanges'));
		$matrix->setDependency('vtiger_crmentityChanges',array('vtiger_usersChanges','vtiger_groupsChanges','vtiger_lastModifiedByChanges'));
		if (!$queryPlanner->requireTable("vtiger_change",$matrix)){
			return '';
		}
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_change","changeid", $queryPlanner);
		if ($queryPlanner->requireTable("innerChange")){
		    $query .= " LEFT JOIN (
			SELECT vtiger_change.changeid,
			(CASE WHEN (vtiger_change.currency_id = " . $current_user->currency_id . " ) THEN vtiger_change.unit_price
			WHEN (vtiger_productcurrencyrel.actual_price IS NOT NULL) THEN vtiger_productcurrencyrel.actual_price
			ELSE (vtiger_change.unit_price / vtiger_currency_info.conversion_rate) * ". $current_user->conv_rate . " END
			) AS actual_unit_price FROM vtiger_change
            LEFT JOIN vtiger_currency_info ON vtiger_change.currency_id = vtiger_currency_info.id
            LEFT JOIN vtiger_productcurrencyrel ON vtiger_change.changeid = vtiger_productcurrencyrel.productid
			AND vtiger_productcurrencyrel.currencyid = ". $current_user->currency_id . ")
            AS innerChange ON innerChange.changeid = vtiger_change.changeid";
		}
		if ($queryPlanner->requireTable("vtiger_crmentityChanges",$matrix)){
		    $query .= " left join vtiger_crmentity as vtiger_crmentityChanges on vtiger_crmentityChanges.crmid=vtiger_change.changeid and vtiger_crmentityChanges.deleted=0";
		}
		if ($queryPlanner->requireTable("vtiger_changecf")){
		    $query .= " left join vtiger_changecf on vtiger_change.changeid = vtiger_changecf.changeid";
		}
		if ($queryPlanner->requireTable("vtiger_usersChanges")){
		    $query .= " left join vtiger_users as vtiger_usersChanges on vtiger_usersChanges.id = vtiger_crmentityChanges.smownerid";
		}
		if ($queryPlanner->requireTable("vtiger_groupsChanges")){
		    $query .= " left join vtiger_groups as vtiger_groupsChanges on vtiger_groupsChanges.groupid = vtiger_crmentityChanges.smownerid";
		}
		if ($queryPlanner->requireTable("vtiger_lastModifiedByChanges")){
		    $query .= " left join vtiger_users as vtiger_lastModifiedByChanges on vtiger_lastModifiedByChanges.id = vtiger_crmentityChanges.modifiedby ";
		}
        if ($queryPlanner->requireTable("vtiger_createdbyChanges")){
			$query .= " left join vtiger_users as vtiger_createdbyChanges on vtiger_createdbyChanges.id = vtiger_crmentityChanges.smcreatorid ";
		}
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Quotes" => array("vtiger_inventoryproductrel"=>array("productid","id"),"vtiger_change"=>"changeid"),
			"PurchaseOrder" => array("vtiger_inventoryproductrel"=>array("productid","id"),"vtiger_change"=>"changeid"),
			"SalesOrder" => array("vtiger_inventoryproductrel"=>array("productid","id"),"vtiger_change"=>"changeid"),
			"Invoice" => array("vtiger_inventoryproductrel"=>array("productid","id"),"vtiger_change"=>"changeid"),
			"PriceBooks" => array("vtiger_pricebookproductrel"=>array("productid","pricebookid"),"vtiger_change"=>"changeid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_change"=>"changeid"),
		);
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
 		...
 		if($eventType == 'module.postinstall') {
			...
			$ttModuleInstance->setRelatedList($moduleInstance,'Changes',array('select'));

			$leadModuleInstance = Vtiger_Module::getInstance('Leads');
			$leadModuleInstance->setRelatedList($moduleInstance,'Changes',array('select'));

			$accModuleInstance = Vtiger_Module::getInstance('Accounts');
			$accModuleInstance->setRelatedList($moduleInstance,'Changes',array('select'));

			$conModuleInstance = Vtiger_Module::getInstance('Contacts');
			$conModuleInstance->setRelatedList($moduleInstance,'Changes',array('select'));

			$potModuleInstance = Vtiger_Module::getInstance('Potentials');
			$potModuleInstance->setRelatedList($moduleInstance,'Changes',array('select'));

			$pbModuleInstance = Vtiger_Module::getInstance('PriceBooks');
			$pbModuleInstance->setRelatedList($moduleInstance,'Changes',array('select'),'get_pricebook_changes');

			// Initialize module sequence for the module
			$adb->pquery("INSERT into vtiger_modentity_num values(?,?,?,?,?,?)",array($adb->getUniqueId("vtiger_modentity_num"),$moduleName,'CNG',1,1,1));
		...
		// TODO Handle actions after this module is updated.
			...
			$ChangesModule  = Vtiger_Module::getInstance('Changes');
			Vtiger_Access::setDefaultSharing($ChangesModule);
		}
 	}
	...
	*/
	function get_changes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		global $log, $singlepane_view,$currentModule,$current_user;
		...
		$query = "SELECT vtiger_change.changeid, vtiger_change.changename,
			vtiger_change.change_no, vtiger_change.commissionrate,
			vtiger_change.change_usageunit, vtiger_change.unit_price,
			vtiger_crmentity.crmid, vtiger_crmentity.smownerid
			FROM vtiger_change
			INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_change.changeid
			INNER JOIN vtiger_changecf
				ON vtiger_change.changeid = vtiger_changecf.changeid
			LEFT JOIN vtiger_crmentityrel ON vtiger_crmentityrel.relcrmid = vtiger_change.changeid AND vtiger_crmentityrel.module='Changes'
			LEFT JOIN vtiger_users
				ON vtiger_users.id=vtiger_crmentity.smownerid
			LEFT JOIN vtiger_groups
				ON vtiger_groups.groupid = vtiger_crmentity.smownerid
  ...
  function save($module_name, $fileid='', $pushToConnec=true) {
    ...
    if(class_exists($mapper)) {
      $changeMapper = new $mapper();
      $changeMapper->processLocalUpdate($this, $pushToConnec, false);
    }
