...
try {
		...
		$query = "SELECT vp.productid FROM vtiger_productcf vp left join vtiger_crmentity vcm on vp.productid = vcm.crmid where vcm.deleted = 0 and vp.quickbook_id ='' ";
		...
		if($products_ids_data > 0){ 
			...
			while($resultrow = $adb->fetch_array($records)) { 
				...
				if(isset($Context)){
					...
					if($vtiger_data['description'] == ''){  $vtiger_data['description'] = 'Description'; }
					$Item->setDescription($vtiger_data['description']);
					$Item->setType('Inventory');
					if($vtiger_data['qtyinstock'] != '' || $vtiger_data['qtyinstock'] > 0) { $vtiger_data['qtyinstock'] = 1; }
					$Item->setQtyOnHand(round($vtiger_data['qtyinstock']));
					$Item->setTrackQtyOnHand(true);
					if($vtiger_data['sales_start_date'] == ''){  $vtiger_data['sales_start_date'] = '01/01/2017'; }
					...
					if($quickbook_id != ''){
						...
						if ($resp = $ItemService->add($Context, $realm, $Item))
						{
							...
							$sq = "UPDATE `vtiger_productcf` SET `quickbook_id`= $qb_id  WHERE  productid = $vtiger_id ";
							$adb->query($sq);
							$qb_create++;							
						}
						else
