...
//added by sl for barcode :start
	...
	//$filename,'modules/Products/uploads/'.$request->get('barcode').'_'.$recordModel->getId().'.png','png'
			...
			$query = "UPDATE  vtiger_invoicecf SET cf_959 = ?  WHERE 	invoiceid = ?";
			$db->pquery($query,array($fileName.'.png',$recordModel->getId()));
			// Link file attached to document
			$db->pquery("INSERT INTO vtiger_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)",
			...
			if($request->get('module')=='PurchaseOrder' && $request->get('jbase64')!=''){
				$currentUserModel = Users_Record_Model::getCurrentUserModel();
							$log->debug('savesignatureeeeeeeeeeeeeeeeeeeeeeee150eeeeeeeeeeeeeeeeeee');
				...
				$date_var = $db->formatDate(date('YmdHis'), true);
				$usetime = $db->formatDate($date_var, true);
							$log->debug('saveBarcodeeeeeeeeeeeeeeeeeeeeeeee151eeeeeeeeeeeeeeeeeee');
							$log->debug('INSERT INTO vtiger_crmentity(crmid, smcreatorid, smownerid,
				modifiedby, setype, description, createdtime, modifiedtime, presence, deleted)
			...
			$query = "UPDATE  vtiger_purchaseordercf SET cf_960 = ?  WHERE 	purchaseorderid = ?";
			$db->pquery($query,array($fileName.'.png',$recordModel->getId()));
			// Link file attached to document
			$db->pquery("INSERT INTO vtiger_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)",
	...
	//added by jayant for signature :start-------------------------------------------------------------------Project --------------------------------------------------------------------------------------------------------------
			if($request->get('module')=='Project' && $request->get('jbase64')!=''){
				$currentUserModel = Users_Record_Model::getCurrentUserModel();
							$log->debug('savesignatureeeeeeeeeeeeeeeeeeeeeeee155eeeeeeeeeeeeeeeeeee');
				...
				$date_var = $db->formatDate(date('YmdHis'), true);
				$usetime = $db->formatDate($date_var, true);
							$log->debug('saveBarcodeeeeeeeeeeeeeeeeeeeeeeee156eeeeeeeeeeeeeeeeeee');
							$log->debug('INSERT INTO vtiger_crmentity(crmid, smcreatorid, smownerid,
				modifiedby, setype, description, createdtime, modifiedtime, presence, deleted)
			...
			$query = "UPDATE  vtiger_projectcf SET cf_962 = ?  WHERE 	projectid = ?";
			$db->pquery($query,array($fileName.'.png',$recordModel->getId()));
			// Link file attached to document
			$db->pquery("INSERT INTO vtiger_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)",
		...
		//added by jayant for signature :start-------------------------------------------------------------------HelpDesk --------------------------------------------------------------------------------------------------------------
			if($request->get('module')=='HelpDesk' && $request->get('jbase64')!=''){
				$currentUserModel = Users_Record_Model::getCurrentUserModel();
							$log->debug('savesignatureeeeeeeeeeeeeeeeeeeeeeee175eeeeeeeeeeeeeeeeeee');
				...
				$date_var = $db->formatDate(date('YmdHis'), true);
				$usetime = $db->formatDate($date_var, true);
							$log->debug('saveBarcodeeeeeeeeeeeeeeeeeeeeeeee177eeeeeeeeeeeeeeeeeee');
							$log->debug('INSERT INTO vtiger_crmentity(crmid, smcreatorid, smownerid,
				modifiedby, setype, description, createdtime, modifiedtime, presence, deleted)
				VALUES ($attachid, $currentUserModel->getId(), $currentUserModel->getId(), $currentUserModel->getId(), "HelpDesk Attachment", $description, $usetime, $usetime, 1, 0)');

				$db->pquery("INSERT INTO vtiger_crmentity(crmid, smcreatorid, smownerid,
				modifiedby, setype, description, createdtime, modifiedtime, presence, deleted)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
						Array($attachid, $currentUserModel->getId(), $currentUserModel->getId(), $currentUserModel->getId(), "HelpDesk Attachment", $description, $usetime, $usetime, 1, 0));
			...
			$query = "UPDATE  vtiger_ticketcf SET cf_964 = ?  WHERE 	ticketid = ?";
			$db->pquery($query,array($fileName.'.png',$recordModel->getId()));
			// Link file attached to document
			$db->pquery("INSERT INTO vtiger_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)",
	...
	//added by jayant for signature :start-------------------------------------------------------------------Contacts --------------------------------------------------------------------------------------------------------------
			...
			$query = "UPDATE  vtiger_contactscf SET cf_966 = ?  WHERE 	contactid = ?";
			$db->pquery($query,array($fileName.'.png',$recordModel->getId()));
			// Link file attached to document
			$db->pquery("INSERT INTO vtiger_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)",
