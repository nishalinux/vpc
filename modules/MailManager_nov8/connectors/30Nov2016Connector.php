	...
	public function folderMails($folder, $start, $maxLimit) {
		...
		if ($folderCheck->Nmsgs) {
			...
			if($flagseen == 1){//Added by SL for attachments in listview of MailManager
				$mailats = $this->openMailAttach($result->msgno);//Added by SL for attachments in listview of MailManager
				$result->{"mattch"} = $mailats->attachments(false);//Added by SL for attachments in listview of MailManager
				$result->{"mboxx"} = imap_uid($this->mBox, $result->msgno);//Added by SL for attachments in listview of MailManager
				...
				$result->{"mattch"} = $mailats->attachments(false);//Added by SL for attachments in listview of MailManager
				$result->{"mboxx"} = imap_uid($this->mBox, $result->msgno);//Added by SL for attachments in listview of MailManager
				$this->markMailUnread($result->msgno);
...
public function openMailAttach($msgno) {
		$this->clearDBCache();
		return new MailManager_Message_Model($this->mBox, $msgno, true,true,true);
	}
	...
	public function searchMails($query, $folder, $start, $maxLimit) {
		...
		if (!empty($nos)) {
			...
			if($flagseen == 1){
				...
				$result->{"mattch"} = $mailats->attachments(false);//Added by SL for attachments in listview of MailManager
				$result->{"mboxx"} = imap_uid($this->mBox, $result->msgno);//Added by SL for attachments in listview of MailManager
				$this->markMailRead($result->msgno);
			}
			else{
				$mailats = $this->openMailAttach($result->msgno);//Added by SL for attachments in listview of MailManager
				$result->{"mattch"} = $mailats->attachments(false);//Added by SL for attachments in listview of MailManager
