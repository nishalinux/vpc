 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Germination.php script which will get converted as the written out as Germination.php
...
// vtiger_germination is a custom vtiger entity generated by vtDZiner Module DZiner
class Germination extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Germination";
	var $table_name = "vtiger_germination";
	var $table_index= 'germinationid';

	var $tab_name = Array('vtiger_crmentity','vtiger_germination','vtiger_germinationcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_germination'=>'germinationid','vtiger_germinationcf'=>'germinationid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_germinationcf', 'germinationid');
	...
	var $list_link_field= 'germinationname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'germinationname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'germinationname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_germinationgrouprelation','germinationid');
	function Germination() {
		$this->log = LoggerManager::getLogger('germination');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Germination');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['GERMINATION_SORT_ORDER'] != '')?($_SESSION['GERMINATION_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Germinationname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['GERMINATION_ORDER_BY'] != '')?($_SESSION['GERMINATION_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_germination.germinationid,  vtiger_germination.germinationname, vtiger_germination.dateclosed FROM vtiger_germination, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_germination.germinationid, vtiger_germination.germinationname, vtiger_crmentity.smcreatorid FROM vtiger_germination inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_germination.germinationid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Germination',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export Germination Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Germination", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_germination
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_germination.germinationid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_germinationcf on vtiger_germinationcf.germinationid=vtiger_germination.germinationid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_germination.germinationid, vtiger_germination.germinationname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_germination
					inner join vtiger_contgerminationrel on vtiger_contgerminationrel.germinationid = vtiger_germination.germinationid
					inner join vtiger_contactdetails on vtiger_contgerminationrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_germination.germinationid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_germination on vtiger_germination.germinationid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Germination'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_germination ON vtiger_germination.germinationid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_germination.germinationid = $id";
	...
	/**
	* Function to get Germination related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Germination',$query,$id);
	}


	  /**
	  * Function to get Germination related Quotes
	  * @param  integer   $id  - germinationid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'germination_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_germination.germinationname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_germination on vtiger_germination.germinationid=vtiger_quotes.germinationid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contgerminationrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contgerminationrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contgerminationrel"=>"germinationid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"germinationid","vtiger_salesorder"=>"germinationid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_germination","germinationid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityGermination on vtiger_crmentityGermination.crmid=vtiger_germination.germinationid and vtiger_crmentityGermination.deleted=0
		left join vtiger_germinationcf on vtiger_germinationcf.germinationid = vtiger_germination.germinationid
		left join vtiger_groups vtiger_groupsGermination on vtiger_groupsGermination.groupid = vtiger_crmentityGermination.smownerid
		left join vtiger_users as vtiger_usersGermination on vtiger_usersGermination.id = vtiger_crmentityGermination.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_germination"=>"germinationid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_germination"=>"germinationid"),
			"Quotes" => array("vtiger_quotes"=>array("germinationid","quoteid"),"vtiger_germination"=>"germinationid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("germinationid","salesorderid"),"vtiger_germination"=>"germinationid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_germination"=>"germinationid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Germination Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contgerminationrel WHERE germinationid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
