 ...
 ********************************************************************************/
 // This is the proforma vtDZiner PaymentProcessing.php script which will get converted as the written out as PaymentProcessing.php
...
// vtiger_paymentprocessing is a custom vtiger entity generated by vtDZiner Module DZiner
class PaymentProcessing extends CRMEntity {
	var $log;
	var $db;

	var $module_name="PaymentProcessing";
	var $table_name = "vtiger_paymentprocessing";
	var $table_index= 'paymentprocessingid';

	var $tab_name = Array('vtiger_crmentity','vtiger_paymentprocessing','vtiger_paymentprocessingcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_paymentprocessing'=>'paymentprocessingid','vtiger_paymentprocessingcf'=>'paymentprocessingid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_paymentprocessingcf', 'paymentprocessingid');
	...
	var $list_link_field= 'paymentprocessingname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'paymentprocessingname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'paymentprocessingname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_paymentprocessinggrouprelation','paymentprocessingid');
	function PaymentProcessing() {
		$this->log = LoggerManager::getLogger('paymentprocessing');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('PaymentProcessing');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['PAYMENTPROCESSING_SORT_ORDER'] != '')?($_SESSION['PAYMENTPROCESSING_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'PaymentProcessingname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['PAYMENTPROCESSING_ORDER_BY'] != '')?($_SESSION['PAYMENTPROCESSING_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		require('user_privileges/user_privileges_'.$current_user->id.'.php');
        	...
        	$tab_id = getTabid("PaymentProcessing");
		...
		if($account_required)
		{
			$query = "SELECT vtiger_paymentprocessing.paymentprocessingid,  vtiger_paymentprocessing.paymentprocessingname, vtiger_paymentprocessing.dateclosed FROM vtiger_paymentprocessing, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_paymentprocessing.paymentprocessingid, vtiger_paymentprocessing.paymentprocessingname, vtiger_crmentity.smcreatorid FROM vtiger_paymentprocessing inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_paymentprocessing.paymentprocessingid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('PaymentProcessing',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export PaymentProcessing Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("PaymentProcessing", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_paymentprocessing
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_paymentprocessing.paymentprocessingid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_paymentprocessingcf on vtiger_paymentprocessingcf.paymentprocessingid=vtiger_paymentprocessing.paymentprocessingid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_paymentprocessing.paymentprocessingid, vtiger_paymentprocessing.paymentprocessingname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_paymentprocessing
					inner join vtiger_contpaymentprocessingrel on vtiger_contpaymentprocessingrel.paymentprocessingid = vtiger_paymentprocessing.paymentprocessingid
					inner join vtiger_contactdetails on vtiger_contpaymentprocessingrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_paymentprocessing.paymentprocessingid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_paymentprocessing on vtiger_paymentprocessing.paymentprocessingid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'PaymentProcessing'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_paymentprocessing ON vtiger_paymentprocessing.paymentprocessingid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_paymentprocessing.paymentprocessingid = $id";
	...
	/**
	* Function to get PaymentProcessing related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('PaymentProcessing',$query,$id);
	}


	  /**
	  * Function to get PaymentProcessing related Quotes
	  * @param  integer   $id  - paymentprocessingid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'paymentprocessing_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_paymentprocessing.paymentprocessingname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_paymentprocessing on vtiger_paymentprocessing.paymentprocessingid=vtiger_quotes.paymentprocessingid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contpaymentprocessingrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contpaymentprocessingrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contpaymentprocessingrel"=>"paymentprocessingid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"paymentprocessingid","vtiger_salesorder"=>"paymentprocessingid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_paymentprocessing","paymentprocessingid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityPaymentProcessing on vtiger_crmentityPaymentProcessing.crmid=vtiger_paymentprocessing.paymentprocessingid and vtiger_crmentityPaymentProcessing.deleted=0
		left join vtiger_paymentprocessingcf on vtiger_paymentprocessingcf.paymentprocessingid = vtiger_paymentprocessing.paymentprocessingid
		left join vtiger_groups vtiger_groupsPaymentProcessing on vtiger_groupsPaymentProcessing.groupid = vtiger_crmentityPaymentProcessing.smownerid
		left join vtiger_users as vtiger_usersPaymentProcessing on vtiger_usersPaymentProcessing.id = vtiger_crmentityPaymentProcessing.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_paymentprocessing"=>"paymentprocessingid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_paymentprocessing"=>"paymentprocessingid"),
			"Quotes" => array("vtiger_quotes"=>array("paymentprocessingid","quoteid"),"vtiger_paymentprocessing"=>"paymentprocessingid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("paymentprocessingid","salesorderid"),"vtiger_paymentprocessing"=>"paymentprocessingid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_paymentprocessing"=>"paymentprocessingid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-PaymentProcessing Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contpaymentprocessingrel WHERE paymentprocessingid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
