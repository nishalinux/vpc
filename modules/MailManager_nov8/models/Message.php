...
class MailManager_Message_Model extends Vtiger_MailRecord  {
	 ...
	 */
	public function __construct($mBox=false, $msgno=false, $fetchbody=false,$mode=false) {
		if ($mBox && $msgno) {
	...
	public function __getpart($imap, $messageid, $p, $partno) {
		...
		elseif ($p->ifdisposition == "" && $p->bytes > 0 &&
                (strtolower($p->subtype) == 'plain' || strtolower($p->subtype) == 'html')) {
	...
	protected function saveToDB($uid) {
		...
		if (!empty($this->_attachments)) {
			foreach($this->_attachments as $aName => $aValue) {
				...
				$this->setAttNo($this->mMsgNo);
				$this->setAttMuid($uid);
				if(is_array($attachInfo) && !empty($attachInfo)) {
					$renameatt = preg_replace('/[^a-zA-Z0-9.]/s', '', $attachInfo['name']);//manasa 20 oct
					$db->pquery("INSERT INTO vtiger_mailmanager_mailattachments
					(userid, muid, attachid, aname, path, lastsavedtime,orgname) VALUES (?, ?, ?, ?, ?, ?,?)",
					array($currentUserModel->getId(), $uid, $attachInfo['attachid'], $renameatt, $attachInfo['path'], $savedtime, $aName));
		...
		foreach($this->_inline_attachments as $index => $info) {
			...
			if(is_array($attachInfo) && !empty($attachInfo)) {
				$renameatt = preg_replace('/[^a-zA-Z0-9.]/s', '', $attachInfo['name']);//manasa 20 oct
				$db->pquery("INSERT INTO vtiger_mailmanager_mailattachments
				(userid, muid, attachid, aname, path, lastsavedtime, cid,orgname) VALUES (?, ?, ?, ?, ?, ?, ?,?)",
				array($currentUserModel->getId(), $uid, $attachInfo['attachid'],$renameatt, $attachInfo['path'], $savedtime, $info['cid'], $aName));
	...
	public function __SaveAttachmentFile($filename, $filecontent) {
		...
		$filename = preg_replace('/[^a-zA-Z0-9.]/s', '', $filename);//manasa 20 oct
...
//Added by SL for attachments in listview of MailManager :start
	public function setAttNo($msgno) {
			$this->mAttach = $msgno;
		}
	public function AttNo() {
		return	$this->mAttach  ;
		}
		public function setAttMuid($msguid) {
			$this->mUid = $msguid;
		}
	public function AttMuid() {
		return	$this->mUid  ;
		}
	...
	public static function parseOverview($result) {
		...
		$instance->setAttNo($result->mattch);//Added by SL for attachments in listview of MailManager
		$instance->setAttMuid($result->mboxx);//Added by SL for attachments in listview of MailManager
