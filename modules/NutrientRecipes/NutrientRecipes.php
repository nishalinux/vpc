 ...
 ********************************************************************************/
 // This is the proforma vtDZiner NutrientRecipes.php script which will get converted as the written out as NutrientRecipes.php
...
// vtiger_nutrientrecipes is a custom vtiger entity generated by vtDZiner Module DZiner
class NutrientRecipes extends CRMEntity {
	var $log;
	var $db;

	var $module_name="NutrientRecipes";
	var $table_name = "vtiger_nutrientrecipes";
	var $table_index= 'nutrientrecipesid';

	var $tab_name = Array('vtiger_crmentity','vtiger_nutrientrecipes','vtiger_nutrientrecipescf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_nutrientrecipes'=>'nutrientrecipesid','vtiger_nutrientrecipescf'=>'nutrientrecipesid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_nutrientrecipescf', 'nutrientrecipesid');
	...
	var $list_link_field= 'nutrientrecipesname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'nutrientrecipesname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'nutrientrecipesname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_nutrientrecipesgrouprelation','nutrientrecipesid');
	function NutrientRecipes() {
		$this->log = LoggerManager::getLogger('nutrientrecipes');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('NutrientRecipes');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['NUTRIENTRECIPES_SORT_ORDER'] != '')?($_SESSION['NUTRIENTRECIPES_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'NutrientRecipesname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['NUTRIENTRECIPES_ORDER_BY'] != '')?($_SESSION['NUTRIENTRECIPES_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		require('user_privileges/user_privileges_'.$current_user->id.'.php');
        	...
        	$tab_id = getTabid("NutrientRecipes");
		...
		if($account_required)
		{
			$query = "SELECT vtiger_nutrientrecipes.nutrientrecipesid,  vtiger_nutrientrecipes.nutrientrecipesname, vtiger_nutrientrecipes.dateclosed FROM vtiger_nutrientrecipes, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_nutrientrecipes.nutrientrecipesid, vtiger_nutrientrecipes.nutrientrecipesname, vtiger_crmentity.smcreatorid FROM vtiger_nutrientrecipes inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_nutrientrecipes.nutrientrecipesid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('NutrientRecipes',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export NutrientRecipes Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("NutrientRecipes", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_nutrientrecipes
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_nutrientrecipes.nutrientrecipesid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_nutrientrecipescf on vtiger_nutrientrecipescf.nutrientrecipesid=vtiger_nutrientrecipes.nutrientrecipesid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_nutrientrecipes.nutrientrecipesid, vtiger_nutrientrecipes.nutrientrecipesname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_nutrientrecipes
					inner join vtiger_contnutrientrecipesrel on vtiger_contnutrientrecipesrel.nutrientrecipesid = vtiger_nutrientrecipes.nutrientrecipesid
					inner join vtiger_contactdetails on vtiger_contnutrientrecipesrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_nutrientrecipes.nutrientrecipesid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_nutrientrecipes on vtiger_nutrientrecipes.nutrientrecipesid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'NutrientRecipes'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_nutrientrecipes ON vtiger_nutrientrecipes.nutrientrecipesid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_nutrientrecipes.nutrientrecipesid = $id";
	...
	/**
	* Function to get NutrientRecipes related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('NutrientRecipes',$query,$id);
	}


	  /**
	  * Function to get NutrientRecipes related Quotes
	  * @param  integer   $id  - nutrientrecipesid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'nutrientrecipes_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_nutrientrecipes.nutrientrecipesname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_nutrientrecipes on vtiger_nutrientrecipes.nutrientrecipesid=vtiger_quotes.nutrientrecipesid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contnutrientrecipesrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contnutrientrecipesrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contnutrientrecipesrel"=>"nutrientrecipesid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"nutrientrecipesid","vtiger_salesorder"=>"nutrientrecipesid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_nutrientrecipes","nutrientrecipesid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityNutrientRecipes on vtiger_crmentityNutrientRecipes.crmid=vtiger_nutrientrecipes.nutrientrecipesid and vtiger_crmentityNutrientRecipes.deleted=0
		left join vtiger_nutrientrecipescf on vtiger_nutrientrecipescf.nutrientrecipesid = vtiger_nutrientrecipes.nutrientrecipesid
		left join vtiger_groups vtiger_groupsNutrientRecipes on vtiger_groupsNutrientRecipes.groupid = vtiger_crmentityNutrientRecipes.smownerid
		left join vtiger_users as vtiger_usersNutrientRecipes on vtiger_usersNutrientRecipes.id = vtiger_crmentityNutrientRecipes.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_nutrientrecipes"=>"nutrientrecipesid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_nutrientrecipes"=>"nutrientrecipesid"),
			"Quotes" => array("vtiger_quotes"=>array("nutrientrecipesid","quoteid"),"vtiger_nutrientrecipes"=>"nutrientrecipesid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("nutrientrecipesid","salesorderid"),"vtiger_nutrientrecipes"=>"nutrientrecipesid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_nutrientrecipes"=>"nutrientrecipesid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-NutrientRecipes Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contnutrientrecipesrel WHERE nutrientrecipesid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
