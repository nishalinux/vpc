    ...
    <div class='editViewContainer tab-pane active' id="tab_accounts">
		...
		{else}
			<table class="table tableRWD table-bordered">
				...
				<tbody>
					{assign var=USERS_ENTITY_INFO value=$RRUSERS_ENTITY_INFO}
					{$RECORD_MODEL->getUserList()|@print_r} 
						...
						<tr id="row_account_{$row['user_id']}" style="{cycle values="'',background-color: #f9f9f9"}">
							...
							<td>
								<select id="user_list_{$row['user_id']}" data-user="{$row['user_id']}" name="user_list_{$row['user_id']}" class="form-control select2">
									<optgroup label="{vtranslate('User list', 'OSSMailScanner')}">
										...
										{foreach from=$USERS_LIST item=meuser}
											 
											<option value="{$meuser['id']}" {if $row['crm_user_id'] == $meuser['id']} selected="selected"{/if} >{$meuser['first_name']} {$meuser['last_name']}</option>
										{/foreach}
									</optgroup>
									<optgroup label="{vtranslate('Group list', 'OSSMailScanner')}">
										{foreach item=item from=$RECORD_MODEL->getAllGroupList()}
							...
							<td class='scanerMailActionsButtons'>
								<div class="btn-toolbar">
									<div class="btn-group">
										<button title="{vtranslate('LBL_SHOW_ACCOUNT_DETAILS', 'OSSMailScanner')}" type="button" data-user-id="1" class="btn btn-default expand-hide">
											<span class="glyphicon glyphicon-chevron-down">Inbox</span>
										</button>
										<button title="{vtranslate('LBL_EDIT_FOLDER_ACCOUNT', 'OSSMailScanner')}" type="button" data-user="1" class="btn btn-default editFolders">
											<span class="glyphicon glyphicon-folder-open">Sent</span>
										</button>
										<button title="{vtranslate('LBL_DELETE_ACCOUNT', 'OSSMailScanner')}" type="button" data-user-id="1" class="btn btn-default delate_accont">
