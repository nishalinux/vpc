 ...
 ********************************************************************************/
 // This is the proforma vtDZiner ExtractRun.php script which will get converted as the written out as ExtractRun.php
...
// vtiger_extractrun is a custom vtiger entity generated by vtDZiner Module DZiner
class ExtractRun extends CRMEntity {
	var $log;
	var $db;

	var $module_name="ExtractRun";
	var $table_name = "vtiger_extractrun";
	var $table_index= 'extractrunid';

	var $tab_name = Array('vtiger_crmentity','vtiger_extractrun','vtiger_extractruncf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_extractrun'=>'extractrunid','vtiger_extractruncf'=>'extractrunid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_extractruncf', 'extractrunid');
	...
	var $list_link_field= 'extractrunname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'extractrunname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'extractrunname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_extractrungrouprelation','extractrunid');
	function ExtractRun() {
		$this->log = LoggerManager::getLogger('extractrun');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('ExtractRun');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['EXTRACTRUN_SORT_ORDER'] != '')?($_SESSION['EXTRACTRUN_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'ExtractRunname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['EXTRACTRUN_ORDER_BY'] != '')?($_SESSION['EXTRACTRUN_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_extractrun.extractrunid,  vtiger_extractrun.extractrunname, vtiger_extractrun.dateclosed FROM vtiger_extractrun, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_extractrun.extractrunid, vtiger_extractrun.extractrunname, vtiger_crmentity.smcreatorid FROM vtiger_extractrun inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_extractrun.extractrunid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('ExtractRun',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export ExtractRun Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("ExtractRun", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_extractrun
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_extractrun.extractrunid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_extractruncf on vtiger_extractruncf.extractrunid=vtiger_extractrun.extractrunid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_extractrun.extractrunid, vtiger_extractrun.extractrunname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_extractrun
					inner join vtiger_contextractrunrel on vtiger_contextractrunrel.extractrunid = vtiger_extractrun.extractrunid
					inner join vtiger_contactdetails on vtiger_contextractrunrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_extractrun.extractrunid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_extractrun on vtiger_extractrun.extractrunid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'ExtractRun'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_extractrun ON vtiger_extractrun.extractrunid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_extractrun.extractrunid = $id";
	...
	/**
	* Function to get ExtractRun related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('ExtractRun',$query,$id);
	}


	  /**
	  * Function to get ExtractRun related Quotes
	  * @param  integer   $id  - extractrunid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'extractrun_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_extractrun.extractrunname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_extractrun on vtiger_extractrun.extractrunid=vtiger_quotes.extractrunid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contextractrunrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contextractrunrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contextractrunrel"=>"extractrunid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"extractrunid","vtiger_salesorder"=>"extractrunid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_extractrun","extractrunid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityExtractRun on vtiger_crmentityExtractRun.crmid=vtiger_extractrun.extractrunid and vtiger_crmentityExtractRun.deleted=0
		left join vtiger_extractruncf on vtiger_extractruncf.extractrunid = vtiger_extractrun.extractrunid
		left join vtiger_groups vtiger_groupsExtractRun on vtiger_groupsExtractRun.groupid = vtiger_crmentityExtractRun.smownerid
		left join vtiger_users as vtiger_usersExtractRun on vtiger_usersExtractRun.id = vtiger_crmentityExtractRun.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_extractrun"=>"extractrunid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_extractrun"=>"extractrunid"),
			"Quotes" => array("vtiger_quotes"=>array("extractrunid","quoteid"),"vtiger_extractrun"=>"extractrunid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("extractrunid","salesorderid"),"vtiger_extractrun"=>"extractrunid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_extractrun"=>"extractrunid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-ExtractRun Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contextractrunrel WHERE extractrunid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
