...
class OSSMailScanner_CreatedEmail_ScannerAction
{
	...
	public function process(OSSMail_Mail_Model $mail)
	{
		...
		if ($mailId === false && !empty($mail->get('message_id'))) {
			...
			$params12 =array($current_id,$current_user->id,$current_user->id,$current_user->id,'OSSMailView',$mail->get('subject'),$noqqq,$noqqq,0,1,$mail->get('subject'));
			//$log->debug($sql12);
            ...
            $seseq = "UPDATE `vtiger_crmentity_seq` SET `id`=? WHERE 1";
