	...
	public function process(OSSMail_Mail_Model $mail)
	{
		...
		if ($create) {
			...
			if ($mailId) {
				...
				$documentId = $db->query_result($result, 0, 'documentsid');
				//while ($documentId = $db->getSingleValue($result)) {
				/*	$db->insert('vtiger_senotesrel', [
						'crmid' => $id,
						'notesid' => $documentId
					]);*/
					$db->pquery('INSERT INTO vtiger_senotesrel (`crmid`,`notesid`) VALUES (?,?);', [$id, $documentId]);
				//}
			}
			...
			$db->pquery('UPDATE vtiger_crmentity SET createdtime=?,smcreatorid=?,modifiedby=? WHERE crmid=?;', [$mail->get('udate_formated'), $accountOwner,$accountOwner,$id]);
		}
		return $id;
	}
}
