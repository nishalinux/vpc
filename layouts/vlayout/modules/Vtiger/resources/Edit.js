			...
			if(typeof editViewForm.data('submit') != "undefined") {
				...
				if(editViewForm.validationEngine('validate')) {
                    ...
                    if(app.getModuleName()=='Invoice'){
                        ...
                        var ordered_inv=jQuery('input[name="cf_950"]').val();
                        var invoicedate=jQuery('input[name="invoicedate"]').val();

                        // calculate total sum grams
                        var invInstance=Inventory_Edit_Js.getInstance();
                        var products={};
                       var lineItemContentsContainer  = invInstance.getLineItemContentsContainer();
                        jQuery('tr.'+invInstance.rowClass ,lineItemContentsContainer).each(function(index,domElement){
                            var lineItemRow = jQuery(domElement);
                            var productid=  jQuery('input.selectedModuleId',lineItemRow).val();
                            var quantity=  jQuery('input.qty',lineItemRow).val();
                        ...
                        AppConnector.request(params).then(
                            function(data){
                               //alert(data.result.test)
                                ...
                                {
                                    Vtiger_Helper_Js.showPnotify(data.result.msg);
                                    editViewForm.removeData('submit');
                                }else{
                                    editViewForm.unbind('submit').submit();
                                }
                            }
                        );
                    }
...
//added by sl for signature :end
	...
	this.registerSubmitEvent();
		...
		this.registerEventForSignDelete();//added by sl for signature :end
		this.registerSignEvent();//added by sl for signature :end
