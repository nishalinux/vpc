 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Sample.php script which will get converted as the written out as Sample.php
...
// vtiger_sample is a custom vtiger entity generated by vtDZiner Module DZiner
class Sample extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Sample";
	var $table_name = "vtiger_sample";
	var $table_index= 'sampleid';

	var $tab_name = Array('vtiger_crmentity','vtiger_sample','vtiger_samplecf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_sample'=>'sampleid','vtiger_samplecf'=>'sampleid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_samplecf', 'sampleid');
	...
	var $popup_fields=Array('samplename');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'Sample'=>Array('sample'=>'samplename'),
	...
	var $list_link_field= 'samplename';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'samplename');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'samplename';
	var $default_sort_order = 'ASC';
	//anjaneya Date: 03-08-2016
	// This is the list of vtiger_fields that are in the lists.
	/* var $list_fields = Array(
				'Sample Name'=>Array('sample'=>'samplename'),
				'Sample Date'=>Array('samplecf'=>'cf_850'),
				'Image1'=>Array('samplecf'=>'cf_853'),
				'Image2'=>Array('samplecf'=>'cf_854'),
				'Chromatograph1'=>Array('samplecf'=>'cf_855'),
				'Chromatograph2'=>Array('samplecf'=>'cf_856')				 
				);
	var $list_fields_name = Array(
				...
				'Image1'=>'cf_853',
				'Image2'=>'cf_854',
				'Chromatograph1'=>'cf_855',
				'Chromatograph2'=> 'cf_856'
				     ); */
	//END anjaneya Date: 03-08-2016
	//var $groupTable = Array('vtiger_samplegrouprelation','sampleid');
	function Sample() {
		$this->log = LoggerManager::getLogger('sample');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Sample');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['SAMPLE_SORT_ORDER'] != '')?($_SESSION['SAMPLE_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Samplename')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['SAMPLE_ORDER_BY'] != '')?($_SESSION['SAMPLE_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_sample.sampleid,  vtiger_sample.samplename, vtiger_sample.dateclosed FROM vtiger_sample, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_sample.sampleid, vtiger_sample.samplename, vtiger_crmentity.smcreatorid FROM vtiger_sample inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_sample.sampleid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Sample',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Sample", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_sample
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_sample.sampleid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_samplecf on vtiger_samplecf.sampleid=vtiger_sample.sampleid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_sample.sampleid, vtiger_sample.samplename, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_sample
					inner join vtiger_contsamplerel on vtiger_contsamplerel.sampleid = vtiger_sample.sampleid
					inner join vtiger_contactdetails on vtiger_contsamplerel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_sample.sampleid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_sample on vtiger_sample.sampleid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Sample'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_sample ON vtiger_sample.sampleid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_sample.sampleid = $id";
	...
	/**
	* Function to get Sample related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Sample',$query,$id);
	}


	  /**
	  * Function to get Sample related Quotes
	  * @param  integer   $id  - sampleid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'sample_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_sample.samplename from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_sample on vtiger_sample.sampleid=vtiger_quotes.sampleid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contsamplerel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contsamplerel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contsamplerel"=>"sampleid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"sampleid","vtiger_salesorder"=>"sampleid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_sample","sampleid");
		$query .= " left join vtiger_crmentity as vtiger_crmentitySample on vtiger_crmentitySample.crmid=vtiger_sample.sampleid and vtiger_crmentitySample.deleted=0
		left join vtiger_samplecf on vtiger_samplecf.sampleid = vtiger_sample.sampleid
		left join vtiger_groups vtiger_groupsSample on vtiger_groupsSample.groupid = vtiger_crmentitySample.smownerid
		left join vtiger_users as vtiger_usersSample on vtiger_usersSample.id = vtiger_crmentitySample.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_sample"=>"sampleid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_sample"=>"sampleid"),
			"Quotes" => array("vtiger_quotes"=>array("sampleid","quoteid"),"vtiger_sample"=>"sampleid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("sampleid","salesorderid"),"vtiger_sample"=>"sampleid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_sample"=>"sampleid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Sample Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contsamplerel WHERE sampleid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
