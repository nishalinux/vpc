...
class Settings_OS2LoginHistory_Detail_View extends Settings_Vtiger_Index_View {
  ...
  $qry = "SELECT * from vtiger_users where user_name='$user_name'";
             $res = $adb->pquery($qry);
              ...
              $id = $row['id'];
			   //$login_time = Vtiger_Datetime_UIType::getDBDateTimeValue($signint_start,$id);
		// $logout_time = Vtiger_Datetime_UIType::getDBDateTimeValue($signint_end);
 ...
 public function getUplineList($recordId)
  {
    ...
    $qry1 = "SELECT login_id,login_time,logout_time,user_name FROM vtiger_loginhistory where login_id = '$recordId'";
        ...
        $row = $adb->fetchByAssoc($result);
         ...
         $logout_time = $row['logout_time'];
         $user_name = $row['user_name'];
		// $login_time = Vtiger_Datetime_UIType::getDBDateTimeValue($signint_start);
		// $logout_time = Vtiger_Datetime_UIType::getDBDateTimeValue($signint_end);
  ...
  $qry = "SELECT * from vtiger_users where user_name='$user_name'";
             $res = $adb->pquery($qry);
              ...
              $id = $row['id'];
			 // echo $login_time;
			 // $login_time = Vtiger_Datetime_UIType::getDBDateTimeValue($login_time,$id);
				//$login_time = Vtiger_Datetime_UIType::getDisplayDateTimeValue($login_time,$id);
				//$login_time_date = date('Y-m-d',strtotime($login_time));
				//$login_time_date = date('Y-m-d',strtotime($login_time));
				//$login_time_time = date('H:i:s A',strtotime($login_time));
				//$login_time_dt = Vtiger_Datetime_UIType::__convertToDBFormat($login_time_date);
				//$login_time_tm = Vtiger_Datetime_UIType::getTimeValueWithSeconds($login_time_time);
				//$login_time = $login_time_dt.' '.$login_time_tm;
