...
class Events_Record_Model extends Calendar_Record_Model {
	...
	public function getEditViewUrl() {
		...
		return 'index.php?module=Calendar&mode=Events&view='.$module->getEditViewName().'&record='.$this->getId();
	}
    ...
    public function getRelatedContactInfo() {
     ...
     public function getInviteUserMailData() {
            ...
            $mail_data['assign_type'] = $this->getInvities();
	 ...
	 //Manasa has written for custom purpose
	 public function getInviteUserMailEventsData($recordModule) {
            $adb = PearDatabase::getInstance();
	      $return_id = $recordModule->get('id');
            $cont_qry = "select * from vtiger_cntactivityrel where activityid=?";
            $cont_res = $adb->pquery($cont_qry, array($return_id));
            $noofrows = $adb->num_rows($cont_res);
            $cont_id = array();
			...
			$parentId = $recordModule->get('parent_id');
			$parentName = '';
			if($parentId != '') {
				$parentName = Vtiger_Util_Helper::getRecordName($parentId);
			}
            ...
            $cont_name  = trim($cont_name,', ');
            $mail_data = Array();
            $mail_data['user_id'] = $recordModule->get('assigned_user_id');
            $mail_data['subject'] = $recordModule->get('subject');
            $moduleName = $recordModule->getModuleName();
            $mail_data['status'] = (($moduleName=='Calendar')?($recordModule->get('taskstatus')):($recordModule->get('eventstatus')));
            $mail_data['activity_mode'] = (($moduleName=='Calendar')?('Task'):('Events'));
            $mail_data['taskpriority'] = $recordModule->get('taskpriority');
            $mail_data['relatedto'] = $parentName;
            $mail_data['contact_name'] = $cont_name;
            $mail_data['description'] = $recordModule->get('description');
            $mail_data['assign_type'] = $recordModule->get('assigntype');
            $mail_data['group_name'] = getGroupName($recordModule->get('assigned_user_id'));
            $mail_data['mode'] = $recordModule->get('mode');
            //TODO : remove dependency on request;
            $value = getaddEventPopupTime($_REQUEST['time_start'],$_REQUEST['time_end'],'24');
            $start_hour = $value['starthour'].':'.$value['startmin'].''.$value['startfmt'];
            if($_REQUEST['activity_mode']!='Task')
            ...
            $mail_data['st_date_time'] = $startDate->getDBInsertDateTimeValue();
            $mail_data['end_date_time'] = $endDate->getDBInsertDateTimeValue();
            $mail_data['location']=$recordModule->get('location');
