	...
	public function send() {
		...
		foreach($toEmailInfo as $id => $emails) {
			...
			foreach($emails as $email) {
				...
				$lquery = $db->pquery("select * from vtiger_leaddetails, vtiger_crmentity where (email like '{$email}' or secondaryemail like '{$email}') and vtiger_crmentity.crmid=vtiger_leaddetails.leadid and vtiger_crmentity.deleted=0", array());
				$lnrows = $db->num_rows($lquery);
				$log->debug("NNNN");
				$log->debug($lnrows);
				$cquery = $db->pquery("select * from vtiger_contactdetails, vtiger_crmentity where (email like '{$email}' or secondaryemail like '{$email}') and vtiger_crmentity.crmid=vtiger_contactdetails.contactid and vtiger_crmentity.deleted=0", array());
				...
				if(!$lnrows && !$cnrows && !$anrows){
					...
					$lcrmid = $db->pquery("SELECT MAX(crmid) AS cid FROM vtiger_crmentity", array());
					$lastcrmid = $db->query_result($lcrmid, 0, "cid");
					$lastcrmid--;
					$log->debug("WWWW");
					$log->debug($lastcrmid);					
					...
					$log->debug("NAMONAMO");
					$log->debug(html_entity_decode($sub)."//".html_entity_decode($bdy)."//".html_entity_decode($mfrom));
					/*
					...
					$IC = $db->pquery($insertCRM, $params1);
					$log->debug("XXXXX");
					$log->debug(print_r($params1, true));	
					
					$updatecrmseqquery = "UPDATE vtiger_crmentity_seq SET id=?";
					$updtCS = $adb->pquery($updatecrmseqquery, array($lastcrmid));
					*/
					$lqury = "SELECT MAX(leadid) AS lid FROM vtiger_leaddetails where email='".$email."'";
					$leadid = $db->pquery($lqury, array());
					$lid = $db->query_result($leadid, 0, "lid");
					...
					$MREL = $db->pquery($insertMailrel, $params3);	
					$log->debug("ZZZZZ");
