...
jQuery.Class("Settings_ProcessFlow_Js",{},{
	
	/*
	 * function to Save the Outgoing Server Details
	 */
	saveProcessFlowDetails : function(form) {
		...
		data.action = 'ProcessFlowSaveAjax';

		AppConnector.request(data).then(
			function(data) {
				if(data['success']) {
					var ProcessFlowDetailUrl = form.data('detailUrl');
					//after save, load detail view contents and register events
					thisInstance.loadContents(ProcessFlowDetailUrl).then(
						function(data) {
	...
	registerEditViewEvents : function() {
		var thisInstance = this;
		var form = jQuery('#ProcessFlowForm');
		...
		params.onValidationComplete = function(form, valid){
			if(valid) {
				thisInstance.saveProcessFlowDetails(form);
				return valid;
			}
		}
	...
	registerDetailViewEvents : function() {
		...
		var container = jQuery('#ProcessFlowDetails');
	...
	registerEvents: function() {
		...
		if(jQuery('#ProcessFlowForm').length > 0) {
			thisInstance.registerEditViewEvents();
		} else {
			thisInstance.registerDetailViewEvents();
...
jQuery(document).ready(function(e){
	var instance = new Settings_ProcessFlow_Js();
