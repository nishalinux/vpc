...
class Vtiger_PackageExport {
	...
	*/
	function xml_format(&$simpleXmlObject){
	...
	/**
	 * Cleanup the manifest file of custom fields
	 * @param String manifest_filename to use
	 */

	function cleanupManifest($manifest_filename){
		$newfieldnames = Array();
		$xml=simplexml_load_file($manifest_filename) or die("Error: Cannot create object");
		foreach($xml->children() as $blocks) { 
			$blocktype = $blocks->getName();
			foreach($blocks->children() as $block) { 
				foreach($block->children() as $fields) { 
					foreach($fields->children() as $field) { 
						if (strpos($field->fieldname, "cf_") === 0){
							$i = (string)$field->fieldname;
							if ($blocktype == 'blocks') {
								$newfieldname = str_replace(' ', '_', strtolower($field->fieldlabel));
								$newfieldname = str_replace('-', '_', $newfieldname);
								$newtablename = substr ( (string)$field->tablename  , 0, strlen((string)$field->tablename ) - 2 );
								$newfieldnames[$i]= $newfieldname;
								$field->fieldname = $newfieldname;
								$field->columnname = $newfieldname;
								$field->tablename = $newtablename;
							} else {
								$field->fieldname = $newfieldnames[$i];
							}
						}
					}
				}
		...
		$idname = explode("vtiger_", $newtablename)[1] . "id";
		$manifest_xml = $xml->asXML();
		$manifest_xml = str_replace(array_keys($newfieldnames), array_values($newfieldnames), $manifest_xml);
		$manifest_xml = preg_replace('/\n(^|[ ]*)\) ENGINE=InnoDB(.*)`'.$idname.'` int\(11\) NOT NULL,/s', ',', $manifest_xml);
		$cf_tablename = $newtablename . "cf";
		$mod_xml = new SimpleXMLElement($manifest_xml);

		$cf_table = $mod_xml->tables->addChild('table');
		$cf_table->addChild('name', $cf_tablename);
		$cf_table->addChild('sql','<![CDATA[CREATE TABLE `'.$cf_tablename.'` (`'.$idname.'` int(11) NOT NULL, PRIMARY KEY (`'.$idname.'`)) ENGINE=InnoDB DEFAULT CHARSET=utf8]]>');
		$mod_xml = $this->xml_format($mod_xml);
		file_put_contents($manifest_filename, $mod_xml);
	}
	/**
	...
	function export($moduleInstance, $todir='', $zipfilename='', $directDownload=false) {
		...
		$this->cleanupManifest($this->__getManifestFilePath());
		// Export as Zip
