...
jQuery.Class("Vtiger_Detail_Js",{
	...
	triggerSendEmail : function(detailActionUrl, module){
        ...
        Vtiger_Helper_Js.checkServerConfig(module).then(function(data){
			...
			//	window.open('index.php?module=OSSMail&view=composemail&sourceRecord='+parentRecord+'&sourceModule='+module_name);
			
					window.open('index.php?module=OSSMail&view=index&sourceRecord='+parentRecord+'&sourceModule='+module_name);
			} else {
	...
	saveComment : function(e) {
		...
		if(commentMode == "edit"){
			...
			postData['filename'] = thisInstance.commentfiles;
	...
	registerEventForRelatedTabClick : function(){
		...
		jQuery('.related', detailContainer).on('click', 'li', function(e, urlAttributes){
			...
			//anjaneya :: for PCM reload issue , date : 15-3-2017
			var n = url.search("ProjectTask"); 
			if(n > 0){
				element.progressIndicator({'mode': 'hide'});
