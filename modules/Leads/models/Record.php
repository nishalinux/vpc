...
class Leads_Record_Model extends Vtiger_Record_Model {
	...
	function getContactFieldsForLeadConvert() {
		...
		if ($moduleModel->isActive()) {
			$fieldModels = $moduleModel->getFields();
            $complusoryFields = array('firstname','cf_876', 'email','cf_879');// SL added accountid for convert lead popup
            foreach($fieldModels as $fieldName => $fieldModel) {
                if($fieldModel->isMandatory() &&  $fieldName != 'assigned_user_id') {//SL eliminated accountid for convert lead popup
                    $keyIndex = array_search($fieldName,$complusoryFields);
                    if($keyIndex !== false) {
                        unset($complusoryFields[$keyIndex]);
                    }
