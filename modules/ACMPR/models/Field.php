 ...
 */
class ACMPR_Field_Model extends Vtiger_Field_Model {
	public function getValidator() {
		$validator = array();
		$fieldName = $this->getName();

		switch($fieldName) {
			case 'num_arpic_submitting':	$funcName = array('name' => 'numbercountgridarpic');
								array_push($validator, $funcName);
								break;
            case 'num_persons_cannabis':	$funcName = array('name' => 'numbercountgridperson');
								array_push($validator, $funcName);
								break;
	...
	public function isAjaxEditable() {
		...
		if(!$this->isEditable() || in_array($this->get('uitype'), $ajaxRestrictedFields)) {
			return false;
		}else if($this->get('column') == 'num_arpic_submitting' || $this->get('column') == 'num_persons_cannabis'){
			return false;
		}
		return true;
	}
	public function getFieldDataType() {
		if($this->get('uitype') == '171') {
			return 'Radio';
		}
		if($this->get('uitype') == '845') {
			return 'Measurement';
		}
	...
	//Manasa added this special purpose 24-oct-2017
	public function getMeasurement($fieldname){
		global $adb;
		return $adb->query_result($adb->pquery("SELECT measurement FROM vtigress_measurement WHERE fieldname=?",array($fieldname)),0,'measurement');
	}
	//Manasa ended here
