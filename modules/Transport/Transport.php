 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Transport.php script which will get converted as the written out as Transport.php
...
// vtiger_transport is a custom vtiger entity generated by vtDZiner Module DZiner
class Transport extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Transport";
	var $table_name = "vtiger_transport";
	var $table_index= 'transportid';

	var $tab_name = Array('vtiger_crmentity','vtiger_transport','vtiger_transportcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_transport'=>'transportid','vtiger_transportcf'=>'transportid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_transportcf', 'transportid');
	...
	var $popup_fields=Array('transportname');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'Transport'=>Array('transport'=>'transportname'),
	...
	var $list_link_field= 'transportname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'transportname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'transportname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_transportgrouprelation','transportid');
	function Transport() {
		$this->log = LoggerManager::getLogger('transport');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Transport');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['TRANSPORT_SORT_ORDER'] != '')?($_SESSION['TRANSPORT_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Transportname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['TRANSPORT_ORDER_BY'] != '')?($_SESSION['TRANSPORT_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_transport.transportid,  vtiger_transport.transportname, vtiger_transport.dateclosed FROM vtiger_transport, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_transport.transportid, vtiger_transport.transportname, vtiger_crmentity.smcreatorid FROM vtiger_transport inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_transport.transportid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Transport',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export Transport Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Transport", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_transport
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_transport.transportid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_transportcf on vtiger_transportcf.transportid=vtiger_transport.transportid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_transport.transportid, vtiger_transport.transportname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_transport
					inner join vtiger_conttransportrel on vtiger_conttransportrel.transportid = vtiger_transport.transportid
					inner join vtiger_contactdetails on vtiger_conttransportrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_transport.transportid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_transport on vtiger_transport.transportid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Transport'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_transport ON vtiger_transport.transportid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_transport.transportid = $id";
	...
	/**
	* Function to get Transport related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Transport',$query,$id);
	}


	  /**
	  * Function to get Transport related Quotes
	  * @param  integer   $id  - transportid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'transport_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_transport.transportname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_transport on vtiger_transport.transportid=vtiger_quotes.transportid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_conttransportrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_conttransportrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_conttransportrel"=>"transportid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"transportid","vtiger_salesorder"=>"transportid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_transport","transportid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityTransport on vtiger_crmentityTransport.crmid=vtiger_transport.transportid and vtiger_crmentityTransport.deleted=0
		left join vtiger_transportcf on vtiger_transportcf.transportid = vtiger_transport.transportid
		left join vtiger_groups vtiger_groupsTransport on vtiger_groupsTransport.groupid = vtiger_crmentityTransport.smownerid
		left join vtiger_users as vtiger_usersTransport on vtiger_usersTransport.id = vtiger_crmentityTransport.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_transport"=>"transportid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_transport"=>"transportid"),
			"Quotes" => array("vtiger_quotes"=>array("transportid","quoteid"),"vtiger_transport"=>"transportid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("transportid","salesorderid"),"vtiger_transport"=>"transportid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_transport"=>"transportid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Transport Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_conttransportrel WHERE transportid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
