 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Mothers.php script which will get converted as the written out as Mothers.php
...
// vtiger_mothers is a custom vtiger entity generated by vtDZiner Module DZiner
class Mothers extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Mothers";
	var $table_name = "vtiger_mothers";
	var $table_index= 'mothersid';

	var $tab_name = Array('vtiger_crmentity','vtiger_mothers','vtiger_motherscf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_mothers'=>'mothersid','vtiger_motherscf'=>'mothersid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_motherscf', 'mothersid');
	...
	var $popup_fields=Array('mothersname');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'Mothers'=>Array('mothers'=>'mothersname'),
	...
	var $list_link_field= 'mothersname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'mothersname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'mothersname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_mothersgrouprelation','mothersid');
	function Mothers() {
		$this->log = LoggerManager::getLogger('mothers');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Mothers');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['MOTHERS_SORT_ORDER'] != '')?($_SESSION['MOTHERS_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Mothersname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['MOTHERS_ORDER_BY'] != '')?($_SESSION['MOTHERS_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_mothers.mothersid,  vtiger_mothers.mothersname, vtiger_mothers.dateclosed FROM vtiger_mothers, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_mothers.mothersid, vtiger_mothers.mothersname, vtiger_crmentity.smcreatorid FROM vtiger_mothers inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_mothers.mothersid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Mothers',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Mothers", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_mothers
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_mothers.mothersid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_motherscf on vtiger_motherscf.mothersid=vtiger_mothers.mothersid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_mothers.mothersid, vtiger_mothers.mothersname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_mothers
					inner join vtiger_contmothersrel on vtiger_contmothersrel.mothersid = vtiger_mothers.mothersid
					inner join vtiger_contactdetails on vtiger_contmothersrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_mothers.mothersid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_mothers on vtiger_mothers.mothersid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Mothers'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_mothers ON vtiger_mothers.mothersid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_mothers.mothersid = $id";
	...
	/**
	* Function to get Mothers related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Mothers',$query,$id);
	}


	  /**
	  * Function to get Mothers related Quotes
	  * @param  integer   $id  - mothersid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'mothers_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_mothers.mothersname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_mothers on vtiger_mothers.mothersid=vtiger_quotes.mothersid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contmothersrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contmothersrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contmothersrel"=>"mothersid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"mothersid","vtiger_salesorder"=>"mothersid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_mothers","mothersid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityMothers on vtiger_crmentityMothers.crmid=vtiger_mothers.mothersid and vtiger_crmentityMothers.deleted=0
		left join vtiger_motherscf on vtiger_motherscf.mothersid = vtiger_mothers.mothersid
		left join vtiger_groups vtiger_groupsMothers on vtiger_groupsMothers.groupid = vtiger_crmentityMothers.smownerid
		left join vtiger_users as vtiger_usersMothers on vtiger_usersMothers.id = vtiger_crmentityMothers.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_mothers"=>"mothersid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_mothers"=>"mothersid"),
			"Quotes" => array("vtiger_quotes"=>array("mothersid","quoteid"),"vtiger_mothers"=>"mothersid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("mothersid","salesorderid"),"vtiger_mothers"=>"mothersid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_mothers"=>"mothersid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Mothers Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contmothersrel WHERE mothersid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
