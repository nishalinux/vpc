...
class ProjectStockPDFHeaderViewer extends Vtiger_PDF_InventoryHeaderViewer {
    function display($parent) {
        ...
        if ($this->model) {
            ...
            $pdf->MultiCell($headerColumnWidth, $contentHeight, $modelColumnLeft['summary'], 0, 'L', 0, 1, $headerFrame->x, $headerFrame->y + $imageHeightInMM + 2);

            $pdf->SetFont('freeserif', '');
            $contentHeight = $pdf->GetStringHeight($modelColumnLeft['content'], $headerColumnWidth);
            $pdf->MultiCell($headerColumnWidth, $contentHeight, $modelColumnLeft['content'], 0, 'L', 0, 1, $headerFrame->x, $pdf->GetY());
            ...
            foreach ($modelColumnRight as $label => $value) {
                if (is_array($value)) {
                    ...
                    $pdf->SetFont('freeserif', 'B');
                    $pdf->SetFillColor(205, 201, 201);
                    $pdf->MultiCell($headerColumnWidth - $offsetX, 7, $label, 1, 'L', 1, 1, $headerFrame->x + $headerColumnWidth * 2.0 + $offsetX, $pdf->GetY() + $offsetY);
