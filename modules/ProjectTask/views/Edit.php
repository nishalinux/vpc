...
Class ProjectTask_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if(!empty($recordId)) {
			$projecttaskuserinfo = $this->getProjectTaskUserInfo($recordId);
			//var_dump($projecttaskuserinfo);
			$viewer->assign('PROJECT_TASK_USERINFO', $projecttaskuserinfo);
		}
	...
	//manasa
	public function getProjectTaskUserInfo($projecttaskid){
		global $adb,$current_user;
		$dateformat = str_replace(array('yyyy', 'mm','dd'), array('Y', 'm', 'd'), $current_user->date_format);
		$query = $adb->pquery("SELECT * FROM vtiger_usersprojecttasksrel WHERE projecttaskid=?",array($projecttaskid));
		while($row = $adb->fetch_array($query)){
			if($row['start_date'] != ''){
				$row['start_date'] = date($dateformat, strtotime($row['start_date']));
			}
			if($row['end_date'] != ''){
				$row['end_date'] = date($dateformat, strtotime($row['end_date']));
			}
			$result[] = $row;
		}
		return $result;
