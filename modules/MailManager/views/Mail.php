...
class MailManager_Mail_View extends MailManager_Abstract_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if ('open' == $this->getOperationArg($request)) {
			...
			$viewer->assign('ATTACHMENTS', $mail->attachments(true));
			$body = $mail->body();
...
$log->debug('forward line 30666666666666');
...
$log->debug(count($attachments));
$log->debug('forward line 320');

$log->debug(count($inlineAttachments));

			if(is_array($inlineAttachments)) {
				$log->debug('forward line 325 inside if');

				foreach($inlineAttachments as $index => $att) {
									$log->debug('forward line 325 inside if for');
			...
			if (!empty($attachments)) {
													$log->debug('forward line 340 inside if  ');

				foreach($attachments as $aValue) {
														$log->debug('forward line 343 inside if for each');

					if(!empty($inline_cid[$aValue['filename']])) {
																				$log->debug('forward line 343 inside if for each if');

						continue;
					}
					$attachInfo = $mail->__SaveAttachmentFile($aValue['filename'], $aValue['data']);
																									$log->debug('forward line 350 inside if for each if');
																									$log->debug(print_r($attachInfo,true));

					if(is_array($attachInfo) && !empty($attachInfo) && $attachInfo['size'] > 0) {
																									$log->debug('forward line 355 inside if for each if');
																									//$log->debug(print_r($attachInfo,true));

						if(!MailManager::checkModuleWriteAccessForCurrentUser('Documents')) return;
																									$log->debug('forward line 359 inside if for each if');
																									//$log->debug(print_r($attachInfo,true));
