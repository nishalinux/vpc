...
jQuery.Class("Calendar_CalendarView_Js",{
	...
	fetchCalendarFeed : function(feedcheckbox) {
		...
		this.calendarfeedDS[type] = function(start, end, callback) {
			...
			//alert(3);
			var restoreMessage= "Loading Please Wait..";
			...
			AppConnector.request(params).then(function(events){
				...
				feedcheckbox.attr('disabled', false).attr('checked', true);
				 progressIndicatorElement.progressIndicator({'mode' : 'hide'});//manasa aug 22 2018
			},
            function(error){
...
console.log(this.calendarfeedDS[type]);
	...
	registerCalendarFeedChange : function() {
		...
		jQuery('#calendarview-feeds').on('change', '[data-calendar-feed]', function(e){
			...
			if(currentTarget.is(':checked')) {
				...
				thisInstance.vtigressCalendarRemove(currentTarget);
				...
				thisInstance.vtigressCalendarAdd(currentTarget);
			}
		});
	},
	//vtigress added this on oct 21 2016
	vtigressCalendarAdd : function (currentTarget){
	var thisInstance = this;
			var params = {
				module: 'Calendar',
				action: 'OtherFeed',
				...
				type: currentTarget.data('calendar-feed'),
				fieldname: currentTarget.data('calendar-fieldname'),
				userid : currentTarget.data('calendar-userid'),
				color : currentTarget.data('calendar-feed-color'),
				textColor : currentTarget.data('calendar-feed-textcolor')
			}
			var sourcekey = currentTarget.data('calendar-sourcekey');
			var restoreMessage= "Loading Please Wait..";
			 var progressIndicatorElement = jQuery.progressIndicator({
                    'message' : restoreMessage,
                    'position' : 'html',
			...
			AppConnector.request(params).then(function(events){
			if(events != null){
					thisInstance.getCalendarView().fullCalendar('addEventSource', events);
			}
		 progressIndicatorElement.progressIndicator({'mode' : 'hide'});
	});
},
vtigressCalendarRemove : function (currentTarget){
	var thisInstance = this;
			var params = {
				module: 'Calendar',
				action: 'OtherFeed',
	...
	AppConnector.request(params).then(function(events){
		if(events != null){
			...
			$.each(events,function(k,v){
				eventid = v.id;
				//console.log(eventid);
				//thisInstance.getCalendarView().fullCalendar('removeEvents', eventid);//removeEvents
			});
           ...
           eventResize : function(event, dayDelta, minuteDelta, revertFunc, jsEvent, ui, view){
               if(event.module != 'Calendar' && event.module != 'Events'){
                ...
                AppConnector.request(params).then(function(data){
					...
					//Manasa added on 16th Aug 2018 Drag and drop purpose
					 if(response['result'].iserror){
						  Vtiger_Helper_Js.showPnotify(response['result'].iserror);
						  revertFunc();
					 }
					 //Ended here
                });
