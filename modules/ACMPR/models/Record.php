...
class ACMPR_Record_Model extends Vtiger_Record_Model {
	...
	public function getImageDetails($fieldname) {
		...
		if ($recordId) {
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						...
						WHERE vtiger_crmentity.setype = 'ACMPR Attachment' AND vtiger_seattachmentsrel.crmid = ? and subject=?";
	...
	public function getGalleryDetails($fieldname) {
		...
		if ($recordId) {
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'ACMPR Attachment' AND vtiger_seattachmentsrel.crmid = ?  and subject=? ORDER BY subject";
...
public function getGalleryEditDetails() {
		...
		if ($recordId) {
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'ACMPR Attachment' AND vtiger_seattachmentsrel.crmid = ? ORDER BY subject";
