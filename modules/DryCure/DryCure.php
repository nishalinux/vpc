 ...
 ********************************************************************************/
 // This is the proforma vtDZiner DryCure.php script which will get converted as the written out as DryCure.php
...
// vtiger_drycure is a custom vtiger entity generated by vtDZiner Module DZiner
class DryCure extends CRMEntity {
	var $log;
	var $db;

	var $module_name="DryCure";
	var $table_name = "vtiger_drycure";
	var $table_index= 'drycureid';

	var $tab_name = Array('vtiger_crmentity','vtiger_drycure','vtiger_drycurecf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_drycure'=>'drycureid','vtiger_drycurecf'=>'drycureid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_drycurecf', 'drycureid');
	...
	var $popup_fields=Array('drycurename');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'DryCure'=>Array('drycure'=>'drycurename'),
	...
	var $list_link_field= 'drycurename';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'drycurename');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'drycurename';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_drycuregrouprelation','drycureid');
	function DryCure() {
		$this->log = LoggerManager::getLogger('drycure');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('DryCure');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['DRYCURE_SORT_ORDER'] != '')?($_SESSION['DRYCURE_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'DryCurename')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['DRYCURE_ORDER_BY'] != '')?($_SESSION['DRYCURE_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_drycure.drycureid,  vtiger_drycure.drycurename, vtiger_drycure.dateclosed FROM vtiger_drycure, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_drycure.drycureid, vtiger_drycure.drycurename, vtiger_crmentity.smcreatorid FROM vtiger_drycure inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_drycure.drycureid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('DryCure',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("DryCure", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_drycure
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_drycure.drycureid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_drycurecf on vtiger_drycurecf.drycureid=vtiger_drycure.drycureid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_drycure.drycureid, vtiger_drycure.drycurename, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_drycure
					inner join vtiger_contdrycurerel on vtiger_contdrycurerel.drycureid = vtiger_drycure.drycureid
					inner join vtiger_contactdetails on vtiger_contdrycurerel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_drycure.drycureid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_drycure on vtiger_drycure.drycureid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'DryCure'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_drycure ON vtiger_drycure.drycureid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_drycure.drycureid = $id";
	...
	/**
	* Function to get DryCure related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('DryCure',$query,$id);
	}


	  /**
	  * Function to get DryCure related Quotes
	  * @param  integer   $id  - drycureid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'drycure_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_drycure.drycurename from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_drycure on vtiger_drycure.drycureid=vtiger_quotes.drycureid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contdrycurerel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contdrycurerel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contdrycurerel"=>"drycureid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"drycureid","vtiger_salesorder"=>"drycureid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_drycure","drycureid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityDryCure on vtiger_crmentityDryCure.crmid=vtiger_drycure.drycureid and vtiger_crmentityDryCure.deleted=0
		left join vtiger_drycurecf on vtiger_drycurecf.drycureid = vtiger_drycure.drycureid
		left join vtiger_groups vtiger_groupsDryCure on vtiger_groupsDryCure.groupid = vtiger_crmentityDryCure.smownerid
		left join vtiger_users as vtiger_usersDryCure on vtiger_usersDryCure.id = vtiger_crmentityDryCure.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_drycure"=>"drycureid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_drycure"=>"drycureid"),
			"Quotes" => array("vtiger_quotes"=>array("drycureid","quoteid"),"vtiger_drycure"=>"drycureid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("drycureid","salesorderid"),"vtiger_drycure"=>"drycureid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_drycure"=>"drycureid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-DryCure Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contdrycurerel WHERE drycureid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
