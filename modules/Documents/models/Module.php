...
class Documents_Module_Model extends Vtiger_Module_Model {
	...
	/**
	 * Function returns list of associated modules
	 * @return <Array> folder list
	 */
	public static function getAllModuleAssociations() {
		$db = PearDatabase::getInstance();
				$custommoduleslist =$db->pquery( "INSERT vtiger_documentmodulesrel (
			tabid, modulename, modulelabel, defaultfolder, multiplerecords, enabled
			)SELECT tabid, name, tablabel, 1, false, false FROM vtiger_tab WHERE isentitytype=1 AND tabid NOT IN (8,9,10,16,28,35,39,40) and tabid not in (select tabid from vtiger_documentmodulesrel)",array());

		$result = $db->pquery('SELECT * FROM vtiger_documentmodulesrel, vtiger_attachmentsfolder WHERE defaultfolder=folderid ORDER BY tabid', array());
	...
	/**
	 * Function returns the reports Settings
	 * @param <Number> $limit
	 * @return <Array of Reports_Record_Model>
	 */
	public function getDDSettings() {
		require_once("modules/Documents/config.php");
		return $vtDocsSettings;
	}
    ...
    public function getSettingLinks() {
		...
		$settingsLinks[] = array(
				'linktype' => 'LISTVIEWSETTING',
				'linklabel' => 'Docs++',
				'linkurl' => 'index.php?parent=Settings&module=Documents&view=Settings&sourceModule='.$this->getName(),
					'linkicon' => ''
			);
		$settingsLinks[] = array(
				'linktype' => 'LISTVIEWSETTING',
				'linklabel' => 'Folder Tree',
				'linkurl' => 'index.php?module=Documents&view=Folders',
				'linkicon' => ''
			);
