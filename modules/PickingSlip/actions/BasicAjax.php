...
class PickingSlip_BasicAjax_Action extends Vtiger_BasicAjax_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		if($searchModule == 'Products' && $relatedModule == 'PickingSlip'){
			$result = array();
			$projectid = $request->get('projectid');
			$staff = $this->getPickingSlipProjects($searchValue,$projectid);
			for($i=0;$i<count($staff[0]);$i++){
				$name = $staff[0][$i];
				$fval = $staff[1][$i];
				$result[] = array('label'=>$name, 'value'=>$name, 'id'=>$fval);
			}
		}else if($searchModule == 'Project' && $relatedModule == 'PickingSlip'){
			$result = array();
			$projectid = $request->get('projectid');
	...
	public function getPickingSlipProjects($searchTerm,$projectid){
		global $adb;
		//$adb->setDebug(true);
	...
	public function getInProgressProjects($searchTerm){
		global $adb;
		//$adb->setDebug(true);
		$sql = $adb->pquery("SELECT * FROM vtiger_project,vtiger_crmentity where vtiger_crmentity.crmid = vtiger_project.projectid  and deleted=0 and projectname like '%$searchTerm%' and projectstatus not in ('completed') ",array());
		$rows=$adb->num_rows($sql);
		$staff =array();
		for($i=0;$i<$rows;$i++){
			$staffname = $adb->query_result($sql,$i,'projectname');
