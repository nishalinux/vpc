 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Insurance.php script which will get converted as the written out as Insurance.php
...
// vtiger_insurance is a custom vtiger entity generated by vtDZiner Module DZiner
class Insurance extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Insurance";
	...
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_insurance'=>'insuranceid','vtiger_insurancecf'=>'insuranceid');
	/**
	...
	var $popup_fields=Array('insurancename');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'Insurance'=>Array('insurance'=>'insurancename'),
	...
	var $list_link_field= 'insurancename';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'insurancename');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'insurancename';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_insurancegrouprelation','insuranceid');
	function Insurance() {
		$this->log = LoggerManager::getLogger('insurance');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Insurance');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['INSURANCE_SORT_ORDER'] != '')?($_SESSION['INSURANCE_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Insurancename')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['INSURANCE_ORDER_BY'] != '')?($_SESSION['INSURANCE_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_insurance.insuranceid,  vtiger_insurance.insurancename, vtiger_insurance.dateclosed FROM vtiger_insurance, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_insurance.insuranceid, vtiger_insurance.insurancename, vtiger_crmentity.smcreatorid FROM vtiger_insurance inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_insurance.insuranceid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Insurance',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export Insurance Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Insurance", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_insurance
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_insurance.insuranceid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_insurancecf on vtiger_insurancecf.insuranceid=vtiger_insurance.insuranceid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_insurance.insuranceid, vtiger_insurance.insurancename, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_insurance
					inner join vtiger_continsurancerel on vtiger_continsurancerel.insuranceid = vtiger_insurance.insuranceid
					inner join vtiger_contactdetails on vtiger_continsurancerel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_insurance.insuranceid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_insurance on vtiger_insurance.insuranceid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Insurance'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_insurance ON vtiger_insurance.insuranceid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_insurance.insuranceid = $id";
	...
	/**
	* Function to get Insurance related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Insurance',$query,$id);
	}


	  /**
	  * Function to get Insurance related Quotes
	  * @param  integer   $id  - insuranceid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'insurance_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_insurance.insurancename from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_insurance on vtiger_insurance.insuranceid=vtiger_quotes.insuranceid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_continsurancerel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_continsurancerel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_continsurancerel"=>"insuranceid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"insuranceid","vtiger_salesorder"=>"insuranceid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_insurance","insuranceid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityInsurance on vtiger_crmentityInsurance.crmid=vtiger_insurance.insuranceid and vtiger_crmentityInsurance.deleted=0
		left join vtiger_insurancecf on vtiger_insurancecf.insuranceid = vtiger_insurance.insuranceid
		left join vtiger_groups vtiger_groupsInsurance on vtiger_groupsInsurance.groupid = vtiger_crmentityInsurance.smownerid
		left join vtiger_users as vtiger_usersInsurance on vtiger_usersInsurance.id = vtiger_crmentityInsurance.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_insurance"=>"insuranceid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_insurance"=>"insuranceid"),
			"Quotes" => array("vtiger_quotes"=>array("insuranceid","quoteid"),"vtiger_insurance"=>"insuranceid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("insuranceid","salesorderid"),"vtiger_insurance"=>"insuranceid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_insurance"=>"insuranceid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Insurance Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_continsurancerel WHERE insuranceid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
