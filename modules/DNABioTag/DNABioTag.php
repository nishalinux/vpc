 ...
 ********************************************************************************/
 // This is the proforma vtDZiner DNABioTag.php script which will get converted as the written out as DNABioTag.php
...
// vtiger_dnabiotag is a custom vtiger entity generated by vtDZiner Module DZiner
class DNABioTag extends CRMEntity {
	var $log;
	var $db;

	var $module_name="DNABioTag";
	var $table_name = "vtiger_dnabiotag";
	var $table_index= 'dnabiotagid';

	var $tab_name = Array('vtiger_crmentity','vtiger_dnabiotag','vtiger_dnabiotagcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_dnabiotag'=>'dnabiotagid','vtiger_dnabiotagcf'=>'dnabiotagid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_dnabiotagcf', 'dnabiotagid');
	...
	var $list_link_field= 'dnabiotagname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'dnabiotagname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'dnabiotagname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_dnabiotaggrouprelation','dnabiotagid');
	function DNABioTag() {
		$this->log = LoggerManager::getLogger('dnabiotag');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('DNABioTag');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['DNABIOTAG_SORT_ORDER'] != '')?($_SESSION['DNABIOTAG_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'DNABioTagname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['DNABIOTAG_ORDER_BY'] != '')?($_SESSION['DNABIOTAG_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_dnabiotag.dnabiotagid,  vtiger_dnabiotag.dnabiotagname, vtiger_dnabiotag.dateclosed FROM vtiger_dnabiotag, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_dnabiotag.dnabiotagid, vtiger_dnabiotag.dnabiotagname, vtiger_crmentity.smcreatorid FROM vtiger_dnabiotag inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_dnabiotag.dnabiotagid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('DNABioTag',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export DNABioTag Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("DNABioTag", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_dnabiotag
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_dnabiotag.dnabiotagid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_dnabiotagcf on vtiger_dnabiotagcf.dnabiotagid=vtiger_dnabiotag.dnabiotagid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_dnabiotag.dnabiotagid, vtiger_dnabiotag.dnabiotagname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_dnabiotag
					inner join vtiger_contdnabiotagrel on vtiger_contdnabiotagrel.dnabiotagid = vtiger_dnabiotag.dnabiotagid
					inner join vtiger_contactdetails on vtiger_contdnabiotagrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_dnabiotag.dnabiotagid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_dnabiotag on vtiger_dnabiotag.dnabiotagid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'DNABioTag'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_dnabiotag ON vtiger_dnabiotag.dnabiotagid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_dnabiotag.dnabiotagid = $id";
	...
	/**
	* Function to get DNABioTag related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('DNABioTag',$query,$id);
	}


	  /**
	  * Function to get DNABioTag related Quotes
	  * @param  integer   $id  - dnabiotagid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'dnabiotag_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_dnabiotag.dnabiotagname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_dnabiotag on vtiger_dnabiotag.dnabiotagid=vtiger_quotes.dnabiotagid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contdnabiotagrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contdnabiotagrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contdnabiotagrel"=>"dnabiotagid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"dnabiotagid","vtiger_salesorder"=>"dnabiotagid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_dnabiotag","dnabiotagid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityDNABioTag on vtiger_crmentityDNABioTag.crmid=vtiger_dnabiotag.dnabiotagid and vtiger_crmentityDNABioTag.deleted=0
		left join vtiger_dnabiotagcf on vtiger_dnabiotagcf.dnabiotagid = vtiger_dnabiotag.dnabiotagid
		left join vtiger_groups vtiger_groupsDNABioTag on vtiger_groupsDNABioTag.groupid = vtiger_crmentityDNABioTag.smownerid
		left join vtiger_users as vtiger_usersDNABioTag on vtiger_usersDNABioTag.id = vtiger_crmentityDNABioTag.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_dnabiotag"=>"dnabiotagid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_dnabiotag"=>"dnabiotagid"),
			"Quotes" => array("vtiger_quotes"=>array("dnabiotagid","quoteid"),"vtiger_dnabiotag"=>"dnabiotagid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("dnabiotagid","salesorderid"),"vtiger_dnabiotag"=>"dnabiotagid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_dnabiotag"=>"dnabiotagid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-DNABioTag Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contdnabiotagrel WHERE dnabiotagid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
