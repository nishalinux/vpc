 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Employees.php script which will get converted as the written out as Employees.php
...
// vtiger_employees is a custom vtiger entity generated by vtDZiner Module DZiner
class Employees extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Employees";
	...
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_employees'=>'employeesid','vtiger_employeescf'=>'employeesid');
	/**
	...
	var $popup_fields=Array('employeesname');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'Employees'=>Array('employees'=>'employeesname'),
	...
	var $list_link_field= 'employeesname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'employeesname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'employeesname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_employeesgrouprelation','employeesid');
	function Employees() {
		$this->log = LoggerManager::getLogger('employees');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Employees');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['EMPLOYEES_SORT_ORDER'] != '')?($_SESSION['EMPLOYEES_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Employeesname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['EMPLOYEES_ORDER_BY'] != '')?($_SESSION['EMPLOYEES_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_employees.employeesid,  vtiger_employees.employeesname, vtiger_employees.dateclosed FROM vtiger_employees, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_employees.employeesid, vtiger_employees.employeesname, vtiger_crmentity.smcreatorid FROM vtiger_employees inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_employees.employeesid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Employees',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export Employees Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Employees", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				...
				LEFT JOIN vtiger_employeescf on vtiger_employeescf.employeesid=vtiger_employees.employeesid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_employees.employeesid, vtiger_employees.employeesname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_employees
					inner join vtiger_contemployeesrel on vtiger_contemployeesrel.employeesid = vtiger_employees.employeesid
					inner join vtiger_contactdetails on vtiger_contemployeesrel.contactid = vtiger_contactdetails.contactid
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_employees on vtiger_employees.employeesid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Employees'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_employees ON vtiger_employees.employeesid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_employees.employeesid = $id";
	...
	/**
	* Function to get Employees related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Employees',$query,$id);
	}


	  /**
	  * Function to get Employees related Quotes
	  * @param  integer   $id  - employeesid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'employees_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_employees.employeesname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_employees on vtiger_employees.employeesid=vtiger_quotes.employeesid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contemployeesrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contemployeesrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contemployeesrel"=>"employeesid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"employeesid","vtiger_salesorder"=>"employeesid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_employees","employeesid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityEmployees on vtiger_crmentityEmployees.crmid=vtiger_employees.employeesid and vtiger_crmentityEmployees.deleted=0
		left join vtiger_employeescf on vtiger_employeescf.employeesid = vtiger_employees.employeesid
		left join vtiger_groups vtiger_groupsEmployees on vtiger_groupsEmployees.groupid = vtiger_crmentityEmployees.smownerid
		left join vtiger_users as vtiger_usersEmployees on vtiger_usersEmployees.id = vtiger_crmentityEmployees.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_employees"=>"employeesid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_employees"=>"employeesid"),
			"Quotes" => array("vtiger_quotes"=>array("employeesid","quoteid"),"vtiger_employees"=>"employeesid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("employeesid","salesorderid"),"vtiger_employees"=>"employeesid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_employees"=>"employeesid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Employees Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contemployeesrel WHERE employeesid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
