...
class MailManager_Draft_Model
{
    ...
    {
        global $adb, $log, $current_user; //murali
         $currentUserModel = Users_Record_Model::getCurrentUserModel();
        $uid              = $currentUserModel->getId(); //murali
        ...
        $ordeybyq = ' ORDER BY  id DESC ';
        $u_email  = "select mail_username from vtiger_mail_accounts where user_id=$uid"; //murali
         $u_result   = $adb->pquery($u_email, array()); //murali
        $user_email = $adb->query_result($u_result, 0, "mail_username"); //murali
         
        $user_email = "'" . $user_email . "'"; //murali
        
        $query = "SELECT * FROM Emails where from_email = $user_email and email_flag='SAVED' $where $ordeybyq $limitClause;"; //murali
		
         $draftMails = vtws_query($query, $currentUserModel);
        //$draftMails_raw = $adb->pquery($query,array());// $currentUserModel);
        //$draftMails = $adb->result_array($draftMails_raw);// $currentUserModel);
        
        
          $cul = vglobal('current_user');
        //$draftMails1 = vtws_query($query, $cul);
    ...
    public function getTotalDraftCount_change()
    {
        global $log;
        ...
        $currentUserModel = Users_Record_Model::getCurrentUserModel();
         $uid = $currentUserModel->getId(); //murali
         $u_email  = "select mail_username from vtiger_mail_accounts where user_id=$uid"; //murali
        $u_result = $db->pquery($u_email, array()); //murali
        
        $user_email = $db->query_result($u_result, 0, "mail_username"); //murali
         
        $user_email = "'" . $user_email . "'"; //murali
		
        if (empty(self::$totalDraftCount)) {
            $u_result = $db->pquery("SELECT count(*) as totalDrafts FROM vtiger_emaildetails where from_email = $user_email and email_flag='SAVED'", array());
 			$totalDrafts = $db->query_result($u_result, 0, "totalDrafts"); //murali
			
 			
            self::$totalDraftCount = $totalDrafts;
            return self::$totalDraftCount;
        } else {
            self::$totalDraftCount = 0;
            return self::$totalDraftCount;
        }
    }
    ...
    public function getTotalDraftCount()
    {
        ...
        $user_email       = "'" . $user_email . "'"; //murali
        if (empty(self::$totalDraftCount)) {
            $DraftRes              = $query = "SELECT * FROM Emails where from_email = $user_email and email_flag='SAVED';";
            $draftMails            = vtws_query($query, $currentUserModel);
    ...
    public function saveEmailParentRel($emailId, $parentIds)
    {
        ...
        if (!empty($emailId)) {
            $db->pquery("delete from vtiger_seactivityrel where activityid=?", array(
                $emailId
        ...
        for ($i = 0; $i < (count($myids)); $i++) {
            ...
            if (!empty($realid[0]) && !empty($emailId)) {
                ...
                $result = $db->pquery("SELECT * FROM vtiger_seactivityrel WHERE crmid=? AND activityid=?", array(
                    $realid[0],
                    $emailId
                ));
                if (!$db->num_rows($result)) {
                    $db->pquery('INSERT INTO vtiger_seactivityrel(crmid, activityid) VALUES(?,?)', array(
                        $realid[0],
                        $emailId
                    ));
                }
    ...
    public function getFromEmailAddress()
    {
        ...
        if (Vtiger_Version::check('5.2.0', '>=')) {
            $smtpFromResult = $db->pquery('SELECT from_email_field FROM vtiger_systems WHERE server_type=?', array(
                'email'
            ));
    ...
    public function saveEmailDocumentRel($emailId, $documentId)
    {
        ...
        if (!empty($emailId) && !empty($documentId)) {
            $db->pquery("INSERT INTO vtiger_senotesrel(crmid, notesid) VALUES(?,?)", Array(
                $emailId,
                $documentId
            ));
        }
    ...
    public function saveAttachmentRel($crmid, $attachId)
    {
        ...
        if (!empty($crmid) && !empty($attachId)) {
            $db->pquery("INSERT INTO vtiger_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)", Array(
                $crmid,
                $attachId
            ));
        }
    ...
    public function deleteAttachment($request)
    {
        ...
        if (!empty($docid) && !empty($emailid)) {
            $db->pquery("DELETE FROM vtiger_senotesrel WHERE crmid = ? AND notesid = ?", array(
                $emailid,
                $docid
            ));
            return true;
