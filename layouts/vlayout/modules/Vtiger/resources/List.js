...
jQuery.Class("Vtiger_List_Js",{
   ...
   getInstance: function(){
	...
	triggerSendEmail : function(massActionUrl, module, params){
		var mainurl = massActionUrl;
		var repl = mainurl.replace("index.php?module=", "");
		var module_name = repl.split("&", 1);
		...
		if(validationResult != true){
			...
			//window.open('index.php?module=OSSMail&view=composemail&sourceRecord='+selectedIds+'&sourceModule='+module_name);
			
				window.open('index.php?module=OSSMail&view=index&sourceRecord='+selectedIds+'&sourceModule='+module_name);
	...
	massBarcodeGenerationRecords : function(url,instance) {
		var	listInstance = Vtiger_List_Js.getInstance();
		if(typeof instance != "undefined"){
			listInstance = instance;
		}
		...
		if(validationResult != true){
			...
			var excludedIds = listInstance.readExcludedIds(true);
			var cvId = listInstance.getCurrentCvId();
			var message = app.vtranslate('LBL_MASS_BARCODE_CONFIRMATION');
			Vtiger_Helper_Js.showConfirmationBox({'message' : message}).then(
				function(e) {
	...
	*/
	...
	registerEvents : function(){
		...
		this.registerAutoCompleteFields(listViewContainer); //Added for listViewSearch
	},
	
	registerAutoCompleteFields : function(container) {
		var thisInstance = this;
		container.find('input.listSearchContributor').autocomplete({
			'minLength' : '3',
	...
	setReferenceFieldValue : function(container, params) {
		...
		fieldElement.trigger(Vtiger_List_Js.referenceSelectionEvent, {'source_module' : popupReferenceModule, 'record' : id, 'selectedName' : selectedName});
