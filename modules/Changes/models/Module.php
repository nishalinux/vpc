...
class Changes_Module_Model extends Products_Module_Model {
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		...
		if (($sourceModule == 'PriceBooks' && $field == 'priceBookRelatedList')
			...
			$condition = " vtiger_change.discontinued = 1 ";

			if ($sourceModule == 'PriceBooks' && $field == 'priceBookRelatedList') {
				$condition .= " AND vtiger_change.changeid NOT IN (SELECT productid FROM vtiger_pricebookproductrel WHERE pricebookid = '$record') ";
			} elseif (in_array($sourceModule, $supportedModulesList)) {
				$condition .= " AND vtiger_change.changeid NOT IN (SELECT relcrmid FROM vtiger_crmentityrel WHERE crmid = '$record' UNION SELECT crmid FROM vtiger_crmentityrel WHERE relcrmid = '$record') ";
			}
	...
	/**
	 * Function returns query for Changes-PriceBooks Relationship
	 * @param <Vtiger_Record_Model> $recordModel
	 * @param <Vtiger_Record_Model> $relatedModuleModel
	 * @return <String>
	 */
	function get_change_pricebooks($recordModel, $relatedModuleModel) {
		$query = 'SELECT vtiger_pricebook.pricebookid, vtiger_pricebook.bookname, vtiger_pricebook.active, vtiger_crmentity.crmid, 
						vtiger_crmentity.smownerid, vtiger_pricebookproductrel.listprice, vtiger_change.unit_price
					...
					INNER JOIN vtiger_change on vtiger_change.changeid = vtiger_pricebookproductrel.productid
					...
					WHERE vtiger_change.changeid = '.$recordModel->getId().' and vtiger_crmentity.deleted = 0';
