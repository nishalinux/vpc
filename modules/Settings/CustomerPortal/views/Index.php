...
//added by jyothi
		$profiles = self::getProfilePresent();
		$viewer->assign('PROFILE_MODELS',$profiles);

		$portalvalue = self::getTypesPresent();
		$viewer->assign('PORTAL_VALUES',$portalvalue);
//ende here
		$viewer->view('Index.tpl', $qualifiedModuleName);
	}

	public function getProfilePresent(){
		global $adb;
		// $profiles = array();
		$sql = $adb->query("SELECT * FROM `vtiger_profile`");
		while($res = $adb->fetchByAssoc($sql)){
			$profiles[] = $res['profilename'];
		}
		// print_r($profiles);
		return $profiles;
	}

	public function getTypesPresent(){
		global $adb;
		// $profiles = array();
		$sql = $adb->query("SELECT * FROM `vtiger_portaltype`");
		while($res = $adb->fetchByAssoc($sql)){
			$portalvalue[] = $res['portaltype'];
		}
		// print_r($profiles);
		return $portalvalue;
	}
