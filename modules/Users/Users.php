...
class Users_CRMSetup {
    ...
    function vtlib_handler($modulename, $event_type) { 
        ...
        if($event_type == 'module.postinstall') {
            $adb->pquery("UPDATE vtiger_field SET block=? AND typeofdata=? WHERE columnname=? and fieldname=?", array('0','P~O','user_password','user_password'));
            $adb->pquery("UPDATE vtiger_field SET block=? AND typeofdata=? WHERE columnname=? and fieldname=?", array('0','P~O','confirm_password','confirm_password'));           
            
            /** if User login Settings filed not updated from manifest executive */
            $q = "ALTER TABLE  vtiger_users ADD  `login_starts_at` VARCHAR( 30 ) NULL AFTER is_owner "; 
            $adb->pquery($q,array());

            $q = "ALTER TABLE  vtiger_users ADD  `login_ends_at` VARCHAR( 30 ) NULL AFTER  is_owner ";
            $adb->pquery($q,array());

            $q = "ALTER TABLE  vtiger_users ADD  `allowed_ips` VARCHAR( 257 ) NULL AFTER  is_owner ";
            $adb->pquery($q,array());

            $q = "ALTER TABLE  vtiger_users  ADD  `login_dow` VARCHAR( 255 ) NULL AFTER   is_owner ";
            $adb->pquery($q,array());

            $q = "ALTER TABLE  vtiger_users ADD  `login_allow_holiday` VARCHAR( 50 ) NULL AFTER  is_owner ";
            $adb->pquery($q,array());
