 ...
 ********************************************************************************/
 // This is the proforma vtDZiner ModelModule.php script which will get converted as the written out as ModelModule.php
...
// vtiger_modelmodule is a custom vtiger entity generated by vtDZiner Module DZiner
class ModelModule extends CRMEntity {
	var $log;
	var $db;

	var $module_name="ModelModule";
	var $table_name = "vtiger_modelmodule";
	var $table_index= 'modelmoduleid';

	var $tab_name = Array('vtiger_crmentity','vtiger_modelmodule','vtiger_modelmodulecf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_modelmodule'=>'modelmoduleid','vtiger_modelmodulecf'=>'modelmoduleid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_modelmodulecf', 'modelmoduleid');
	...
	var $popup_fields=Array('modelmodulename');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'ModelModule'=>Array('modelmodule'=>'modelmodulename'),
	...
	var $list_link_field= 'modelmodulename';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'modelmodulename');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'modelmodulename';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_modelmodulegrouprelation','modelmoduleid');
	function ModelModule() {
		$this->log = LoggerManager::getLogger('modelmodule');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('ModelModule');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['MODELMODULE_SORT_ORDER'] != '')?($_SESSION['MODELMODULE_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'ModelModulename')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['MODELMODULE_ORDER_BY'] != '')?($_SESSION['MODELMODULE_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_modelmodule.modelmoduleid,  vtiger_modelmodule.modelmodulename, vtiger_modelmodule.dateclosed FROM vtiger_modelmodule, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_modelmodule.modelmoduleid, vtiger_modelmodule.modelmodulename, vtiger_crmentity.smcreatorid FROM vtiger_modelmodule inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_modelmodule.modelmoduleid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('ModelModule',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export ModelModule Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("ModelModule", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_modelmodule
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_modelmodule.modelmoduleid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_modelmodulecf on vtiger_modelmodulecf.modelmoduleid=vtiger_modelmodule.modelmoduleid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_modelmodule.modelmoduleid, vtiger_modelmodule.modelmodulename, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_modelmodule
					inner join vtiger_contmodelmodulerel on vtiger_contmodelmodulerel.modelmoduleid = vtiger_modelmodule.modelmoduleid
					inner join vtiger_contactdetails on vtiger_contmodelmodulerel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_modelmodule.modelmoduleid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_modelmodule on vtiger_modelmodule.modelmoduleid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'ModelModule'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_modelmodule ON vtiger_modelmodule.modelmoduleid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_modelmodule.modelmoduleid = $id";
	...
	/**
	* Function to get ModelModule related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('ModelModule',$query,$id);
	}


	  /**
	  * Function to get ModelModule related Quotes
	  * @param  integer   $id  - modelmoduleid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'modelmodule_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_modelmodule.modelmodulename from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_modelmodule on vtiger_modelmodule.modelmoduleid=vtiger_quotes.modelmoduleid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contmodelmodulerel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contmodelmodulerel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contmodelmodulerel"=>"modelmoduleid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"modelmoduleid","vtiger_salesorder"=>"modelmoduleid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_modelmodule","modelmoduleid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityModelModule on vtiger_crmentityModelModule.crmid=vtiger_modelmodule.modelmoduleid and vtiger_crmentityModelModule.deleted=0
		left join vtiger_modelmodulecf on vtiger_modelmodulecf.modelmoduleid = vtiger_modelmodule.modelmoduleid
		left join vtiger_groups vtiger_groupsModelModule on vtiger_groupsModelModule.groupid = vtiger_crmentityModelModule.smownerid
		left join vtiger_users as vtiger_usersModelModule on vtiger_usersModelModule.id = vtiger_crmentityModelModule.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_modelmodule"=>"modelmoduleid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_modelmodule"=>"modelmoduleid"),
			"Quotes" => array("vtiger_quotes"=>array("modelmoduleid","quoteid"),"vtiger_modelmodule"=>"modelmoduleid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("modelmoduleid","salesorderid"),"vtiger_modelmodule"=>"modelmoduleid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_modelmodule"=>"modelmoduleid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-ModelModule Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contmodelmodulerel WHERE modelmoduleid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
