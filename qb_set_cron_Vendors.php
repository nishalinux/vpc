...
try {
		...
		$query = "SELECT vv.vendorid FROM vtiger_vendorcf vv left join vtiger_crmentity vcm on vv.vendorid = vcm.crmid where vcm.deleted = 0 and vv.quickbook_id ='' ";
				...
				if(isset($Context)){ 
					...
					if($quickbook_id != ''){
						...
						if ($resp = $VendorService->add($Context, $realm, $Vendor))
						{
							...
							$sq = "UPDATE `vtiger_vendorcf` SET `quickbook_id`= $qb_id  WHERE  vendorid = $vtiger_id ";
							$adb->query($sq); 
							$qb_create++;
						}
						else
