...
submit : function(){
	...
	unavailableUsers: function(){
		...
		if(nousers !=0 || nousers != ''){
			//bootbox.alert("Please delete users are not available for meeting at that specific time." );
			//return false;
		}
	},
	...
	checkAssignedUser: function(){
		...
		AppConnector.request(params).then(function(data){
			...
			if(res != 0 && res != ''){
				...
				bootbox.alert("Assigned To scheduled event that time.Please change time.");
				return false;
			}else{
