...
class SampleAnalysis_Detail_View extends Vtiger_Detail_View {
        ...
        public function showModuleDetailView(Vtiger_Request $request) {
			...
			$viewer->assign('KITDETAILS', $this->kitwidget($recordId));
	...
	public function kitwidget($id){
		global $adb;
		$q = "select genotype_kit_id as kid,confirmation_id as cid from vtiger_sampleanalysiscf where sampleanalysisid = ?";
		$result = $adb->pquery($q,array($id));
		$result = $adb->fetch_row($result);
		
		$kid = $result['kid'];
		$cid = $result['cid']; 
		$final = array();
		if($cid != ''){ $final['ckymotype'] = true; }else{ $final['ckymotype'] = false; }
		$final['kid'] = $kid;
		return $final;
