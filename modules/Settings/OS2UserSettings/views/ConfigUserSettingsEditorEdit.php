...
class Settings_OS2UserSettings_ConfigUserSettingsEditorEdit_View extends Settings_Vtiger_Index_View {
	...
	public function process(Vtiger_Request $request) {
		...
		$viewer->assign('USERFIELDS', $moduleModel->getEditableFields());
		$viewer->assign('QUALIFIED_MODULE', $qualifiedName);
		$viewer->assign('CURRENT_USER_MODEL', Users_Record_Model::getCurrentUserModel());
		$viewer->view('ConfigUserSettingsEditorEdit.tpl', $qualifiedName);
	}
	
		
	function getPageTitle(Vtiger_Request $request) {
		$qualifiedModuleName = $request->getModule(false);
		return vtranslate('User Settings Editor',$qualifiedModuleName);
	}
