 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Room.php script which will get converted as the written out as Room.php
...
// vtiger_room is a custom vtiger entity generated by vtDZiner Module DZiner
class Room extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Room";
	var $table_name = "vtiger_room";
	var $table_index= 'roomid';

	var $tab_name = Array('vtiger_crmentity','vtiger_room','vtiger_roomcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_room'=>'roomid','vtiger_roomcf'=>'roomid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_roomcf', 'roomid');
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'roomname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'roomname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_roomgrouprelation','roomid');
	function Room() {
		$this->log = LoggerManager::getLogger('room');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Room');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['ROOM_SORT_ORDER'] != '')?($_SESSION['ROOM_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Roomname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['ROOM_ORDER_BY'] != '')?($_SESSION['ROOM_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_room.roomid,  vtiger_room.roomname, vtiger_room.dateclosed FROM vtiger_room, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_room.roomid, vtiger_room.roomname, vtiger_crmentity.smcreatorid FROM vtiger_room inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_room.roomid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Room',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Room", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_room
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_room.roomid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_roomcf on vtiger_roomcf.roomid=vtiger_room.roomid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_room.roomid, vtiger_room.roomname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_room
					inner join vtiger_controomrel on vtiger_controomrel.roomid = vtiger_room.roomid
					inner join vtiger_contactdetails on vtiger_controomrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_room.roomid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_room on vtiger_room.roomid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Room'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_room ON vtiger_room.roomid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_room.roomid = $id";
	...
	/**
	* Function to get Room related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Room',$query,$id);
	}


	  /**
	  * Function to get Room related Quotes
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'room_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_room.roomname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_room on vtiger_room.roomid=vtiger_quotes.roomid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_controomrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_controomrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_controomrel"=>"roomid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"roomid","vtiger_salesorder"=>"roomid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_room","roomid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityRoom on vtiger_crmentityRoom.crmid=vtiger_room.roomid and vtiger_crmentityRoom.deleted=0
		left join vtiger_roomcf on vtiger_roomcf.roomid = vtiger_room.roomid
		left join vtiger_groups vtiger_groupsRoom on vtiger_groupsRoom.groupid = vtiger_crmentityRoom.smownerid
		left join vtiger_users as vtiger_usersRoom on vtiger_usersRoom.id = vtiger_crmentityRoom.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_room"=>"roomid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_room"=>"roomid"),
			"Quotes" => array("vtiger_quotes"=>array("roomid","quoteid"),"vtiger_room"=>"roomid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("roomid","salesorderid"),"vtiger_room"=>"roomid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_room"=>"roomid"),
		);
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_controomrel WHERE roomid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
