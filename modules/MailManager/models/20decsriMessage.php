...
class MailManager_Message_Model extends Vtiger_MailRecord  {
	...
	protected function loadAttachmentsFromDB($withContent, $aName=false) {
		...
		if (empty($this->_attachments)) {
			...
			$params = array($currentUserModel->getId(), $this->muid(), $this->vtfolder);
			...
			$atResult = $db->pquery("SELECT {$filteredColumns} FROM vtiger_mailmanager_mailattachments
						WHERE userid=? AND muid=? and mfolder=? $whereClause", $params);
	...
	protected function saveToDB($uid, $folder = '') {
		...
		if (!empty($this->_attachments)) {
			foreach($this->_attachments as $index => $aValue) {
				...
				if(is_array($attachInfo) && !empty($attachInfo)) {
					...
					(userid, muid, attachid, aname, path, lastsavedtime, mfolder) VALUES (?, ?, ?, ?, ?, ?, ?)",
					array($currentUserModel->getId(), $uid, $attachInfo['attachid'], $attachInfo['name'], $attachInfo['path'], $savedtime, $folder));
		...
		foreach($this->_inline_attachments as $index => $info) {
			...
			if(is_array($attachInfo) && !empty($attachInfo)) {
				$db->pquery("INSERT INTO vtiger_mailmanager_mailattachments
				(userid, muid, attachid, aname, path, lastsavedtime, cid, mfolder) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
				array($currentUserModel->getId(), $uid, $attachInfo['attachid'], @self::__mime_decode($attachInfo['name']), $attachInfo['path'], $savedtime, $info['cid'], $folder));
	...
	public function __SaveAttachmentFile($filename, $filecontent) {
		...
		//fclose($fh);
		  file_put_contents($saveasfile,$filecontent);
