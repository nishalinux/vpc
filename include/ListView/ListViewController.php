...
//							$value = CurrencyField::appendCurrencySymbol($currencyValue, $currencySymbol);
					...
					$value = "<span align='right'>".textlength_check($value)."</div>";
				       ...
				       $sqlproducts = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'Products Image' AND vtiger_seattachmentsrel.crmid = ? Limit 0,1";
                                $resultprod = $this->db->pquery($sqlproducts, array($recordId));
                                $productimg = $this->db->fetch_row($resultprod);
                                $productimname = $productimg['path'].$productimg['attachmentsid'].'_'.$productimg['name'];
                                    $value = '<img src="'.$productimname.'" width="120" height="120">';
				}elseif($field->getUIType() == 545) {
					$value = $this->getGalleryDetailsList($recordId,$fieldName);
				} else {
					$value = textlength_check($value);
				}
...
//				// END
	...
	public function getGalleryDetailsList($recordId,$fieldName){
		$db = PearDatabase::getInstance();
		...
		if ($recordId) {
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'sample Attachment' AND vtiger_seattachmentsrel.crmid = ? and subject=? ORDER BY subject";
		}
		$result = $db->pquery($sql, array($recordId,$fieldName));
		$count = $db->num_rows($result);
		for($i=0; $i<$count; $i++) {
				$idslist = $db->query_result($result, $i, 'attachmentsid');
				$imgpath = $db->query_result($result, $i, 'path');
				$galleryName = $db->query_result($result, $i, 'name');
				$name = decode_html($galleryName);
		}
		$path = $imgpath.$idslist."_".$name;
		if($count != 0){
			return '<img src="'.$path.'" target="_blank" width="150" height="80" />';
		}else{
			return '';
		}
	}
