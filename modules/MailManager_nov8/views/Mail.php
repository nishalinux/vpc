...
class MailManager_Mail_View extends MailManager_Abstract_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if ('open' == $this->getOperationArg($request)) {
			...
			//if($request->get('loction')=='mopen' || $request->get('loction')=='fopen'){
			...
			//$userid=$current_user->id; //Added by madhav
			$query = 'select mail_username from vtiger_mail_accounts where user_id ='.$userid; //added by madhav
			$res = $db->pquery($query, array()); //added by madhav
			$pick_val = $db->query_result($res, 0, 'mail_username'); //added by madhav
			$viewer->assign('PCL', $pick_val);
			//$viewer->assign('user_id', $db->getRow($quary));  //added by madhav
