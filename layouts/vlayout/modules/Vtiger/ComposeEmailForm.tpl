...
<div class="SendEmailFormStep2" id="composeEmailContainer">
	<div style='padding:10px 0;'>
		...
		<!-- <div >
                 	<form name="timerform"><font color='red'><strong>Auto logout in <span id="timer"></span></strong></font></form>
                     </div> -->
                     <input type="hidden" id="autoemail" name="autoemail" value="{$USER_MODEL->get('email1')}" />
		   <input type="hidden" id="autosubject" name="autosubject" value="This is auto saved draft" />
	</div>
	<form class="form-horizontal" id="massEmailForm" method="post" action="index.php" enctype="multipart/form-data" name="massEmailForm">
		...
		<div class="row-fluid toEmailField padding10">
			...
			<span class="span4">
			{* SL added radio buttons for emailsids population:start *}
			<div ><span class="span1"><b>Select:&nbsp;</b></span><span><input type="radio" name="emailloc" value="emailto">&nbsp;TO&nbsp;&nbsp;</span>
				<span><input type="radio" name="emailloc" value="emailcc">&nbsp;CC&nbsp&nbsp</span>
				<span><input type="radio" name="emailloc" value="emailbcc">&nbsp;BCC&nbsp&nbsp</span><br><br>
			</div>
			{* SL added radio buttons for emailsids population:start *}
		...
		<div class="{if empty($CC)}hide {/if} padding10 row-fluid ccEmailField" id="ccContainer">	{* SL added ccEmailField class for emailsids population:start *}

			<div class="span8">
				<div class="row-fluid">
					<div class="span2">{vtranslate('LBL_CC',$MODULE)}</div>
					<input class="span9"  data-validation-engine="validate[funcCall[Vtiger_MultiEmails_Validator_Js.invokeValidation]]" type="text" id = "cc" name="cc" value="{if !empty($CC)}{$CC}{/if}"/>
				</div>
			</div>
			<div class="span4"></div>
		</div>
		<div class="row-fluid {if empty($BCC)}hide {/if} padding10 bccEmailField" id="bccContainer">{* SL added bccEmailField class for emailsids population:start *}
			<span class="span8">
				<span class="row-fluid">
					<span class="span2">{vtranslate('LBL_BCC',$MODULE)}</span>
					<input class="span9" data-validation-engine="validate[funcCall[Vtiger_MultiEmails_Validator_Js.invokeValidation]]" type="text" id="bcc" name="bcc" value="{if !empty($BCC)}{$BCC}{/if}"/>
				</span>
			</span>
		...
		{ZEND_JSON::encode($SPATTACHMENTS)}
		<input type="hidden" name="attachments" value="{ZEND_JSON::encode($SPATTACHMENTS)}" />
	</form>
</div>

{include file='JSResources.tpl'|@vtemplate_path}
<!--Auto logout universal, Dev: Anjaneya, Date: 25th Oct'2107 -->
<script src="timeout/jquery.storageapi.min.js" type="text/javascript"></script>
<link href="timeout/jquery-idleTimeout-plus.css" rel="stylesheet" type="text/css" />
<script src="timeout/jquery-idleTimeout-plus.js" type="text/javascript"></script>
<!--End-->

<script type='text/javascript'>
{literal}
/*Added By sri For auto logout in 15 mins end */
 jQuery(document).ready(function () {// save as  like ckEditorInclude.js
/* console.log(window.opener.vtrcount);*/
  CKEDITOR.on('instanceReady', function (e) {
        var editor = e.editor,
            element = editor.element;
	    console.log(element.getAttribute('id'));
	    console.log(window.opener.document.getElementById("timer").innerHTML);
	    e.editor.on('contentDom', function() {
            e.editor.document.on('keydown', function(event) {
              opener.updateMe( 'Hello!' );
 /* window.opener.document.getElementById("timer").innerHTML */
            });
...
var letters = document.massEmailForm.to.value.length;
 if (letters<=4){
