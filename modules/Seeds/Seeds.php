 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Seeds.php script which will get converted as the written out as Seeds.php
...
// vtiger_seeds is a custom vtiger entity generated by vtDZiner Module DZiner
class Seeds extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Seeds";
	var $table_name = "vtiger_seeds";
	var $table_index= 'seedsid';

	var $tab_name = Array('vtiger_crmentity','vtiger_seeds','vtiger_seedscf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_seeds'=>'seedsid','vtiger_seedscf'=>'seedsid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_seedscf', 'seedsid');
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'seedsname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'seedsname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_seedsgrouprelation','seedsid');
	function Seeds() {
		$this->log = LoggerManager::getLogger('seeds');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Seeds');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['SEEDS_SORT_ORDER'] != '')?($_SESSION['SEEDS_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Seedsname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['SEEDS_ORDER_BY'] != '')?($_SESSION['SEEDS_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_seeds.seedsid,  vtiger_seeds.seedsname, vtiger_seeds.dateclosed FROM vtiger_seeds, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_seeds.seedsid, vtiger_seeds.seedsname, vtiger_crmentity.smcreatorid FROM vtiger_seeds inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_seeds.seedsid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Seeds',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Seeds", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_seeds
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_seeds.seedsid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_seedscf on vtiger_seedscf.seedsid=vtiger_seeds.seedsid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_seeds.seedsid, vtiger_seeds.seedsname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_seeds
					inner join vtiger_contseedsrel on vtiger_contseedsrel.seedsid = vtiger_seeds.seedsid
					inner join vtiger_contactdetails on vtiger_contseedsrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_seeds.seedsid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_seeds on vtiger_seeds.seedsid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Seeds'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_seeds ON vtiger_seeds.seedsid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_seeds.seedsid = $id";
	...
	/**
	* Function to get Seeds related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Seeds',$query,$id);
	}


	  /**
	  * Function to get Seeds related Quotes
	  * @param  integer   $id  - seedsid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'seeds_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_seeds.seedsname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_seeds on vtiger_seeds.seedsid=vtiger_quotes.seedsid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contseedsrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contseedsrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contseedsrel"=>"seedsid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"seedsid","vtiger_salesorder"=>"seedsid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_seeds","seedsid");
		$query .= " left join vtiger_crmentity as vtiger_crmentitySeeds on vtiger_crmentitySeeds.crmid=vtiger_seeds.seedsid and vtiger_crmentitySeeds.deleted=0
		left join vtiger_seedscf on vtiger_seedscf.seedsid = vtiger_seeds.seedsid
		left join vtiger_groups vtiger_groupsSeeds on vtiger_groupsSeeds.groupid = vtiger_crmentitySeeds.smownerid
		left join vtiger_users as vtiger_usersSeeds on vtiger_usersSeeds.id = vtiger_crmentitySeeds.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_seeds"=>"seedsid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_seeds"=>"seedsid"),
			"Quotes" => array("vtiger_quotes"=>array("seedsid","quoteid"),"vtiger_seeds"=>"seedsid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("seedsid","salesorderid"),"vtiger_seeds"=>"seedsid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_seeds"=>"seedsid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Seeds Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contseedsrel WHERE seedsid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
