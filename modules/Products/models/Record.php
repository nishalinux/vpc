	...
	public function getBarcodeDetails() {
		...
		if ($recordId) {
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'Products Attachment' AND vtiger_seattachmentsrel.crmid = ?";
...
//added by sl for barcode start
	public function deleteBarcode($imageId) {
		$db = PearDatabase::getInstance();
	...
	//vtDZiner images Support purpose added :: Nov 24 2017
	...
	public function getGalleryDetails($fieldname) {
		...
		if ($recordId) {
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'ModelModule Attachment' AND vtiger_seattachmentsrel.crmid = ?  and subject=? ORDER BY subject";
	...
	public function getGalleryEditDetails() {
		...
		if ($recordId) {
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'ModelModule Attachment' AND vtiger_seattachmentsrel.crmid = ? ORDER BY subject";
