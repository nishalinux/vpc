 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Extract.php script which will get converted as the written out as Extract.php
...
// vtiger_extract is a custom vtiger entity generated by vtDZiner Module DZiner
class Extract extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Extract";
	var $table_name = "vtiger_extract";
	var $table_index= 'extractid';

	var $tab_name = Array('vtiger_crmentity','vtiger_extract','vtiger_extractcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_extract'=>'extractid','vtiger_extractcf'=>'extractid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_extractcf', 'extractid');
	...
	var $popup_fields=Array('extractname');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'Extract'=>Array('extract'=>'extractname'),
	...
	var $list_link_field= 'extractname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'extractname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'extractname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_extractgrouprelation','extractid');
	function Extract() {
		$this->log = LoggerManager::getLogger('extract');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Extract');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['EXTRACT_SORT_ORDER'] != '')?($_SESSION['EXTRACT_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Extractname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['EXTRACT_ORDER_BY'] != '')?($_SESSION['EXTRACT_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_extract.extractid,  vtiger_extract.extractname, vtiger_extract.dateclosed FROM vtiger_extract, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_extract.extractid, vtiger_extract.extractname, vtiger_crmentity.smcreatorid FROM vtiger_extract inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_extract.extractid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Extract',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Extract", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_extract
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_extract.extractid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_extractcf on vtiger_extractcf.extractid=vtiger_extract.extractid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_extract.extractid, vtiger_extract.extractname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_extract
					inner join vtiger_contextractrel on vtiger_contextractrel.extractid = vtiger_extract.extractid
					inner join vtiger_contactdetails on vtiger_contextractrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_extract.extractid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_extract on vtiger_extract.extractid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Extract'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_extract ON vtiger_extract.extractid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_extract.extractid = $id";
	...
	/**
	* Function to get Extract related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Extract',$query,$id);
	}


	  /**
	  * Function to get Extract related Quotes
	  * @param  integer   $id  - extractid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'extract_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_extract.extractname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_extract on vtiger_extract.extractid=vtiger_quotes.extractid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contextractrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contextractrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contextractrel"=>"extractid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"extractid","vtiger_salesorder"=>"extractid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_extract","extractid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityExtract on vtiger_crmentityExtract.crmid=vtiger_extract.extractid and vtiger_crmentityExtract.deleted=0
		left join vtiger_extractcf on vtiger_extractcf.extractid = vtiger_extract.extractid
		left join vtiger_groups vtiger_groupsExtract on vtiger_groupsExtract.groupid = vtiger_crmentityExtract.smownerid
		left join vtiger_users as vtiger_usersExtract on vtiger_usersExtract.id = vtiger_crmentityExtract.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_extract"=>"extractid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_extract"=>"extractid"),
			"Quotes" => array("vtiger_quotes"=>array("extractid","quoteid"),"vtiger_extract"=>"extractid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("extractid","salesorderid"),"vtiger_extract"=>"extractid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_extract"=>"extractid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Extract Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contextractrel WHERE extractid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
