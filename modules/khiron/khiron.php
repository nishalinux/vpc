 ...
 ********************************************************************************/
 // This is the proforma vtDZiner khiron.php script which will get converted as the written out as khiron.php
...
// vtiger_khiron is a custom vtiger entity generated by vtDZiner Module DZiner
class khiron extends CRMEntity {
	var $log;
	var $db;

	var $module_name="khiron";
	var $table_name = "vtiger_khiron";
	var $table_index= 'khironid';

	var $tab_name = Array('vtiger_crmentity','vtiger_khiron','vtiger_khironcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_khiron'=>'khironid','vtiger_khironcf'=>'khironid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_khironcf', 'khironid');
	...
	var $popup_fields=Array('khironname');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'khiron'=>Array('khiron'=>'khironname'),
	...
	var $list_link_field= 'khironname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'khironname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'khironname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_khirongrouprelation','khironid');
	function khiron() {
		$this->log = LoggerManager::getLogger('khiron');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('khiron');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['KHIRON_SORT_ORDER'] != '')?($_SESSION['KHIRON_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'khironname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['KHIRON_ORDER_BY'] != '')?($_SESSION['KHIRON_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_khiron.khironid,  vtiger_khiron.khironname, vtiger_khiron.dateclosed FROM vtiger_khiron, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_khiron.khironid, vtiger_khiron.khironname, vtiger_crmentity.smcreatorid FROM vtiger_khiron inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_khiron.khironid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('khiron',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("khiron", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_khiron
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_khiron.khironid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_khironcf on vtiger_khironcf.khironid=vtiger_khiron.khironid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_khiron.khironid, vtiger_khiron.khironname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_khiron
					inner join vtiger_contkhironrel on vtiger_contkhironrel.khironid = vtiger_khiron.khironid
					inner join vtiger_contactdetails on vtiger_contkhironrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_khiron.khironid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_khiron on vtiger_khiron.khironid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'khiron'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_khiron ON vtiger_khiron.khironid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_khiron.khironid = $id";
	...
	/**
	* Function to get khiron related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('khiron',$query,$id);
	}


	  /**
	  * Function to get khiron related Quotes
	  * @param  integer   $id  - khironid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'khiron_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_khiron.khironname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_khiron on vtiger_khiron.khironid=vtiger_quotes.khironid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contkhironrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contkhironrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contkhironrel"=>"khironid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"khironid","vtiger_salesorder"=>"khironid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_khiron","khironid");
		$query .= " left join vtiger_crmentity as vtiger_crmentitykhiron on vtiger_crmentitykhiron.crmid=vtiger_khiron.khironid and vtiger_crmentitykhiron.deleted=0
		left join vtiger_khironcf on vtiger_khironcf.khironid = vtiger_khiron.khironid
		left join vtiger_groups vtiger_groupskhiron on vtiger_groupskhiron.groupid = vtiger_crmentitykhiron.smownerid
		left join vtiger_users as vtiger_userskhiron on vtiger_userskhiron.id = vtiger_crmentitykhiron.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_khiron"=>"khironid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_khiron"=>"khironid"),
			"Quotes" => array("vtiger_quotes"=>array("khironid","quoteid"),"vtiger_khiron"=>"khironid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("khironid","salesorderid"),"vtiger_khiron"=>"khironid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_khiron"=>"khironid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-khiron Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contkhironrel WHERE khironid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
