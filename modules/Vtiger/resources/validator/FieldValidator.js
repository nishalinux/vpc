...
Vtiger_Base_Validator_Js("Vtiger_Email_Validator_Js",{
	...
	validateValue : function(fieldValue){
		...
		if (!emailFilter.test(jQuery.trim(fieldValue))) {//SL added jquery.trim to remove traailing spaces
...
//manasa currencey seperator validation
Vtiger_Base_Validator_Js('Vtiger_currencyseparator_Validator_Js',{
	invokeValidation : function(field, rules, i, options) {
			var validatorInstance = new Vtiger_MultiSelect_Validator_Js ();
			validatorInstance.setElement(field);
			var result = validatorInstance.validate();
	...
	validate: function(){
		var fieldInstance = this.getElement();
		var selectElementValue = fieldInstance.val();
		var DS = jQuery('select[name="currency_decimal_separator"]').val();
		var CG = jQuery('select[name="currency_grouping_separator"]').val();
		var fieldValue = this.getFieldValue();	
		console.log(DS+" ***** "+CG);
		if(DS == CG){
			var errorInfo = app.vtranslate('Decimal seperator and Grouping seperator cant be same.');
			this.setError(errorInfo);
			return false;
		}
		return true;
