 ...
 ********************************************************************************/
 // This is the proforma vtDZiner TestvtDziner.php script which will get converted as the written out as TestvtDziner.php
...
// vtiger_testvtdziner is a custom vtiger entity generated by vtDZiner Module DZiner
class TestvtDziner extends CRMEntity {
	var $log;
	var $db;

	var $module_name="TestvtDziner";
	var $table_name = "vtiger_testvtdziner";
	var $table_index= 'testvtdzinerid';

	var $tab_name = Array('vtiger_crmentity','vtiger_testvtdziner','vtiger_testvtdzinercf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_testvtdziner'=>'testvtdzinerid','vtiger_testvtdzinercf'=>'testvtdzinerid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_testvtdzinercf', 'testvtdzinerid');
	...
	var $list_link_field= 'testvtdzinername';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'testvtdzinername');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'testvtdzinername';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_testvtdzinergrouprelation','testvtdzinerid');
	function TestvtDziner() {
		$this->log = LoggerManager::getLogger('testvtdziner');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('TestvtDziner');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['TESTVTDZINER_SORT_ORDER'] != '')?($_SESSION['TESTVTDZINER_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'TestvtDzinername')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['TESTVTDZINER_ORDER_BY'] != '')?($_SESSION['TESTVTDZINER_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_testvtdziner.testvtdzinerid,  vtiger_testvtdziner.testvtdzinername, vtiger_testvtdziner.dateclosed FROM vtiger_testvtdziner, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_testvtdziner.testvtdzinerid, vtiger_testvtdziner.testvtdzinername, vtiger_crmentity.smcreatorid FROM vtiger_testvtdziner inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_testvtdziner.testvtdzinerid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('TestvtDziner',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export TestvtDziner Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("TestvtDziner", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_testvtdziner
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_testvtdziner.testvtdzinerid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_testvtdzinercf on vtiger_testvtdzinercf.testvtdzinerid=vtiger_testvtdziner.testvtdzinerid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_testvtdziner.testvtdzinerid, vtiger_testvtdziner.testvtdzinername, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_testvtdziner
					inner join vtiger_conttestvtdzinerrel on vtiger_conttestvtdzinerrel.testvtdzinerid = vtiger_testvtdziner.testvtdzinerid
					inner join vtiger_contactdetails on vtiger_conttestvtdzinerrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_testvtdziner.testvtdzinerid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_testvtdziner on vtiger_testvtdziner.testvtdzinerid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'TestvtDziner'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_testvtdziner ON vtiger_testvtdziner.testvtdzinerid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_testvtdziner.testvtdzinerid = $id";
	...
	/**
	* Function to get TestvtDziner related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('TestvtDziner',$query,$id);
	}


	  /**
	  * Function to get TestvtDziner related Quotes
	  * @param  integer   $id  - testvtdzinerid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'testvtdziner_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_testvtdziner.testvtdzinername from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_testvtdziner on vtiger_testvtdziner.testvtdzinerid=vtiger_quotes.testvtdzinerid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_conttestvtdzinerrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_conttestvtdzinerrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_conttestvtdzinerrel"=>"testvtdzinerid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"testvtdzinerid","vtiger_salesorder"=>"testvtdzinerid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_testvtdziner","testvtdzinerid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityTestvtDziner on vtiger_crmentityTestvtDziner.crmid=vtiger_testvtdziner.testvtdzinerid and vtiger_crmentityTestvtDziner.deleted=0
		left join vtiger_testvtdzinercf on vtiger_testvtdzinercf.testvtdzinerid = vtiger_testvtdziner.testvtdzinerid
		left join vtiger_groups vtiger_groupsTestvtDziner on vtiger_groupsTestvtDziner.groupid = vtiger_crmentityTestvtDziner.smownerid
		left join vtiger_users as vtiger_usersTestvtDziner on vtiger_usersTestvtDziner.id = vtiger_crmentityTestvtDziner.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_testvtdziner"=>"testvtdzinerid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_testvtdziner"=>"testvtdzinerid"),
			"Quotes" => array("vtiger_quotes"=>array("testvtdzinerid","quoteid"),"vtiger_testvtdziner"=>"testvtdzinerid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("testvtdzinerid","salesorderid"),"vtiger_testvtdziner"=>"testvtdzinerid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_testvtdziner"=>"testvtdzinerid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-TestvtDziner Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_conttestvtdzinerrel WHERE testvtdzinerid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
