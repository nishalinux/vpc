  ...
  <body style="background-color:#fafafb;">
                          ...
                          $mdata = json_decode($processflow_data['materials'],true); $i=0;
                            foreach($mdata as $m){ ?>
                              <tr id='tr_<?php echo $i;?>'>
                                ...
                                <td><select class="form-control input-sm chosen" id="idMaterials" name="material[<?php echo $i;?>][materialid]" required>
                                    <option value="">-Select-</option>                  
                                   <?php foreach($pc as $pcid=>$pcname){  ?>   
                                    <option value='<?php echo $pcid;?>' <?php if($pcid == $m['materialid']){ echo 'selected';}?> ><?php echo $pcname;?></option>
                                      <?php } ?>               
                                    </select>
                                </td>
    ...
    $(document).ready(function(){ 
     ...
     $("#idBtnAddMaterial").click(function () {
       ...
       html += '<td><select class="form-control input-sm chosen" id="idMaterials" name="material['+sid+'][materialid]" required><option value="">-Select-</option>';                    
                  <?php 
                  $m = explode(',',$processflow_data['materials']);
                    foreach($pc as $pcid=>$pcname){       ?>                
                     
                     html += "<option value='<?php echo $pcid;?>'><?php echo $pcname;?></option>";
                <?php } ?>               
                html += "</select></td>";
...
<script id="code">
  ...
  function savefinal(savemode){
    ...
    $.when(check_array_special_char_cleaner(JSON.parse(final_process_json))).then(function(x) { 
    if(x != 0 ){
      return false;
      }else{
