...
{strip}
	...
	<td>
		<input id="{$qty}" name="{$qty}" type="text" class="qty smallInputBox" {if $MODULE neq 'PickingSlip'}data-validation-engine="validate[required,funcCall[Vtiger_GreaterThanZero_Validator_Js.invokeValidation]]"{else} data-validation-engine="validate[required,funcCall[Vtiger_GreaterThanPickingSlipQty_Validator_Js.invokeValidation]]"{/if} value="{if !empty($data.$qty)}{$data.$qty}{else}1{/if}"/>
		{if $MODULE neq 'PurchaseOrder'}
		<br>
		<span class="stockAlert redColor {if $data.$qty <= $data.$qtyInStock}hide{/if}" >
			{*modified for Picking slip Module purpose*}
				{if $MODULE neq 'PickingSlip'}
				{vtranslate('LBL_STOCK_NOT_ENOUGH',$MODULE)}
				<br>
				{vtranslate('LBL_MAX_QTY_SELECT',$MODULE)}&nbsp;
				<span class="maxQuantity">{$data.$qtyInStock}</span>
				{else}
