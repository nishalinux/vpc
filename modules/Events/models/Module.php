...
class Events_Module_Model extends Calendar_Module_Model {
	...
	public function getListViewUrl() {
		return 'index.php?module=Calendar&mode=Events&view='.$this->getListViewName();
	}
   ...
   /**
	...
	public function saveRecord($recordModel) {
       ...
       echo  $selectUsers = str_replace("[",'',$_REQUEST['selectedusers']);
		$selectUsers = str_replace("]",'',$selectUsers);
		$selectUsers = str_replace('"','',$selectUsers);
		//$contactids = $recordModel->get('contact_id');
		$recordid = $recordModel->getId();
		$notification = $recordModel->get('sendnotification');
		
        if(!empty($selectUsers) && $selectUsers != 'false')
        {
            $invities = $selectUsers;//implode(';',$selectUsers);
		...
		if($notification == 'on')
        {
			$mail_contents = $recordModel->getInviteUserMailData();
			$activityMode = ($recordModel->getModuleName()=='Calendar') ? 'Task' : 'Events';
            sendInvitationContacts($recordid,$activityMode,$recordModel->get('subject'),$mail_contents);
		}
