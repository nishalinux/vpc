...
/*print_r($request);
		global $log;
		$log->debug('.................Ganesh.......');
		$log->debug('showSendSMSForm');
		$log->debug('.................Ganesh.......');*/
		$sourceModule = $request->getModule();/*Request from module */
//echo $sourceModule;
		$moduleName = 'SMSNotifier';
		$selectedIds = $this->getRecordsListFromRequest($request);/*User id*/
		$excludedIds = $request->get('excluded_ids');
		$cvId = $request->get('viewname');
		$user = Users_Record_Model::getCurrentUserModel();/*Current user(admin)*/
		$moduleModel = Vtiger_Module_Model::getInstance($sourceModule);
		$phoneFields = $moduleModel->getFieldsByType('phone');
		$nameFields = $moduleModel->getFieldsByLabel('name');
		...
		if(count($selectedIds) == 1){
			...
			$viewer->assign('SINGLE_NAME_RECORD', $selectedRecordModel);
