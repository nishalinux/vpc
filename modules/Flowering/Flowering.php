 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Flowering.php script which will get converted as the written out as Flowering.php
...
// vtiger_flowering is a custom vtiger entity generated by vtDZiner Module DZiner
class Flowering extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Flowering";
	var $table_name = "vtiger_flowering";
	var $table_index= 'floweringid';

	var $tab_name = Array('vtiger_crmentity','vtiger_flowering','vtiger_floweringcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_flowering'=>'floweringid','vtiger_floweringcf'=>'floweringid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_floweringcf', 'floweringid');
	...
	var $list_link_field= 'floweringname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'floweringname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'floweringname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_floweringgrouprelation','floweringid');
	function Flowering() {
		$this->log = LoggerManager::getLogger('flowering');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Flowering');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['FLOWERING_SORT_ORDER'] != '')?($_SESSION['FLOWERING_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Floweringname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['FLOWERING_ORDER_BY'] != '')?($_SESSION['FLOWERING_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_flowering.floweringid,  vtiger_flowering.floweringname, vtiger_flowering.dateclosed FROM vtiger_flowering, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_flowering.floweringid, vtiger_flowering.floweringname, vtiger_crmentity.smcreatorid FROM vtiger_flowering inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_flowering.floweringid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Flowering',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export Flowering Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Flowering", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_flowering
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_flowering.floweringid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_floweringcf on vtiger_floweringcf.floweringid=vtiger_flowering.floweringid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_flowering.floweringid, vtiger_flowering.floweringname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_flowering
					inner join vtiger_contfloweringrel on vtiger_contfloweringrel.floweringid = vtiger_flowering.floweringid
					inner join vtiger_contactdetails on vtiger_contfloweringrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_flowering.floweringid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_flowering on vtiger_flowering.floweringid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Flowering'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_flowering ON vtiger_flowering.floweringid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_flowering.floweringid = $id";
	...
	/**
	* Function to get Flowering related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Flowering',$query,$id);
	}


	  /**
	  * Function to get Flowering related Quotes
	  * @param  integer   $id  - floweringid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'flowering_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_flowering.floweringname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_flowering on vtiger_flowering.floweringid=vtiger_quotes.floweringid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contfloweringrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contfloweringrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contfloweringrel"=>"floweringid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"floweringid","vtiger_salesorder"=>"floweringid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_flowering","floweringid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityFlowering on vtiger_crmentityFlowering.crmid=vtiger_flowering.floweringid and vtiger_crmentityFlowering.deleted=0
		left join vtiger_floweringcf on vtiger_floweringcf.floweringid = vtiger_flowering.floweringid
		left join vtiger_groups vtiger_groupsFlowering on vtiger_groupsFlowering.groupid = vtiger_crmentityFlowering.smownerid
		left join vtiger_users as vtiger_usersFlowering on vtiger_usersFlowering.id = vtiger_crmentityFlowering.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_flowering"=>"floweringid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_flowering"=>"floweringid"),
			"Quotes" => array("vtiger_quotes"=>array("floweringid","quoteid"),"vtiger_flowering"=>"floweringid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("floweringid","salesorderid"),"vtiger_flowering"=>"floweringid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_flowering"=>"floweringid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Flowering Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contfloweringrel WHERE floweringid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
