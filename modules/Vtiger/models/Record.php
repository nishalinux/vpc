...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	...
	//manasa added aug 31 2016 for active crm user list
	public function getActiveAccessibleUsers(){
		$adb = PearDatabase::getInstance();
		$usersListArray = array();
		$query = 'SELECT user_name, first_name, last_name FROM vtiger_users where status="Active"';
		$result = $adb->pquery($query, array());
		while($row = $adb->fetchByAssoc($result)) {
			$usersListArray[$row['user_name']] = $row['first_name']." ".$row['last_name'];
		}
	...
	public function getJsignDetails_backup() {
		global $log;
		$db = PearDatabase::getInstance();
		$imageDetails = array();
		$recordId = $this->getId();
		...
		if ($recordId) {
			...
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'Signature Attachment' AND vtiger_seattachmentsrel.crmid = ?";
		 ...
		 $moduleName = $this->getModuleName();
			...
			$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_attachments.attachmentsid
						WHERE vtiger_crmentity.setype = 'Contacts Attachment' AND vtiger_seattachmentsrel.crmid = ?";
	...
	//added by jayant for signature end
	//added by jayant for signature start
