...
jQuery.Class("Emails_MassEdit_Js",{},{
	...
	showComposeEmailForm : function(params,cb,windowName){
		...
		//var ck_change = window.opener.jQuery('#ck_open_close').val('open');//murali
		//alert(ck_change);
		var timsp = jQuery('#timer').html();//murali
		var tt = timsp.split(" secs");	//murali
		var jt = tt.slice(0,1);//murali
	...
	registerSendEmailEvent : function(){
		...
		});window.opener.jQuery('#ck_open_close').val('close');//murali
	},
	...
	{
		var thisInstance = this;
		var timsp = window.opener.jQuery('#timer').html();//window.opener is to get value from parent window --murali
		var tt = timsp.split(" secs");	//murali
		var jt = tt.slice(0,1);//murali
		var em = jQuery('#autoemail').val();//murali
		var sb = jQuery('#autosubject').val();//murali
		
		
		if (jt <= 100)
		{
		//var newEm=jQuery('#emailField').val(em);//murali
		//var newSb=jQuery('#subject').val(sb);//murali
		var existemail = jQuery('#emailField').val();
		var existsubject = jQuery('#subject').val();
			if(existemail.length==0 && existsubject.length==0){
				var newEm=jQuery('#emailField').val(em);//murali
				var newSb=jQuery('#subject').val(sb);//murali
			}else if(existemail.length==0 && existsubject.length>0){
				var newEm=jQuery('#emailField').val(em);//murali
			}else if(existemail.length>0 && existsubject.length==0){
				var newSb=jQuery('#subject').val(sb);//murali
			}
		jQuery("#saveDraft").click();//murali
		jQuery('#flag').val(thisInstance.saved);//murali
		 //clearInterval(vtrcounter);
	    }
}
		/* setTimeout(function() {  //murali
			var thisInstance = this;
			var timsp = jQuery('#timer').html();//murali
			var tt = timsp.split(" secs");	//murali
			var jt = tt.slice(0,1);//murali
	...
	fileAfterSelectHandler : function(element, value, master_element){
		...
		if(totalAttachmentsSize > maxUploadSize){
			Vtiger_Helper_Js.showPnotify("Attachment size exceed max upload size. This mail will be saved into Draft automtically..!!");//murali
			...
			jQuery("#saveDraft").click();//murali
			//jQuery('#saveDraft').trigger('click');//murali
			jQuery('#flag').val(thisInstance.saved);//murali
	...
	registerEventsForToField : function(){
		...
		this.getMassEmailForm().on('click','.selectEmail',function(e){
			...
			var RadioSele =jQuery("input[name='emailloc']:checked").val();//sl added radio btns for emails start
 
			if(RadioSele == 'emailcc' ){
			var parentElem = jQuery(e.target).closest('.ccEmailField');
			}
			else if(RadioSele == 'emailbcc' ){
			var parentElem = jQuery(e.target).closest('.bccEmailField');
			}
			else {
			var parentElem = jQuery(e.target).closest('.toEmailField');
			}
			//sl added radio btns for emails start
			...
			popupInstance.show(params, function(data){
					...
					for(var id in responseData){
						var data = {
							...
							//sl added third param   for emails  
						thisInstance.setReferenceFieldValue(parentElem, data, RadioSele);
						if(RadioSele != 'emailcc' &&  RadioSele != 'emailbcc'){
	...
	setReferenceFieldValue : function(container,object,RadioSele){
		var thisInstance = this;
...
//sl added radio btns for emails start
		if(RadioSele == 'emailcc' ){
			ccEmailFieldExistingValue= jQuery("input[name='cc']").val();
			var ccEmailFieldNewValue;
			if(ccEmailFieldExistingValue != ""){
				ccEmailFieldNewValue = ccEmailFieldExistingValue+","+object.emailid;
			} else {
				ccEmailFieldNewValue = object.emailid;
			}
			 jQuery("input[name='cc']").val(ccEmailFieldNewValue);
		}
		else if(RadioSele == 'emailbcc' ){
			bccEmailFieldExistingValue= jQuery("input[name='bcc']").val();
			var bccEmailFieldNewValue;
			if(bccEmailFieldExistingValue != ""){
				bccEmailFieldNewValue = bccEmailFieldExistingValue+","+object.emailid;
			} else {
				bccEmailFieldNewValue = object.emailid;
			}
			 jQuery("input[name='bcc']").val(bccEmailFieldNewValue);
		}
		else{//sl added radio btns for emails start
