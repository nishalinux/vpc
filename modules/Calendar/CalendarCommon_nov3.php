...
AND vtiger_cntactivityrel.activityid =?";
	$ccresult = $adb->pquery($ccquery, array($_REQUEST["record"]));
	$ccrows = $adb->num_rows($ccresult);
	$ccmail1 = $adb->query_result($ccresult, 0, "email");
	$ccmail2 = $adb->query_result($ccresult, 1, "email");
	$em = array();
for($i=0; $i<$ccrows; $i++)
{
 $em[$i] = $adb->query_result($ccresult,$i,"email");
}

//exit(0);
	//murali--end
	//echo"<pre>";print_r($em);exit(0);
	...
	foreach($invitees_array as $inviteeid)
	{
		if($inviteeid != '')
		{
			...
			//send_mail('Calendar',$to_email,$current_user->user_name,'',$subject,$description,'',$em);//modified by murali to trigger mail to related contact 
			send_mail('Calendar',$to_email,$current_user->user_name,'',$subject,$description);
			if(!empty($em)){				
				foreach($em as $ccm){
				send_mail('Calendar',$ccm,$current_user->user_name,'',$subject,$description);
				array_shift($em);
					}
				}
		}
