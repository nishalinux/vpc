...
class OSSMail_Mail_Model extends Vtiger_Base_Model
{
	...
	{
		global $log;
		//$log->debug('start getMailCrmId');
		if ($this->mailCrmId != false) {
			//$log->debug('getMailCrmId124'.print_r($this->mailCrmId,true));
		...
		if ($db->getRowCount($result) > 0) {
			//$log->debug('getMailCrmId131');
			//$this->mailCrmId = $db->getSingleValue($result);
			$this->mailCrmId = $db->query_result($result, 0, "ossmailviewid");
		}
		//$log->debug('getMailCrmId return='.$this->mailCrmId);
		return $this->mailCrmId;
	}
	...
	{
		Global $log;
		//$log->debug('setMailcrmidddddddddddddddddddddddddddddddddddddddddd');
		$this->mailCrmId = $id;
	}
					...
					foreach ($emailsArray as $email) {
						...
						if ($cache !== false) {
							...
							//while (($crmid = $db->getSingleValue($result)) !== false) {
								$crmid = $db->query_result($result, 0, $table_index);
								$ids[] = $crmid;
							//}
