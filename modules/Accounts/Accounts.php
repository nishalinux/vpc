	...
	function get_emails($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
			...
			WHERE vtiger_seactivityrel.activityid = vtiger_activity.activityid
				...
				AND vtiger_crmentity.deleted = 0  order by date_start desc ,time_start desc ";
	...
	*/
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		if ($numOfFields > 0) {
			...
			$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>'vtiger_users.first_name','last_name' => 'vtiger_users.last_name'), 'Users');
			if($related_module == 'PBXManager'){//added by murali to display time zone values in related list of organizations
			$query = "SELECT vtiger_crmentity.*, $other->table_name.*,vtiger_pbxmanagercf.cf_1077";
		}else{
			...
			if($related_module == 'PBXManager'){//added by murali to display time zone values in related list of organizations
			$query .= " INNER JOIN vtiger_pbxmanagercf ON vtiger_pbxmanagercf.pbxmanagerid = vtiger_pbxmanager.pbxmanagerid";
			}
			$query .= $more_relation;
