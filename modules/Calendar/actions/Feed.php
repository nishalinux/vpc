...
class Calendar_Feed_Action extends Vtiger_BasicAjax_Action {
	...
	protected function pullEvents($start, $end, &$result, $userid = false,$color = null,$textColor = 'white') {
        ...
        $query.= " AND (vtiger_crmentity.smownerid IN (".  generateQuestionMarks($params).") or  vtiger_invitees.inviteeid=".$currentUser->getId().")";
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			//Manasa 19-July-2018 for invitees display in Calendar
            $moduleName = 'Events';
			$detailViewModel = Vtiger_DetailView_Model::getInstance($moduleName, $crmid);
			$recordModel = $detailViewModel->getRecord();
			$inviteeslist = $recordModel->getInvities();
			$inviteeslistname = array();
			foreach($inviteeslist as $userid){
				if(is_numeric($userid)){
					$inviteeslistname[] = getUserFullName($userid);
				}
			}
			$inviteeslistname[] = getUserFullName($recordModel->get('assigned_user_id'));
			$item['invitees'] = implode(",",$inviteeslistname);
			//Ended here
	...
	protected function pullTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			$item['textColor'] = $textColor;
            $item['module'] = $moduleModel->getName();
			$item['invitees'] = '';//Temp :: Failing in UI added on Jul-24-2018
			$result[] = $item;
		}
	}
