 ...
 ********************************************************************************/
 // This is the proforma vtDZiner EmployeesandTraining.php script which will get converted as the written out as EmployeesandTraining.php
...
// vtiger_employeesandtraining is a custom vtiger entity generated by vtDZiner Module DZiner
class EmployeesandTraining extends CRMEntity {
	var $log;
	var $db;

	var $module_name="EmployeesandTraining";
	var $table_name = "vtiger_employeesandtraining";
	var $table_index= 'employeesandtrainingid';

	var $tab_name = Array('vtiger_crmentity','vtiger_employeesandtraining','vtiger_employeesandtrainingcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_employeesandtraining'=>'employeesandtrainingid','vtiger_employeesandtrainingcf'=>'employeesandtrainingid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_employeesandtrainingcf', 'employeesandtrainingid');

	var $column_fields = Array();

	var $sortby_fields = Array(
			'employeesandtrainingname',
			'smownerid'
			);

	var $popup_fields=Array('employeesandtrainingname');

	// This is the list of vtiger_fields that are in the lists.
	// No fields have been written out by Module DZiner
	var $list_fields = Array(
			'EmployeesandTraining'=>Array('employeesandtraining'=>'employeesandtrainingname'),
			'Assigned To'=>Array('crmentity','smownerid')
			); 	// Fields selected as INCLUDE FIELDS in Module DZiner will be written out into this array

	var $list_fields_name = Array(
			'EmployeesandTraining'=>'employeesandtrainingname',
	...
	var $list_link_field= 'employeesandtrainingname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'employeesandtrainingname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'employeesandtrainingname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_employeesandtraininggrouprelation','employeesandtrainingid');
	function EmployeesandTraining() {
		$this->log = LoggerManager::getLogger('employeesandtraining');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('EmployeesandTraining');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['EMPLOYEESANDTRAINING_SORT_ORDER'] != '')?($_SESSION['EMPLOYEESANDTRAINING_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'EmployeesandTrainingname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['EMPLOYEESANDTRAINING_ORDER_BY'] != '')?($_SESSION['EMPLOYEESANDTRAINING_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		require('user_privileges/user_privileges_'.$current_user->id.'.php');
        	...
        	$tab_id = getTabid("EmployeesandTraining");
		...
		if($account_required)
		{
			$query = "SELECT vtiger_employeesandtraining.employeesandtrainingid,  vtiger_employeesandtraining.employeesandtrainingname, vtiger_employeesandtraining.dateclosed FROM vtiger_employeesandtraining, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_employeesandtraining.employeesandtrainingid, vtiger_employeesandtraining.employeesandtrainingname, vtiger_crmentity.smcreatorid FROM vtiger_employeesandtraining inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_employeesandtraining.employeesandtrainingid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('EmployeesandTraining',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export EmployeesandTraining Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("EmployeesandTraining", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_employeesandtraining
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_employeesandtraining.employeesandtrainingid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_employeesandtrainingcf on vtiger_employeesandtrainingcf.employeesandtrainingid=vtiger_employeesandtraining.employeesandtrainingid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_employeesandtraining.employeesandtrainingid, vtiger_employeesandtraining.employeesandtrainingname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_employeesandtraining
					inner join vtiger_contemployeesandtrainingrel on vtiger_contemployeesandtrainingrel.employeesandtrainingid = vtiger_employeesandtraining.employeesandtrainingid
					inner join vtiger_contactdetails on vtiger_contemployeesandtrainingrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_employeesandtraining.employeesandtrainingid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_employeesandtraining on vtiger_employeesandtraining.employeesandtrainingid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'EmployeesandTraining'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_employeesandtraining ON vtiger_employeesandtraining.employeesandtrainingid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_employeesandtraining.employeesandtrainingid = $id";
	...
	/**
	* Function to get EmployeesandTraining related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('EmployeesandTraining',$query,$id);
	}


	  /**
	  * Function to get EmployeesandTraining related Quotes
	  * @param  integer   $id  - employeesandtrainingid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'employeesandtraining_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_employeesandtraining.employeesandtrainingname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_employeesandtraining on vtiger_employeesandtraining.employeesandtrainingid=vtiger_quotes.employeesandtrainingid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contemployeesandtrainingrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contemployeesandtrainingrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contemployeesandtrainingrel"=>"employeesandtrainingid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"employeesandtrainingid","vtiger_salesorder"=>"employeesandtrainingid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_employeesandtraining","employeesandtrainingid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityEmployeesandTraining on vtiger_crmentityEmployeesandTraining.crmid=vtiger_employeesandtraining.employeesandtrainingid and vtiger_crmentityEmployeesandTraining.deleted=0
		left join vtiger_employeesandtrainingcf on vtiger_employeesandtrainingcf.employeesandtrainingid = vtiger_employeesandtraining.employeesandtrainingid
		left join vtiger_groups vtiger_groupsEmployeesandTraining on vtiger_groupsEmployeesandTraining.groupid = vtiger_crmentityEmployeesandTraining.smownerid
		left join vtiger_users as vtiger_usersEmployeesandTraining on vtiger_usersEmployeesandTraining.id = vtiger_crmentityEmployeesandTraining.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_employeesandtraining"=>"employeesandtrainingid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_employeesandtraining"=>"employeesandtrainingid"),
			"Quotes" => array("vtiger_quotes"=>array("employeesandtrainingid","quoteid"),"vtiger_employeesandtraining"=>"employeesandtrainingid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("employeesandtrainingid","salesorderid"),"vtiger_employeesandtraining"=>"employeesandtrainingid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_employeesandtraining"=>"employeesandtrainingid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-EmployeesandTraining Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contemployeesandtrainingrel WHERE employeesandtrainingid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
