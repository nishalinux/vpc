    ...
    }else if(type === "file")
    {
      ...
      for(var i = 0; i < inputs.length; i++) {
          if(inputs[i].type.toLowerCase() == 'file') {
              var path = inputs[i].value;
          }
      }
      if(path != "")
      {
        var mode="upload";
        var savetype = "uploadfile";
/*
        var object = new ActiveXObject("Scripting.FileSystemObject");
        var file = object.GetFile(path);
        file.Move("/var/www/html/pandb/modules/Settings/ProcessFlow/gojs/storage/images");
        ...
        $.ajax({        
            ...
            success : function(data) {
              if(data != 0)
              {
                ...
                inputs.setAttribute("value", data);
