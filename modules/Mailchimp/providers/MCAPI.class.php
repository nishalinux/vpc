...
class MCAPI {
    ...
    */
    ...
    function callServer($method, $params) {
        ...
        if(!$sock) {
            $this->errorMessage = $mod_string['NO_CONNECT']." $errno: $errstr)";
            $this->errorCode = "-99";
            ob_end_clean();
            return false;
        }
        ...
        ob_end_clean();
        if ($info["timed_out"]) {
            $this->errorMessage = $mod_strings['RESPONSE_TIME_OUT'];
            $this->errorCode = -98;
            return false;
        }
        ...
        if($response && $serial === false) {
        	$response = array("error" => $mod_strings['BAD_RESPONSE'] . $response, "code" => "-99");
        } else {
        	$response = $serial;
        }
        if($errored && is_array($response) && isset($response["error"])) {
            ...
            $this->errorMessage = $mod_strings['NO_ERROR_FOUND'];
