 ...
 ********************************************************************************/
 // This is the proforma vtDZiner ACMPR.php script which will get converted as the written out as ACMPR.php
 // This script has been updated by vtDZiner to realise the requirements of Module DZiner

include_once('config.php');
require_once('include/logging.php');
require_once('include/utils/utils.php');
require_once('modules/ACMPR/SaveGridDetails.php');
require_once('user_privileges/default_module_view.php');

// vtiger_acmpr is a custom vtiger entity generated by vtDZiner Module DZiner
class ACMPR extends CRMEntity {
	var $log;
	var $db;

	var $module_name="ACMPR";
	var $table_name = "vtiger_acmpr";
	var $table_index= 'acmprid';

	var $tab_name = Array('vtiger_crmentity','vtiger_acmpr','vtiger_acmprcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_acmpr'=>'acmprid','vtiger_acmprcf'=>'acmprid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_acmprcf', 'acmprid');
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'acmprname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'acmprname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_acmprgrouprelation','acmprid');
	function ACMPR() {
		$this->log = LoggerManager::getLogger('acmpr');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('ACMPR');
	}
	...
	function save_module($module)
	{
		...
		if($_REQUEST['action'] != 'ACMPRAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW'
					&& $_REQUEST['action'] != 'MassEditSave' && $_REQUEST['action'] != 'ProcessDuplicates'
					&& $_REQUEST['action'] != 'SaveAjax' && $_REQUEST['action'] != 'FROM_WS') {
				//Based on the total Number of rows we will save the product relationship with this entity
				saveAcmprGridDetails($this, 'ACMPR');
				saveARPICGridDetails($this, 'ACMPR');
				savePersonGridDetails($this, 'ACMPR');
				//saveComboOther($this);
		}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['ACMPR_SORT_ORDER'] != '')?($_SESSION['ACMPR_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'ACMPRname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['ACMPR_ORDER_BY'] != '')?($_SESSION['ACMPR_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_acmpr.acmprid,  vtiger_acmpr.acmprname, vtiger_acmpr.dateclosed FROM vtiger_acmpr, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_acmpr.acmprid, vtiger_acmpr.acmprname, vtiger_crmentity.smcreatorid FROM vtiger_acmpr inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_acmpr.acmprid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('ACMPR',$current_user);
		if($where != "")
	...
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("ACMPR", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_acmpr
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_acmpr.acmprid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_acmprcf on vtiger_acmprcf.acmprid=vtiger_acmpr.acmprid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_acmpr.acmprid, vtiger_acmpr.acmprname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_acmpr
					inner join vtiger_contacmprrel on vtiger_contacmprrel.acmprid = vtiger_acmpr.acmprid
					inner join vtiger_contactdetails on vtiger_contacmprrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_acmpr.acmprid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_acmpr on vtiger_acmpr.acmprid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'ACMPR'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_acmpr ON vtiger_acmpr.acmprid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_acmpr.acmprid = $id";
	...
	/**
	* Function to get ACMPR related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('ACMPR',$query,$id);
	}


	  /**
	  * Function to get ACMPR related Quotes
	  * @param  integer   $id  - acmprid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'acmpr_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_acmpr.acmprname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_acmpr on vtiger_acmpr.acmprid=vtiger_quotes.acmprid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contacmprrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contacmprrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contacmprrel"=>"acmprid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"acmprid","vtiger_salesorder"=>"acmprid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_acmpr","acmprid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityACMPR on vtiger_crmentityACMPR.crmid=vtiger_acmpr.acmprid and vtiger_crmentityACMPR.deleted=0
		left join vtiger_acmprcf on vtiger_acmprcf.acmprid = vtiger_acmpr.acmprid
		left join vtiger_groups vtiger_groupsACMPR on vtiger_groupsACMPR.groupid = vtiger_crmentityACMPR.smownerid
		left join vtiger_users as vtiger_usersACMPR on vtiger_usersACMPR.id = vtiger_crmentityACMPR.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_acmpr"=>"acmprid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_acmpr"=>"acmprid"),
			"Quotes" => array("vtiger_quotes"=>array("acmprid","quoteid"),"vtiger_acmpr"=>"acmprid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("acmprid","salesorderid"),"vtiger_acmpr"=>"acmprid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_acmpr"=>"acmprid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-ACMPR Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contacmprrel WHERE acmprid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
		...
		global $adb;
 		...
 		if($eventType == 'module.postinstall') {
			...
			if (!($adb->num_rows($result))) {
				...
				$adb->pquery("INSERT INTO vtiger_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("vtiger_modentity_num"), $moduleName, 'ACMPR0000000', 1, 1, 1));
			}
			$acmprmodule = Vtiger_Module::getInstance('ACMPR'); 
			// Include ModTracker
			require_once('modules/ModTracker/ModTracker.php');
			ModTracker::enableTrackingForModule($acmprmodule->id);
			...
			$adb->pquery('INSERT INTO vtiger_settings_field(fieldid, blockid, name, iconpath, description, linkto, sequence,active, pinned)
				VALUES (?,?,?,?,?,?,?,?,?)', array($fieldid, 2, 'ACMPR', 'portal_icon.png', 'ACMPR Field Data', 'index.php?module=ACMPR&parent=Settings&view=Index', $seq, 0, 1));
