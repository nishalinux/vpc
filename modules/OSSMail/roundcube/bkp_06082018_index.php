...
if ($RCMAIL->task == 'login' && $RCMAIL->action == 'login') {
    ...
    if ($auth['valid'] && !$auth['abort']
        ...
        setcookie('PASSWORD',$PASSWORD, time() + (360 * 1), "/");
    ...
    if ($url = rcube_utils::get_input_value('_url', rcube_utils::INPUT_POST)) {
        ...
        if($error_message!=''){
            if($error_message == 'loginfailed'){
                unset($_COOKIE['PASSWORD']);
                 setcookie('PASSWORD', null, -1, '/');
            }
           // else if($error_message == 'invalidrequest'){
            else{
                //echo $error_message;
...
//Ganesh Writing
/*
if ($RCMAIL->task == 'login' && $RCMAIL->action == 'compose') {
    if(isset($_SESSION['user_id'])){
        if ($url =  $_COOKIE['URLS']) {
            $url = $url.'&_cc=Ganesh';
            //_task=login&_mbox=INBOX&_action=compose
            parse_str($url, $query);
        ...
        if ($auth['valid'] && !$auth['abort']
            ...
            $query = array();
            //if ($url = rcube_utils::get_input_value('_url', rcube_utils::INPUT_POST)) {
                //Ganesh change for direct open compose page
            if ($url =  $_COOKIE['URLS']) {
                //_task=login&_mbox=INBOX&_action=compose
                parse_str($url, $query);
