 ...
 ********************************************************************************/
 // This is the proforma vtDZiner NeuroFeedback.php script which will get converted as the written out as NeuroFeedback.php
...
// vtiger_neurofeedback is a custom vtiger entity generated by vtDZiner Module DZiner
class NeuroFeedback extends CRMEntity {
	var $log;
	var $db;

	var $module_name="NeuroFeedback";
	var $table_name = "vtiger_neurofeedback";
	var $table_index= 'neurofeedbackid';

	var $tab_name = Array('vtiger_crmentity','vtiger_neurofeedback','vtiger_neurofeedbackcf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_neurofeedback'=>'neurofeedbackid','vtiger_neurofeedbackcf'=>'neurofeedbackid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_neurofeedbackcf', 'neurofeedbackid');
	...
	var $list_link_field= 'neurofeedbackname';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'neurofeedbackname');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'neurofeedbackname';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_neurofeedbackgrouprelation','neurofeedbackid');
	function NeuroFeedback() {
		$this->log = LoggerManager::getLogger('neurofeedback');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('NeuroFeedback');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['NEUROFEEDBACK_SORT_ORDER'] != '')?($_SESSION['NEUROFEEDBACK_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'NeuroFeedbackname')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['NEUROFEEDBACK_ORDER_BY'] != '')?($_SESSION['NEUROFEEDBACK_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_neurofeedback.neurofeedbackid,  vtiger_neurofeedback.neurofeedbackname, vtiger_neurofeedback.dateclosed FROM vtiger_neurofeedback, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_neurofeedback.neurofeedbackid, vtiger_neurofeedback.neurofeedbackname, vtiger_crmentity.smcreatorid FROM vtiger_neurofeedback inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_neurofeedback.neurofeedbackid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('NeuroFeedback',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export NeuroFeedback Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("NeuroFeedback", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_neurofeedback
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_neurofeedback.neurofeedbackid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_neurofeedbackcf on vtiger_neurofeedbackcf.neurofeedbackid=vtiger_neurofeedback.neurofeedbackid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_neurofeedback.neurofeedbackid, vtiger_neurofeedback.neurofeedbackname, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_neurofeedback
					inner join vtiger_contneurofeedbackrel on vtiger_contneurofeedbackrel.neurofeedbackid = vtiger_neurofeedback.neurofeedbackid
					inner join vtiger_contactdetails on vtiger_contneurofeedbackrel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_neurofeedback.neurofeedbackid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_neurofeedback on vtiger_neurofeedback.neurofeedbackid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'NeuroFeedback'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_neurofeedback ON vtiger_neurofeedback.neurofeedbackid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_neurofeedback.neurofeedbackid = $id";
	...
	/**
	* Function to get NeuroFeedback related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('NeuroFeedback',$query,$id);
	}


	  /**
	  * Function to get NeuroFeedback related Quotes
	  * @param  integer   $id  - neurofeedbackid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'neurofeedback_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_neurofeedback.neurofeedbackname from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_neurofeedback on vtiger_neurofeedback.neurofeedbackid=vtiger_quotes.neurofeedbackid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contneurofeedbackrel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contneurofeedbackrel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contneurofeedbackrel"=>"neurofeedbackid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"neurofeedbackid","vtiger_salesorder"=>"neurofeedbackid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_neurofeedback","neurofeedbackid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityNeuroFeedback on vtiger_crmentityNeuroFeedback.crmid=vtiger_neurofeedback.neurofeedbackid and vtiger_crmentityNeuroFeedback.deleted=0
		left join vtiger_neurofeedbackcf on vtiger_neurofeedbackcf.neurofeedbackid = vtiger_neurofeedback.neurofeedbackid
		left join vtiger_groups vtiger_groupsNeuroFeedback on vtiger_groupsNeuroFeedback.groupid = vtiger_crmentityNeuroFeedback.smownerid
		left join vtiger_users as vtiger_usersNeuroFeedback on vtiger_usersNeuroFeedback.id = vtiger_crmentityNeuroFeedback.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_neurofeedback"=>"neurofeedbackid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_neurofeedback"=>"neurofeedbackid"),
			"Quotes" => array("vtiger_quotes"=>array("neurofeedbackid","quoteid"),"vtiger_neurofeedback"=>"neurofeedbackid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("neurofeedbackid","salesorderid"),"vtiger_neurofeedback"=>"neurofeedbackid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_neurofeedback"=>"neurofeedbackid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-NeuroFeedback Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contneurofeedbackrel WHERE neurofeedbackid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
