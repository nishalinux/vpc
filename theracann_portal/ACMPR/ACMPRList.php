...
if ($customerid != '')
{
	  $params = array();
		...
		echo '<div class="col-sm-11"><b style="font-size:20px;">'.getTranslatedString("LBL_ACMPR").'</b></div>';    
	$allow_all = $client->call('show_all',array('module'=>'ACMPR'),$Server_Path, $Server_Path);
	
    if($allow_all == 'true'){
      		echo '<div class="col-sm-1 search-form"><div class="btn-group">
	      			<button type="button" class="btn btn-default dropdown-toggle acmpr_owner_btn" data-toggle="dropdown">
		 				'.getTranslatedString('SHOW').'<span class="caret"></span>
		 			</button>
		 			<ul class="dropdown-menu acmpr_owner">
