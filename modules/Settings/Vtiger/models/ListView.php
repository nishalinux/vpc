...
class Settings_Vtiger_ListView_Model extends Vtiger_Base_Model {
	...
	public function getListViewEntries($pagingModel) {
		...
		for ($i = 0; $i < $noOfRecords; ++$i) {
			...
			/*Ganesh for Log ou time and login time difference calculation*/
			$intime = $row['login_time'];
			$outtime = $row['logout_time'];
			if($outtime != '0000-00-00 00:00:00'){
				$dteStart = new DateTime($intime); 
				$dteEnd   = new DateTime($outtime); 
				$dteDiff  = $dteStart->diff($dteEnd);
				$row['spend_time'] = $dteDiff->format("%H:%I:%S");
			}
			else{
				$row['spend_time'] = '--';
			}
