...
rcmail.register_command('yetiforce.SelectEmailTemplate', function (e) {
	...
	rcmail.register_command('yetiforce.SendEmailTemplate', function (e) {
		...
		if(mailtempid > 0){
			if(arrtomail.length>1){
				for(i=0;i<arrtomail.length;i++){
					var tom = arrtomail[i];
					//for(j=0;j<arrselectedcontact.length;j++){
						//var selectedcontactb = arrselectedcontact[j];
						$.ajax({	
							...
							/*	var grpdata = {
								...
								var data = $.param(grpdata);
								$.ajax({        
									url : 'https://70.33.224.116/BMLG_VTE/modules/OSSMail/roundcube/?',
									type : 'POST',
									dataType: "json",
									data : data,
									success : function(data) {
								...
								var nextdata = "_token="+token+"&_task="+task+"&_action="+action+"&_id="+id+"&_attachments= &_from=11&_to="+tom+"&_cc="+cc+"&_bcc="+bcc+"&_replyto="+replyto+"&_followupto="+followupto+"&_subject="+subject+"&mailtempid="+mailtempid+"&selectedcontact="+selectedcontact+"&editorSelector="+editorSelector+"&_priority="+priority+"&_store_target="+store_target+"&_draft_saveid=&_draft=&_is_html="+is_html+"&_framed="+framed+"&_message="+mess;
								
								$.ajax({
									method:"POST",
									url:rcmail.env.site_URL+"modules/OSSMail/roundcube/?_task=mail&_lang=en_US&_unlock=loading"+dNow+"&_framed=1&"+nextdata,
									success: function(result){
									}
								});
