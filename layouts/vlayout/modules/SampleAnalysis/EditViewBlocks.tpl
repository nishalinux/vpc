        ...
        <form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
            ...
            {if $BLOCK_FIELDS|@count lte 0}{continue}{/if} 
			  <table class="table table-bordered blockContainer showInlineTable equalSplit">
				...
				{* Start Product Details, Date : Jan 10 2018, Dev:Anjaneya *}
				{if $BLOCK_LABEL == 'Phylos Bioscience Genetic Info'}  	
				 			
				<tr class='clsTrProductBlock'> 
					<td class="fieldLabel medium"><label class="muted pull-right marginRight10px">Genotype Kit ID</label></td>
					<td class="fieldValue medium">
						{* data-validation-engine="" data-fieldinfo='' *}
						<select class="chzn-select"  id="idKitname" >
							<option value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>
						{foreach item=KITNAME key=KITID from=$KITLIST}
							<option value="{$KITNAME}" {if $KITNAME eq $REFERENCED_MODULE_NAME} selected {/if} >{$KITNAME}</option>
						{/foreach}
						</select>
					</td>
				</tr>
...
jQuery(document).ready(function(){ 
	 jQuery('#idKitname').change(function(){		
		var kitname = jQuery(this).val();
		 console.log(kitname);
		jQuery('input[name="genotype_kit_id"]').val(kitname);
	 });
	 
	 var kitname = jQuery('input[name="genotype_kit_id"]').val();
	 if(kitname != ''){
		jQuery('#idKitname').val(kitname);
	 
        jQuery('#idKitname').trigger("chosen:updated");
	 }
