...
class MailManager_Folder_View extends MailManager_Abstract_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if($currentUserModel->get('mail_limit')=='' ||$currentUserModel->get('mail_limit')==null || $currentUserModel->get('mail_limit')==0){
			$maxEntriesPerPage = vglobal('list_max_entries_per_page');
		}
		else{
			$maxEntriesPerPage = $currentUserModel->get('mail_limit');
		...
		if ('open' == $this->getOperationArg($request)) {
			...
			//added by jyothi
                 ...
                 $total = $folder->count();
                 $page_count = ceil($total/$maxEntriesPerPage);
                 $viewer->assign('PAGE_COUNT', $page_count);
            //ended here
			...
			$viewer->assign('SEARCHSENTOPTIONS' ,self::getSentSearchOptions());
			...
			$viewer->assign('USER_MAIL_LIMIT', $currentUserModel->get('mail_limit'));
	 ...
	 * @return string
	 */
	public static function getSentSearchOptions() {
		$options = array('TO'=>'TO','FROM'=>'FROM','SUBJECT'=>'SUBJECT','BODY'=>'BODY','BCC'=>'BCC','CC'=>'CC','DATE'=>'ON');
		return $options;
	}
     public static function getSearchOptions() {
		$options = array('FROM'=>'FROM','SUBJECT'=>'SUBJECT','BODY'=>'BODY','TO'=>'TO','BCC'=>'BCC','CC'=>'CC','DATE'=>'ON');
		return $options;
	}
