...
class Vtiger_Util_Helper {
	...
	public static function transformUploadedFiles(array $_files, $top = TRUE) {
		global $log;
		$log->debug('transformUploadedFileszssssssssssssssssssssssssss 24');
		$files = array();
		...
		$log->debug('transformUploadedFileszssssssssssssssssssssssssss 45');
	...
	public static function sanitizeUploadFileName($fileName, $badFileExtensions) {
		global $log;
		$log->debug('santizeuploadfilename line 333');
		$fileName = preg_replace('/\s+/', '_', $fileName);//replace space with _ in filename
		$fileName = rtrim($fileName, '\\/<>?*:"<>|');
		$log->debug('santizeuploadfilename line 336');
		$log->debug($fileName);
