...
if (typeof(MailManager) == 'undefined') {
	...
	MailManager = {
		...
		*/
		...
		/* Open email */
		mail_open: function(folder, msgno, loction){
console.log(loction);
			...
			jQuery('#_mailrow_' + msgno).addClass('mm_normal');
                         ...
                         AppConnector.request(MailManager._baseurl() + "_operation=mail&_operationarg=open&_folder=" + encodeURIComponent(folder) + "&_msgno=" + encodeURIComponent(msgno)+ "loction" +loction).then(function(responseJSON) { 
                                        responseJSON = JSON.parse(responseJSON);
					progressIndicatorElement.progressIndicator({
						'mode' : 'hide'
					})
		...
		mail_reply: function(all){
			...
			//CC email ids issue.--Start

			// TODO Strip invalid HTML?
			var from = jQuery('#_mailopen_from').html();
			var madhu = jQuery('.login_user').val();           //added by madhav
			var cd = all ? jQuery('#_mailopen_cc').html() : '';//added by madhav
			
		
			var md = all ? jQuery('#_mailopen_to').html() : '';//added by madhav
				if(cd != null){
					if (cd.startsWith(madhu) == true && cd.endsWith(",") == true){
						cd = cd.replace(madhu+",", '');
						}else if(cd == madhu && cd.endsWith(",") == false){
							cd = cd.replace(madhu, '');
							} else {
							cd = cd.replace(", "+madhu, '');
							}
					if (md.startsWith(madhu) == true){
						if(md.indexOf(',') == -1){
						md = md.replace(madhu, '');	
						} else {
						md = md.replace(madhu+",", '');
			...
			Vtiger_Index_Js.showComposeEmailPopup(params, function(win){
				if (typeof win != 'undefined') {
					setTimeout(function() {fillComposeEmailForm(win);}, 5000);//murali
				}
			});
		},
		...
		mail_forward: function(messageId){
			...
			//var fwdBody = MailManager.sprintf('<p></p><p style="margin:0;padding:0;">%s, %s, %s:</p>%s<br /><p>%s<br/><p>%s</p>', 'On ' + date, from, 'wrote', MailManager.i18n('JSLBL_FORWARD_MESSAGE_TEXT'), fwdMsgMetaInfo, body); // MODIFIED BY MADHAV
			...
			if(attachmentCount) {
				VtigerJS_DialogBox.block();
					...
					"&body=" + encodeURIComponent(fwdBody)).then(function(responseJSON) {//modified by murali for mail forward content issue
                                            responseJSON = JSON.parse(responseJSON);
