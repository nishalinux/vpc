		...
		if ('find' == $this->getOperationArg($request)) {
			...
			if (empty($linkedto)){
				...
				foreach (self::$MODULES as $MODULE) {
					...
					$aquery = $adb->pquery("select * from vtiger_account, vtiger_crmentity where (email1 = '{$from}' or email2 = '{$from}') and vtiger_crmentity.crmid=vtiger_account.accountid and vtiger_crmentity.deleted=0", array());
