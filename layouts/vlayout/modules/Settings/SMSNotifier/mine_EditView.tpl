	...
	<form class="form-horizontal" id="smsConfig" method="POST">
		<div class="modal-body configContent">
			...
			<div class="control-group">
				...
				{if $FIELD_NAME !='password' & 'username'}
				...
				<div class="controls">
					...
					{else if $FIELD_TYPE == 'password'}
						<input type="hidden" name="{$FIELD_NAME}" class="span3" data-validation-engine="validate[required]" value="{$FIELD_VALUE}" />
					{else}
						<input type="hidden" name="{$FIELD_NAME}" class="span3" {if $FIELD_NAME == 'username'} data-validation-engine="validate[required]" {/if} value="{$FIELD_VALUE}" />
					{/if}
				</div>
			</div>
			{/foreach}
