...
console.log(loction);
		...
		mail_forward: function(messageId){
			...
			if(attachmentCount) {
				var FB = fwdBody.replace(/&lt;/g,'< ').replace(/&gt;/g,' >').replace(/&nbsp;/g,' ').replace(/&amp;/g,'&').replace(/<img[^>]*>/g,"");//added by murali to fix mail body loading issue while forwarding 
				VtigerJS_DialogBox.block();
					...
					"&body=" + FB).then(function(responseJSON) {//modified by murali to fix mail body loading issue while forwarding 
                                            responseJSON = JSON.parse(responseJSON);
						VtigerJS_DialogBox.unblock();
						// Open the draft the was saved.
						if (responseJSON['success']) {
							MailManager.mail_draft(responseJSON['result']['emailid'], true);
							//alert("With Attachment<br>"+FB);//murali
						}
					}
					);
				...
				Vtiger_Index_Js.showComposeEmailPopup(params, function(win){
					if (typeof win != 'undefined') {
						...
						//alert("WithOut Attachment<br>"+fwdBody);//murali
					}
				});
			}
		},
