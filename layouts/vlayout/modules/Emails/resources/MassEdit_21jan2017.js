		...
		if(composeEmailForm.length > 0){
			jQuery("#multiFile").MultiFile({
				...
				'afterFileSelect' : function(element, value, master_element){
					...
					/*-----------*/
					var tmp = thisInstance.getTotalAttachmentsSize();
					tfilesize = parseInt(tmp)+ parseInt(tfilesize);
					var maxUploadSize = thisInstance.getMaxUploadSize();
					var x = newElement.context.id;
					if(parseInt(tmp) > maxUploadSize){
						 jQuery('#' +x).attr('disabled','disabled');
						Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_MAX_FILE_UPLOAD_EXCEEDS'));
					}
					else{
						jQuery('.MultiFile-remove').click(function() {
