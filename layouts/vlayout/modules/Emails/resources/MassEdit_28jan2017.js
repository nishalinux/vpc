...
jQuery.Class("Emails_MassEdit_Js",{},{
	...
	autosave : function(){		
		//setTimeout(function() { 
		//var shiv = window.opener.jQuery('#timer').html();
		//alert(shiv); }, 10000);
	...
	fileAfterSelectHandler : function(element, value, master_element){
		...
		if(totalAttachmentsSize > maxUploadSize){
			//alert(totalAttachmentsSize+'///'+maxUploadSize);//murali
			Vtiger_Helper_Js.showPnotify("Attachment size exceed max upload size. This mail will be saved into Draft automtically..!!");
			//Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_MAX_FILE_UPLOAD_EXCEEDS'));
			//jQuery('#sendEmail').attr('disabled',"disabled");//murali
