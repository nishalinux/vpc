...
{foreach item=DETAIL_VIEW_WIDGET from=$DETAILVIEW_LINKS['DETAILVIEWWIDGET']}
	...
	{elseif ($DETAIL_VIEW_WIDGET->getLabel() eq 'LBL_UPDATES')}
		...
		{elseif ($DETAIL_VIEW_WIDGET->getLabel() eq 'Products')}
		{assign var=PRODUCTS_WIDGET_MODEL value=$DETAIL_VIEW_WIDGET}
...
<div class="row-fluid">
	...
	<div class='span5' style="overflow: hidden">
		...
		{if $PRODUCTS_WIDGET_MODEL}
			<div class="summaryWidgetContainer">
				<div class="widgetContainer_products" data-url="{$PRODUCTS_WIDGET_MODEL->getUrl()}" data-name="{$PRODUCTS_WIDGET_MODEL->getLabel()}">
					<div class="widget_header row-fluid">
						<input type="hidden" name="relatedModule" value="{$PRODUCTS_WIDGET_MODEL->get('linkName')}" />
						<span class="span9 margin0px"><h4 class="textOverflowEllipsis">{vtranslate($PRODUCTS_WIDGET_MODEL->getLabel(),$MODULE_NAME)}</h4></span>
						<span class="pull-right">
							<span class="pull-right">
								{if $PRODUCTS_WIDGET_MODEL->get('action')}
									<button class="btn pull-right addButton createRecord" type="button" data-url="{$PRODUCTS_WIDGET_MODEL->get('actionURL')}">
										<strong>{vtranslate('LBL_ADD',$MODULE_NAME)}</strong>
									</button>
								{/if}
							</span>
