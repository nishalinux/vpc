 ...
 ********************************************************************************/
 // This is the proforma vtDZiner Vegetative.php script which will get converted as the written out as Vegetative.php
...
// vtiger_vegetative is a custom vtiger entity generated by vtDZiner Module DZiner
class Vegetative extends CRMEntity {
	var $log;
	var $db;

	var $module_name="Vegetative";
	var $table_name = "vtiger_vegetative";
	var $table_index= 'vegetativeid';

	var $tab_name = Array('vtiger_crmentity','vtiger_vegetative','vtiger_vegetativecf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_vegetative'=>'vegetativeid','vtiger_vegetativecf'=>'vegetativeid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_vegetativecf', 'vegetativeid');
	...
	var $list_link_field= 'vegetativename';
	...
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'vegetativename');

	//Added these variables which are used as default order by and sortorder in ListView
	var $default_order_by = 'vegetativename';
	var $default_sort_order = 'ASC';

	//var $groupTable = Array('vtiger_vegetativegrouprelation','vegetativeid');
	function Vegetative() {
		$this->log = LoggerManager::getLogger('vegetative');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Vegetative');
	}
...
function insertIntoAttachment($id,$module) {
	...
	*/
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['VEGETATIVE_SORT_ORDER'] != '')?($_SESSION['VEGETATIVE_SORT_ORDER']):($this->default_sort_order));
	...
	* return string  $order_by    - fieldname(eg: 'Vegetativename')
	*/
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['VEGETATIVE_ORDER_BY'] != '')?($_SESSION['VEGETATIVE_ORDER_BY']):($use_default_order_by));
	...
	*/
	function create_list_query($order_by, $where)
	{
		...
		if($account_required)
		{
			$query = "SELECT vtiger_vegetative.vegetativeid,  vtiger_vegetative.vegetativename, vtiger_vegetative.dateclosed FROM vtiger_vegetative, vtiger_account ";
			$where_auto = "vtiger_crmentity.deleted=0 ";
		}
		else
		{
			$query = 'SELECT vtiger_vegetative.vegetativeid, vtiger_vegetative.vegetativename, vtiger_crmentity.smcreatorid FROM vtiger_vegetative inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_vegetative.vegetativeid LEFT JOIN vtiger_groups on vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid ';
			$where_auto = ' AND vtiger_crmentity.deleted=0';
		}

		$query .= $this->getNonAdminAccessControlQuery('Vegetative',$current_user);
		if($where != "")
	...
	* @param reference variable - where condition is passed when the query is executed
	* Returns Export Vegetative Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Vegetative", "detail_view");
		...
		$query = "SELECT $fields_list,case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
				FROM vtiger_vegetative
				inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_vegetative.vegetativeid
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid=vtiger_users.id
				LEFT JOIN vtiger_vegetativecf on vtiger_vegetativecf.vegetativeid=vtiger_vegetative.vegetativeid
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = 'select case when (vtiger_users.user_name not like "") then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_contactdetails.accountid,vtiger_vegetative.vegetativeid, vtiger_vegetative.vegetativename, vtiger_contactdetails.contactid,
					...
					vtiger_crmentity.modifiedtime , vtiger_account.accountname from vtiger_vegetative
					inner join vtiger_contvegetativerel on vtiger_contvegetativerel.vegetativeid = vtiger_vegetative.vegetativeid
					inner join vtiger_contactdetails on vtiger_contvegetativerel.contactid = vtiger_contactdetails.contactid
					inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid
					left join vtiger_account on vtiger_account.accountid = vtiger_contactdetails.accountid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_crmentity.smownerid=vtiger_users.id
					where vtiger_vegetative.vegetativeid = '.$id.' and vtiger_crmentity.deleted=0';
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_activity.activityid as 'tmp_activity_id',vtiger_activity.*,vtiger_seactivityrel.*, vtiger_contactdetails.lastname,vtiger_contactdetails.firstname,
					...
					inner join vtiger_vegetative on vtiger_vegetative.vegetativeid=vtiger_seactivityrel.crmid
	...
	function get_products($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode,
			   ...
			   INNER JOIN vtiger_seproductsrel ON vtiger_products.productid = vtiger_seproductsrel.productid and vtiger_seproductsrel.setype = 'Vegetative'
			   INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid
			   INNER JOIN vtiger_vegetative ON vtiger_vegetative.vegetativeid = vtiger_seproductsrel.crmid
			   WHERE vtiger_crmentity.deleted = 0 AND vtiger_vegetative.vegetativeid = $id";
	...
	/**
	* Function to get Vegetative related Task & Event which have activity type Held, Completed or Deferred.
	* @param  integer   $id
	...
	*/
	function get_history($id)
	{
		...
		return getHistory('Vegetative',$query,$id);
	}


	  /**
	  * Function to get Vegetative related Quotes
	  * @param  integer   $id  - vegetativeid
	...
	function get_quotes($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		if($actions && getFieldVisibilityPermission($related_module, $current_user->id, 'vegetative_id') == '0') {
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) && isPermitted($related_module,4, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_SELECT')." ". getTranslatedString($related_module). "' class='crmbutton small edit' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=0,scrollbars=0');\" value='". getTranslatedString('LBL_SELECT'). " " . getTranslatedString($related_module) ."'>&nbsp;";
			}
		...
		$query = "select case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name,
					vtiger_account.accountname, vtiger_crmentity.*, vtiger_quotes.*, vtiger_vegetative.vegetativename from vtiger_quotes
					inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_quotes.quoteid
					left outer join vtiger_vegetative on vtiger_vegetative.vegetativeid=vtiger_quotes.vegetativeid
					left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid
					left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Activities"=>"vtiger_seactivityrel","Contacts"=>"vtiger_contvegetativerel","Products"=>"vtiger_seproductsrel",
						"Attachments"=>"vtiger_seattachmentsrel","Quotes"=>"vtiger_quotes","SalesOrder"=>"vtiger_salesorder",
						"Documents"=>"vtiger_senotesrel");

		$tbl_field_arr = Array("vtiger_seactivityrel"=>"activityid","vtiger_contvegetativerel"=>"contactid","vtiger_seproductsrel"=>"productid",
						"vtiger_seattachmentsrel"=>"attachmentsid","vtiger_quotes"=>"quoteid","vtiger_salesorder"=>"salesorderid",
						"vtiger_senotesrel"=>"notesid");

		$entity_tbl_field_arr = Array("vtiger_seactivityrel"=>"crmid","vtiger_contvegetativerel"=>"vegetativeid","vtiger_seproductsrel"=>"crmid",
						"vtiger_seattachmentsrel"=>"crmid","vtiger_quotes"=>"vegetativeid","vtiger_salesorder"=>"vegetativeid",
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"vtiger_vegetative","vegetativeid");
		$query .= " left join vtiger_crmentity as vtiger_crmentityVegetative on vtiger_crmentityVegetative.crmid=vtiger_vegetative.vegetativeid and vtiger_crmentityVegetative.deleted=0
		left join vtiger_vegetativecf on vtiger_vegetativecf.vegetativeid = vtiger_vegetative.vegetativeid
		left join vtiger_groups vtiger_groupsVegetative on vtiger_groupsVegetative.groupid = vtiger_crmentityVegetative.smownerid
		left join vtiger_users as vtiger_usersVegetative on vtiger_usersVegetative.id = vtiger_crmentityVegetative.smownerid";
		return $query;
	}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Calendar" => array("vtiger_seactivityrel"=>array("crmid","activityid"),"vtiger_vegetative"=>"vegetativeid"),
			"Products" => array("vtiger_seproductsrel"=>array("crmid","productid"),"vtiger_vegetative"=>"vegetativeid"),
			"Quotes" => array("vtiger_quotes"=>array("vegetativeid","quoteid"),"vtiger_vegetative"=>"vegetativeid"),
			"SalesOrder" => array("vtiger_salesorder"=>array("vegetativeid","salesorderid"),"vtiger_vegetative"=>"vegetativeid"),
			"Documents" => array("vtiger_senotesrel"=>array("crmid","notesid"),"vtiger_vegetative"=>"vegetativeid"),
		);
	...
	function unlinkDependencies($module, $id) {
		global $log;
		/*//Backup Activity-Vegetative Relation
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts') {
			...
			$sql = 'DELETE FROM vtiger_contvegetativerel WHERE vegetativeid=? AND contactid=?';
			$this->db->pquery($sql, array($id, $return_id));
